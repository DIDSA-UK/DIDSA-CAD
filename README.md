...
**Stage 0 (environment de-risking) — in progress.**

- Confirmed `pythonocc-core` 7.9.3 (`novtk` build) is published on conda-forge for both `linux-64` and `linux-aarch64`, so the Pi 5 (arm64) target is viable.
...
- `.github/workflows/stage0-verify.yml` builds the backend image for `linux/amd64` and `linux/arm64` (via QEMU) and runs the smoke tests on both, since arm64 hardware isn't available in this dev environment.
