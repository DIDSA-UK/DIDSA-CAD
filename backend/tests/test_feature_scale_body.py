...
def _make_surface(part_id: str, *, x0: float, y0: float = 0.0, size: float = 10.0) -> str:
    ...
    not valid targets for... scale body... and should be")."""
    before = set(_body_ids(part_id))
    sketch = _create_square_sketch_feature(part_id, x0=x0, y0=y0, size=size)
    response = client.post(
...
def test_scale_body_accepts_a_surface_body_id():
    """Bug fix ("Surfaces are not valid targets for... scale body... and
    should be"): `resolve_scale_body_from_bodies`'s own
    `BRepBuilderAPI_Transform` call is shape-agnostic, so a Surface's shell
    scales exactly like a solid Body's does - the old `!= Produces.BODY`
    check in `_validate_scale_body_factor` was purely this validation's own
    gap."""
    part = _create_part()
    ...
    assert response.status_code == 201, response.text
    ranges = _bbox_ranges(part["id"], body_id)
    # A 10x10 (XY) x 0..10 (Z) surface centred at (5,5,5), scaled by 2,
    # spans -5..15 on every axis - same "extent doubles about its own
    # centre" contract the solid-Body test below asserts.
    for min_v, max_v in ranges:
        assert abs(min_v - (-5.0)) < 1e-6
        assert abs(max_v - 15.0) < 1e-6
