"""Prompt D: pure-Python tests for `app.document.graph.build_feature_graph`'s
new `FilletFeature` dependency edge - depends on the owning ExtrudeFeature(s)
of every `edge_refs` entry's Body, deduplicated. Has zero OCCT dependency
(mirrors every other graph-only test file in this project), so this runs
...
def test_fillet_depends_on_the_owning_extrude_feature_of_its_edges():
    ...
    fillet_node = next(n for n in nodes if n.id == "fillet1")
    assert fillet_node.depends_on == (extrude_id,)
    assert topological_order(nodes).index(extrude_id) < topological_order(nodes).index("fillet1")


def test_fillet_referencing_a_split_body_id_depends_on_the_base_extrude_feature():
    """A `#N` split-index suffix (see `app.document.graph.base_feature_id`)
    must be stripped before the dependency edge is built - the Fillet
    depends on the ExtrudeFeature, not the literal (possibly-suffixed) Body
    id string."""
    part, _sketch_feature_id, extrude_id = _part_with_sketch_and_extrude()
    fillet = FilletFeature(id="fillet1", edge_refs=[_edge_ref(f"{extrude_id}#0", 0)], radius=1.0)
    part.add_feature(fillet)
...
def test_cascade_deleting_the_extrude_takes_the_fillet_with_it():
    part, _sketch_feature_id, extrude_id = _part_with_sketch_and_extrude()
    fillet = FilletFeature(id="fillet1", edge_refs=[_edge_ref(extrude_id, 0)], radius=2.0)
    part.add_feature(fillet)

    nodes = build_feature_graph(part)
    assert transitive_dependents(nodes, extrude_id) == {extrude_id, "fillet1"}


def test_deleting_an_unrelated_extrude_leaves_the_fillet_alone():
    ...
    assert "fillet1" not in transitive_dependents(nodes, other_extrude.id)
...
def test_fillet_with_edges_spanning_two_bodies_depends_on_both_owning_extrudes():
    """`build_feature_graph` itself doesn't enforce the "must be one Body"
    rule (that's `app.document.fillet._mixed_body_selection`, an OCCT-
    ...
    nodes = build_feature_graph(part)
    fillet_node = next(n for n in nodes if n.id == "fillet1")
    assert set(fillet_node.depends_on) == {extrude_id, other_extrude.id}
