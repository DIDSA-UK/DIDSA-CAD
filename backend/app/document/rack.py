...
def _rack_outline_points(feature: RackFeature) -> list[tuple[float, float]]:
    ...
    try:
        ...
        tooth_points = full_rack_profile_points(rack_geometry, feature.tooth_count)
    except GearGeometryError as exc:
        raise _invalid_rack_parameters(str(exc)) from exc

    backing_height = (
        feature.backing_height if feature.backing_height is not None else default_rack_backing_height(feature.module)
    )
    if backing_height <= 0:
        raise _invalid_rack_parameters(f"backing_height must be positive, got {backing_height!r}")

    min_x = min(x for x, _ in tooth_points)
    max_x = max(x for x, _ in tooth_points)
    bottom_y = -rack_geometry.dedendum_height - backing_height
...
def resolve_rack_from_bodies(
    ...
    outline_points = _rack_outline_points(feature)
