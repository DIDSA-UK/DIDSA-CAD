...
second, larger open item on top of that: Phases A-D2 are built (Assembly
mode really recognizes distinct parts, confirms the breakdown, and
generates/saves each one as its own file; a Mate's `edge_selector` can now
resolve against a placed component's own real geometry, not just the root
Part's, with zero new wire payload — the spike found the backend's own
per-session `Document` already held everything needed). Only Phase E
(real assembly creation/insert/mate) remains — locked in design, not yet
built, now genuinely unblocked — see
...
| 13 | `13-multi-part-assembly-overhaul.md` | 1, 2, 3, 5 | **Partially built.** Phases A (Multi-body Part / Assembly mode toggle), B (multi-image upload per turn), C (mandatory project folder + auto-naming convention), D (multi-part orchestration — recognizes distinct parts, confirms the breakdown, then generates and saves each as its own real file), and D2 (Document-scoped mate edge/face resolution — a real, confirmed gap in `docs/assembly-scope.md` §2q's own Phase-14 mate edge-selector work, closed with **zero new wire payload**: the backend's own per-session `Document` already held every Part a composed graph pulled in) are built. Only E (assembly creation/insert/mate/open) remains — locked in design but not yet built, now genuinely unblocked since D2 closed the mate-resolution gap it depended on — see that file's own Appendix for the live gap/emergent-work tracker |
...
| 14 | Workstream 13, Phase D2 (Document-scoped mate edge/face resolution) — **done** | A Mate step's `edge_selector` can now resolve against a placed Occurrence's own real target Part, not just the root Part's - zero client changes, zero new wire payload (the spike found the backend's own per-session `Document` singleton already held everything needed) |
| 15+ | Workstream 13, Phase E (real assembly creation/insert/mate/open) | The remaining step: N saved part files -> a real, mated, multi-file assembly, opened and ready to use. Now genuinely unblocked (D and D2 both done) — see `13-multi-part-assembly-overhaul.md` |
