...
| 12 | `12-provenance-edge-selectors.md` | 3, 5 | **Spike run, schema locked; implementation not yet built.** On-device testing (`docs/status.md`'s 2026-09-01 entry) found the four existing Fillet/Chamfer edge selectors are a real ceiling (no single-arbitrary-edge targeting). A real-OCCT spike (same day) confirmed two new selectors — `edge_from_sketch_point`/`edge_from_sketch_line`, resolved via OCCT shape-history (`.Generated()`/`.Modified()`), reusing an idiom already shipped in `app.document.gear`'s own root-fillet code — work on Extrude/Revolve/Sweep, with two confirmed real gotchas now written into the doc. Backend wiring (`body_cache.py` extension, `ai_plan_edges.py` resolver) is still real, unbuilt work |
