...
| Phase 2 chunk 1 (`planetary_gear.py` pooling) | dispatched | branch `claude/lod-phase2-planetary-pooling`, session `session_01PDptGJHTxfjN3tQT6BCkMD` | Independent — adds `ProcessPoolExecutor` pooling mirroring `bevel_pair.py`'s pattern, a real perf win and cancellation prerequisite. |
| Phase 2 chunk 2 (job store + job-mode endpoint, `BevelPairFeature`) | dispatched | branch `claude/lod-phase2-bevel-pair-jobs`, session `session_01WncWBfKiheRrgzd659yAKP` | The primary Phase 2 piece — in-memory job store, kickoff/poll/cancel endpoints, cancellation via terminating `ProcessPoolExecutor` worker processes. Independent of chunk 1 (bevel_pair already has pooling). |
| Phase 2 chunk 3 (extend job-mode to `PlanetaryGearFeature`) | not yet dispatched | — | blocked on chunk 1 merging |
| Phase 2 chunk 4 (client: job-mode create/poll/cancel, resume-on-reconnect) | not yet dispatched | — | blocked on chunk 2 merging at minimum |
