...
| Phase 1 chunks 3+4+5 (Pattern/Loft coarse builders + client toggle) | code-reviewed, fix-up in progress | branch `claude/lod-pattern-loft-client`, [PR #175](https://github.com/DIDSA-UK/DIDSA-CAD/pull/175) | Implemented and PR opened directly by the session (real OCCT: 1909→1919; real Flutter: `analyze` 0 issues, `test` 1453 passed/0 failed). Coordinator code review found a real design deviation: the coarse-tier fetch was wired to fire on all ~38 `_refreshMesh()` call sites (every edit), not just Part re-open as designed — undermining the responsiveness LOD is meant to deliver — plus a smaller gap (coarse fetch skipping `hiddenFeatureIds`/`rollbackExcludedFeatureIds`/`meshQuality` every other fetch passes). Fix-up session `session_01QSnYQwEs5PmeBR73Ft2RUA` dispatched 2026-08-27, continuing on the same branch. Also merged forward against `main` once already (a trivial docs-only conflict, my own coordinator-branch merge to `main` via PR #176 landed in between). **Not yet merged.** |
