...
**Toolchain bootstrap**: same established recipe every prior session above used - `micro.mamba.pm` blocked, micromamba's own GitHub Releases download-asset URL and `conda.anaconda.org` both reachable. Built the real `didsacad` conda-forge env from `backend/environment.yml` (`pythonocc-core=7.9.3=novtk*`). Full pre-existing backend suite re-run against it before starting any new code: 1385/1385, confirming the toolchain and establishing a clean baseline.
