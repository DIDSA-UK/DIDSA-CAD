...
**Honest verification status - no `pythonocc-core` in this sandbox** (this project's own recurring caveat, stated above, applies in full here): `gear_math.py` itself is fully, genuinely tested (35 passing tests). Everything touching real OCCT (`gear.py`, and the `extrude.py`/`router.py` wiring around it) could only be checked via `py_compile`/`ruff` (clean) and, for the OCCT-free parts of the import graph, actually importing them after installing `fastapi`/`pydantic` via pip (`graph.py` imports cleanly end to end, confirming `GearFeature`'s dependency-graph wiring is structurally sound; `pythonocc-core` itself has no pip-installable path, conda-forge only, so `gear.py`/`extrude.py`/`router.py`'s own OCCT-touching code was never actually executed). One real bug was caught and fixed by careful self-review despite this - a wire-vertex-traversal ambiguity in the root-fillet edge-picking logic (`TopExp_Explorer` over a whole wire's vertices doesn't guarantee one entry per unique vertex, the exact reason this codebase's own existing code reaches for `TopTools_IndexedMapOfShape` elsewhere) was rewritten to walk edges instead, which have no such ambiguity. Needs a real on-device/CI pass (this repo's CI does have `pythonocc-core`) before being trusted, same as every other genuinely new OCCT technique in this project's own history.
