...
## 2026-07-30 — Phase 8 (Feature pattern and feature mirror - Cut/Boss into a shared target) implemented

New session, new branch (`claude/pattern-mirror-phase-8-ata2xr`, from latest `main`). Asked to implement Phase 8 of `docs/pattern-mirror-scope.md` (§2.11/§4): a third, mutually-exclusive seed-picking mode on both `MirrorFeature`/`PatternFeature` - `tool_feature_id`, naming an upstream Extrude/Revolve/Sweep Cut/Boss-into-target Feature instead of a Body/Feature-tree Body source, so Pattern/Mirror can repeat/reflect that Feature's own Cut/Boss *effect* into its own shared target rather than producing independent Body copies (the scope doc's own headline example: a plate with one off-center hole, mirrored, ends up with two holes in the same plate). Unlike Phase 7, this phase already carried a "Status: scoped" marker, but the task still asked for a full read of the scope doc and every 2026-07-30-onward `status.md` entry first (Phase 7's four on-device-feedback rounds), per the task's own instruction - done before writing any code.

**Backend** (`backend/app/document/`):
- `tool_feature_id: str | None = None` added to both `MirrorFeature`/`PatternFeature` (`models.py`) - mutually exclusive with `source_body_ids`/`source_feature_ids`, `merge` forced away from `KEEP_SEPARATE` (there is exactly one target by construction once it's set).
- New `resolve_feature_tool_shape` (`extrude.py`, next to `_apply_boss_or_cut`) - factors the tool-shape computation out of `compute_part_bodies`'s three existing inline `ExtrudeFeature`/`RevolveFeature`/`SweepFeature` branches (each previously did its own sketch-feature lookup + resolver call inline) into one shared function returning `(tool_shape, target_body_ids, is_cut) | None`. The three branches were refactored to call it, preserving each branch's own pre-existing exception-tolerance policy exactly (Extrude's narrow `invalid_profile_ref`-only catch, Revolve's blanket catch, Sweep's four-error-type catch) - verified by the full pre-existing test suite passing completely unchanged, confirming the extraction didn't regress anything.
- New `app.document.graph.tool_feature_qualifies(feature)` - a pure, OCCT-free predicate (an Extrude/Revolve/Sweep Feature in Cut mode, or Boss mode with a non-empty `target_body_ids` - a targetless Boss has no shared-target problem, the ordinary seed path already copies it correctly) - shared by the router's own eager validation and `mirror.py`/`pattern.py`'s own recompute-time drift check, the same "validate eagerly, tolerate drift at recompute" split every other reference kind in this codebase already uses.
- New `resolve_mirror_tool_feature_from_bodies`/`resolve_pattern_tool_feature_from_bodies` (`mirror.py`/`pattern.py`) - resolve the referenced Feature's tool shape, mirror/pattern it (Pattern reuses `_rectangular_instances`/`_circular_instances` unchanged, passing the tool shape in as their own `source` parameter - index 0 is already baked into the target from the seed Cut/Boss's own earlier run, so only `count-1` copies are computed), union every realized copy into one combined tool, then apply a single `BRepAlgoAPI_Cut`/`BRepAlgoAPI_Fuse` (matching the referenced Feature's own mode) against the target's current shape - not N separate booleans. `compute_part_bodies`'s `MirrorFeature`/`PatternFeature` branches gained a `tool_feature_id`-dispatch at the top, registering the result in place via `_register_solids` (mirrors `_apply_boss_or_cut`'s own Cut-mode shape).
- New `invalid_tool_feature_ref` (422) - a shared-shape structured error (independently defined in `router.py`/`mirror.py`/`pattern.py`, matching this codebase's own established per-module duplication convention for small identically-shaped errors), raised both by the router's new `_validate_tool_feature_id` (eager: mutual exclusivity, `merge != KEEP_SEPARATE`, `tool_feature_qualifies`) and by the two new resolvers (recompute-time drift).
- New `_tool_feature_dependency`-equivalent edges in `_mirror_dependencies`/`_pattern_dependencies` (`graph.py`) - a bare `tool_feature_id` add, mirroring `source_feature_ids`' own identical treatment, so cascade-delete and topological ordering both work for free.
- `schemas.py`/`native_format.py`: `tool_feature_id` threaded through Create/Update/Response schemas and native-format export/import (defaulting to `None` for any pre-Phase-8 save).
- New `backend/tests/test_stage_p_feature_pattern_mirror.py` (17 tests): the scope doc's own headline plate-with-hole scenario for both Mirror and Pattern (verified via a divergence-theorem volume helper over the real tessellated mesh, not just body-count/bbox checks), a Boss-into-target variant (proving `is_cut=False` fuses rather than cuts), `skip_indices` applying in tool mode, mutual-exclusivity/merge/eligibility validation (wrong Feature type, targetless Boss, non-existent id, a Fillet specifically - confirming Fillet/Chamfer stay correctly out of scope), cascade delete via the new dependency edge, and a PATCH omitted-`tool_feature_id`-leaves-it-unchanged case. Plus 2 new `test_stage_native_format.py` tests (round-trip, and pre-Phase-8-save backward compatibility). Verified for real: full backend `pytest` suite 1182/1182 (1163 prior + 19 new) against genuine `pythonocc-core`.

**Client** (`client/lib/viewport3d/`, `client/lib/api/document_api_client.dart`):
- `FeatureDto.toolFeatureId`/`createMirrorFeature`/`updateMirrorFeature`/`createPatternFeature`/`updatePatternFeature` all gained a `toolFeatureId` field/parameter, mirroring every other optional field's own JSON-wire shape.
- The third seed-picking path shipped as two new long-press context-menu entries - "Pattern into Target"/"Mirror into Target" (`feature_context_menu.dart`'s new `FeatureContextMenuAction.patternIntoTarget`/`.mirrorIntoTarget`), gated on a new `_isEligibleToolFeature` (`part_screen.dart`) computed purely from the already-fetched Feature list's own `extrudeType`/`mode`/`targetBodyIds` fields (mirrors the backend's `tool_feature_qualifies` exactly, no extra network round-trip) - a strictly narrower gate than the existing Phase-6 "Pattern" entry's own `_bodyProducingFeatureTypes` check. Tapping either reuses the identical long-press-to-`configuring`/`pickingPlane` shortcut Phase 6's own "Pattern from Feature" entry already established (`_openPatternPanelFromToolFeature`/`_openMirrorPanelFromToolFeature`, new siblings of `_openPatternPanelFromFeature`/`_openMirrorPanel`), setting `tool_feature_id` and forcing `merge` to `fuseIntoOne` up front. This UI shape (a long-press shortcut rather than a mode toggle embedded in the existing guided `pickingBodies` flow) was the one piece the scope doc's own §2.11 flagged as not fully speculated - chosen because Mirror/Pattern's existing panels only ever render *after* a seed is already committed (there is no in-panel "switch seed mode" affordance today), so a fresh, direct entry point mirroring Phase 6's own precedent was the lower-risk choice, consistent with this doc's own established pattern of choosing the already-proven shape over a novel one.
- `MirrorPanel`/`PatternPanel` both gained a `toolFeatureSummary` field - when set, replaces the ordinary merge-toggle/sourceFeatureIds-from-tree row with a single status line ("Mirroring/Patterning `<Feature>` into its own target - always merges"), since neither is user-editable in this mode.
- `_resetPatternConfiguringState` (the shared reset function `_confirmPatternBodySelection`/`_openPatternPanel`/`_openPatternPanelFromFeature` all already used) gained an optional `toolFeatureId` parameter, forcing `_patternMerge` to `fuseIntoOne` when set - `_openPatternPanelFromToolFeature` is a thin new caller. `_openMirrorPanelFromToolFeature` mirrors `_openMirrorPanel`'s own shape directly (Mirror's flow was never centralized through one reset function the way Pattern's is). Every existing Mirror/Pattern reset/cancel/confirm/edit-open site (`_startMirrorPicker`, `_openMirrorPanel`, `_openMirrorPanelForEdit`, `_confirmMirror`, `_cancelMirror`, `_startPatternPicker`, `_openPatternPanelForEdit` (both Rectangular and Circular branches), `_confirmPattern`, `_cancelPattern`) was individually audited and widened to reset/reconstruct/revert `_mirrorToolFeatureId`/`_patternToolFeatureId` alongside the existing `source_body_ids`/`source_feature_ids` state, plus the "nothing seeded yet" bail-out guards in `_ensureMirrorFeatureExists`/`_scheduleMirrorPreview`/`_ensurePatternFeatureExists`/`_schedulePatternPreview` (all of which previously bailed on "both `source_body_ids` and `source_feature_ids` empty," which is also exactly `tool_feature_id` mode's own shape) and the `_mirrorEditSnapshot`/`_patternEditSnapshot` record types (widened with `toolFeatureId`, threaded through Cancel's revert-PATCH).
- New/widened tests: 2 new `mirror_panel_test.dart` tests and 2 new `pattern_panel_test.dart` tests (`toolFeatureSummary` shown replaces the merge toggle/sourceFeatureIds row; null shows the ordinary controls). 3 new `part_screen_test.dart` tests - long-pressing an eligible Cut offers both new actions and tapping "Pattern into Target" opens `PatternPanel` seeded via `tool_feature_id` (mirrors the existing Phase 6 "long-press Pattern" test's own shape and fake-backend seed data); tapping "Mirror into Target" opens `MirrorPanel` directly at `pickingPlane` with Confirm correctly still disabled until a plane is picked; long-pressing a targetless Boss offers the ordinary "Pattern" entry but not either new one. None of these advance far enough to trigger an actual `createMirrorFeature`/`createPatternFeature` network call (Pattern's own `direction_1`/Mirror's own `mirror_plane` stay unset in each test), so - unlike the carried-over note in this file's own 2026-07-30 Phase-6-round entry about the fake backend having no POST handler for `mirror-features`/`pattern-features` - this pass didn't need to extend `_FakeDocumentBackend` to get real coverage; that gap remains open for a future pass that needs to drive a `tool_feature_id` Feature all the way through creation.

**Toolchain bootstrap**: no `pythonocc-core`/Flutter SDK preinstalled in this sandbox (same starting point every prior phase's own session hit). `micro.mamba.pm` was blocked by this session's own egress policy (403) - installed micromamba instead from `github.com/mamba-org/micromamba-releases`' own GitHub Releases asset (matching Phase 7's own fallback, per that entry), built the `didsacad` conda-forge env from `backend/environment.yml`. For the client, `git clone --depth 1 --branch master https://github.com/flutter/flutter.git` (matching `.github/workflows/client-verify.yml`'s own `channel: master` pin).

Verified for real: full backend `pytest` suite 1182/1182 (1163 prior + 19 new) against genuine `pythonocc-core`; `flutter analyze` clean on the whole client project; full client `flutter test` suite 1077/1077 (1070 prior + 7 new, 7 skipped, pre-existing GPU/Impeller-unavailable skips unrelated to this change). Not yet confirmed on a real device - no phone attached to this sandbox session.

This closes out Phase 8 of `docs/pattern-mirror-scope.md`'s own phased plan. Phase 9+ (pattern-along-a-curve, sketch-driven/table-driven pattern, fill pattern, varying instance spacing, a standalone `CreateAxisFeature`, Fillet/Chamfer feature-pattern/mirror, per-instance independent sketch-pattern editing, equation-driven instance parameters) remain explicitly deferred, not scheduled.

## 2026-07-31 — Bug fix: face-anchored Plane and Sketch external references drifting to the wrong face after a later, unrelated Cut

Phase 8 was merged to `main` (PR #125), then the user tested it on a real device and reported no Pattern preview/no holes after confirming. While reproducing that, the user separately reported (with screenshots, confirmed to already be wrong *before* attempting any Pattern operation - ruling out Phase 8 code entirely) that a Sketch's own circle and its host `CreatePlaneFeature` were rendering on completely different faces than where they were originally placed: "created the plate, then cut the hole, then tried to pattern the hole."

**Root cause**: `_create_plane_feature_response` and `_sketch_has_lost_reference` (`router.py`) both resolve an already-persisted Feature's own stored `SubShapeRef`/`ExternalVertexReference` by calling `compute_part_bodies(part)` (via `resolve_create_plane`/`refresh_external_references`) with *no exclusions* - i.e. against the Part's fully-built, final state, on every `GET .../features` call. A raw OCCT face/edge/vertex index (`topexp.MapShapes`'s own enumeration order over one specific `TopoDS_Shape`) is stable only when resolved against the same shape snapshot it was captured against, not a persistent label. A *later* Boolean (a Cut/Boss into the same Body a Plane is anchored to, or that a Sketch's own external reference points at) can restructure that Body's B-rep enough that the identical index now lands on a genuinely different face/edge/vertex once the whole Part is recomputed, even though the stored reference itself never changed. The Sketch case is the more serious half: `refresh_external_references` doesn't just display the wrong value, it *writes* the newly-(mis)resolved position straight onto the Sketch's own persisted Point data (`sketch.points[point_id].x/y`) on every feature-list read, silently corrupting real geometry.

A full grep sweep of every `compute_part_bodies(part, ...)` call site confirmed these were the *only* two vulnerable spots - the `/mesh` endpoint and live-picking endpoints for external references/Convert Entities legitimately want the full/current state, and every other resolver (Revolve/Mirror/Pattern/Fillet/Chamfer/Sweep validation) is only ever reachable for a brand-new or last/rollback-scoped Feature, where "full state" and "state so far" already coincide.

**Fix**: new `_excluded_feature_ids_after(part, feature_id)` helper - every Feature id that comes *after* `feature_id` in `part.features`' own append-only, creation-order list (`Part`'s own docstring: a Feature can only be edited/deleted while it is the last one in the list). Both call sites now pass this as their `excluded_feature_ids` set. A first attempt used `transitive_dependents(build_feature_graph(part), feature_id)` (B2's own cascade-delete graph query) instead - proven wrong by direct reproduction before ever being trusted: it only excludes Features that *explicitly reference* `feature_id` by id, missing the actual reported case entirely - a later Cut sharing only a Body with the Plane/Sketch, no graph edge at all (the Cut's own `target_body_ids` references the Body directly). List position is always a safe superset of graph dependents (nothing can depend on a Feature that doesn't exist yet), so "exclude everything after" can never accidentally exclude something a Feature actually needs, while correctly covering coincidental same-Body siblings that graph dependents alone cannot.

New `backend/tests/test_bugfix_stale_feature_resolution.py` (3 tests, run for real against genuine `pythonocc-core` in this sandbox): reproduces the exact reported shape - a face-anchored Plane (independent of any sketch), a separate Sketch with an external vertex reference onto the same plate, and a third, unrelated plain-XY Sketch + Cut into that same plate, with no dependency edge between the Cut and either the Plane or the reference Sketch. Confirmed each test genuinely exercises the bug (not just re-asserting current behavior) by running the suite against the pre-fix code first: all 3 fail without the fix (plane origin/normal drift to a different face; the external-reference point silently relocates; `has_lost_reference` stays `False` while the point itself is wrong, since it resolves to *some* valid-looking vertex, just the wrong one) and all 3 pass with it. A third test confirms repeated `GET .../features` calls don't progressively re-corrupt the external-reference point once resolution is correct.

Verified for real: full backend `pytest` suite 1185/1185 (1182 prior + 3 new) against genuine `pythonocc-core`. Client untouched, not re-run. Branch: `claude/fix-stale-feature-resolution` (fresh from `main` post-Phase-8-merge, per this project's own "merged branch restarts fresh" convention).

## 2026-07-31 — Bug fix: Pattern/Mirror `tool_feature_id` mode re-deriving an upstream Cut's tool shape against the wrong face (third instance of the same drift bug, this time corrupting real geometry)

Merged PR #126 (the fix above) to `main`, then the user actually tried the Phase 8 workflow the earlier reports were about - "pattern a hole in a plate" via a `tool_feature_id`-seeded Pattern - and it still produced no visible second hole/preview. Asked the user two diagnostic questions (does Confirm change anything; do they control the backend deploy) and one experiment (try a much smaller spacing) - "nothing changes" and "already small enough" ruled out both a stale-deployment explanation and an off-body-placement explanation, pointing at a real bug rather than a deployment or geometry-scale issue. A direct reproduction (plate + off-center hole Cut + `tool_feature_id` Pattern along a real plate edge) against real `pythonocc-core` initially *worked* - proving the Phase 8 geometry logic sound in general - but a second reproduction adding back the detail from the original report (`sketch>extrude>plane>sketch>cut>pattern` - the hole's Sketch hosted on a face-anchored `CreatePlaneFeature`, not a fixed plane) reproduced the bug exactly: the Cut appeared to silently no-op (vertex count unchanged). The user independently found the same bug from the other direction with a real 100x200mm plate and a small non-zero spacing, and reported (with a screenshot) a small stray cut appearing in the wrong corner instead of a second hole - "it looks like the cylinder is in the wrong orientation."

**Root cause**: a *third* instance of the exact same underlying bug class as the Plane/Sketch display-drift fix above, but this time in Phase 8's own new code and corrupting real, computed geometry rather than just a displayed value. `app.document.pattern.resolve_pattern_tool_feature_from_bodies`/`app.document.mirror.resolve_mirror_tool_feature_from_bodies` both call `app.document.extrude.resolve_feature_tool_shape` to re-derive the referenced upstream `tool_feature_id`'s own standalone, pre-boolean tool shape - which means re-resolving *that Feature's own* upstream dependencies (its Sketch's anchor, when it's a face-anchored Plane rather than a fixed plane) from scratch. Both resolvers passed their *own* `bodies`/`excluded_feature_ids` straight through to that re-derivation - i.e. the Part as of the *calling* Pattern/Mirror's own position in the walk, which is *after* `tool_feature_id`'s own Cut/Boss already ran. Confirmed by direct comparison: the anchor Plane's own resolved basis differed between the two calls - `origin=(0,0,5), normal=(0,0,1)` (correct, resolved when the Cut was first created) vs. `origin=(20,45,0), normal=(0,1,0)` (wrong - a different face of the same box) when re-derived from the Pattern's own later position - the same "OCCT face index isn't a persistent label" problem the Plane/Sketch fix above already diagnosed, just hitting the *internal geometry-construction* path this time instead of a display/refresh path, because Phase 8's `tool_feature_id` mode is the one place that deliberately re-derives an *earlier* Feature's own construction from a *later* point in the walk (needed to get an independent, transformable copy of its tool shape).

**Fix**: `excluded_feature_ids_after` (the list-position-based helper the Plane/Sketch fix introduced, previously private to `router.py`) moved to `app.document.graph` as a shared, public helper. Both `resolve_pattern_tool_feature_from_bodies`/`resolve_mirror_tool_feature_from_bodies` now compute a *separate* bodies snapshot - the Part exactly as it stood right before `tool_feature_id` itself ran (`excluded_feature_ids | {tool_feature_id} | excluded_feature_ids_after(part, tool_feature_id)`) - and resolve `resolve_feature_tool_shape` against that snapshot, instead of the calling Pattern/Mirror's own further-advanced accumulator. The actual Cut/Fuse boolean at the end of each resolver still correctly uses the *current* accumulator (the target body as it stands right now) - only the tool_feature's own pre-boolean shape needed the earlier snapshot.

New `backend/tests/test_bugfix_pattern_mirror_tool_feature_stale_basis.py` (2 tests, run for real against genuine `pythonocc-core`): reproduces the full `sketch>extrude>plane>sketch>cut>pattern`/`...>mirror` chain from the original report and confirms the resulting mesh's vertex count actually changes (a genuine second hole, not a no-op or a stray cut) for both Pattern and Mirror's `tool_feature_id` mode. Confirmed both tests fail without the fix (`360 != 360` - i.e. no change at all, matching the on-device "nothing happens" report) and pass with it.

Verified for real: full backend `pytest` suite 1187/1187 (1185 prior + 2 new) against genuine `pythonocc-core`, including the full pre-existing Pattern/Mirror suite (200 tests) re-run clean. Client untouched, not re-run. Same branch: `claude/fix-stale-feature-resolution`.

## 2026-07-31 — Phase 9 (Pattern/Mirror entry-point unification) implemented, plus a client-only fix for the XYZ triad hidden behind any open tool panel

New session, new branch (`claude/flutter-pattern-mirror-triad-gtvssy`, from latest `main`). Two independent, client-only Flutter UX fixes requested together: Phase 9 of `docs/pattern-mirror-scope.md` (§2.12/§4 - written up as part of this same session, since neither section existed yet despite the task description referencing them) and the `docs/roadmap.md`-scoped triad-visibility bug (also not yet present as its own roadmap entry - the task's inline description served as the authoritative scope for both, verified accurate against the actual code state before starting). No backend/Python work in this session at all.

**Phase 9 - Pattern/Mirror entry-point unification** (`client/lib/`):
- `FeatureContextMenuAction.patternIntoTarget`/`.mirrorIntoTarget` removed from `feature_context_menu.dart`. The plain `pattern` action's gate (`_onFeatureLongPress`'s `showPattern`, `part_screen.dart`) widened to the union of `_bodyProducingFeatureTypes.contains(feature.type)` and `_isEligibleToolFeature(feature)`; a new `mirror` action (`showMirror`, same widened gate) is the entry-point-asymmetry fix itself - Pattern always had a "seed from this Feature" long-press entry since Phase 6, Mirror never did.
- New `_openMirrorPanelFromFeature` (mirrors `_openPatternPanelFromFeature`'s own shape, which itself absorbed the old `_openPatternPanelFromToolFeature`): seeds via `tool_feature_id` when the long-pressed Feature is tool-feature-eligible, `source_feature_ids` otherwise.
- New shared `PatternMirrorSeedKind` enum (`document_api_client.dart`, next to `MergeMode`) backs a new "Pattern Body"/"Pattern Feature" (`MirrorPanel`: "Mirror Body"/"Mirror Feature") `SegmentedButton` toggle inside each panel - shown only when the long-pressed Feature (tracked via new `_patternLongPressSeedFeature`/`_mirrorLongPressSeedFeature` fields, null for every other seed path) qualifies for *both* seed kinds; hidden otherwise, since there's nothing to toggle. `PartScreen._patternSeedKind`/`_mirrorSeedKind` (getters) and `_setPatternSeedKind`/`_setMirrorSeedKind` (setters, flipping which of the two mutually-exclusive fields carries the id and resetting `merge` to `fuseIntoOne`/`keepSeparate` accordingly) drive it.
- New `_resetMirrorConfiguringState` (mirrors the pre-existing `_resetPatternConfiguringState` exactly) replaces three independently hand-duplicated `setState` blocks in `_openMirrorPanel`/`_openMirrorPanelFromToolFeature` (now removed, merged into `_openMirrorPanelFromFeature`)/`_confirmMirrorBodySelection`.
- New "Pick Direction"/"Pick Axis" `IconButton` (compact, mirroring the existing reverse-direction toggle's own narrow shape to avoid overflow in an already-tight row - not an `OutlinedButton.icon` with a text label) in `pattern_panel.dart`'s `_directionSection`/`_circularFields`, surfacing the existing viewport-tap-to-pick hint text via a `SnackBar` on tap - a genuinely new discoverability gap found during this pass (Circular's own axis section had no interactive affordance whatsoever, by design, since a bare fixed axis can't supply a real pivot point).
- `docs/pattern-mirror-scope.md` gained a new §2.12 (full design writeup) and a "Phase 9" entry under §4 (status: implemented) - the old placeholder "Phase 9+ — Explicitly deferred, not scheduled" section renumbered to "Phase 10+" to stay sequential.

**Triad-hidden-behind-panel fix** (`client/lib/viewport3d/`):
- `triad.dart` gained a new pure, unit-testable `triadCenterFor(camera, viewSize, {required anchorAtOrigin, margin})` function - when `anchorAtOrigin` is false, always the pre-fix fixed bottom-left corner; when true, the world origin's own projected screen position via the existing `worldToScreen` helper (`screen_projection.dart`), clamped to stay `margin`-inset from every edge, falling back to the fixed corner when the origin can't be projected at all (behind the camera).
- `PartViewport` gained a new `anchorTriadAtOrigin` bool param (default `false`, fully backward-compatible), threaded into `_ScenePainter`, which now calls `triadCenterFor` instead of always computing the fixed corner.
- `PartScreen` gained a new `_anyToolPanelOpen` getter (ORs together every tool-panel/picker-ribbon `*Active`/mode flag already used elsewhere in the file: Extrude, Create Plane, Fillet, Chamfer, Revolve, Sweep, Mirror, Pattern, the Profile/Path pickers, and New-Sketch plane-selection mode) wired straight into `PartViewport(anchorTriadAtOrigin: _anyToolPanelOpen)`.
- Reproduced (and confirmed fixed) across two different tools in an automated widget test, not just Pattern/Mirror: the "New Sketch" plane picker (a `PickerRibbon`-backed mode) and `ExtrudePanel` (a full `ResizableToolPanel`) - `PartViewport.anchorTriadAtOrigin` reads `false` with nothing open, `true` while either tool's panel/ribbon is up, and `false` again once each session ends.
- `docs/roadmap.md` gained a new "Other open items" entry documenting the bug, the fix, and its residual on-device-verification gap (real GPU rendering/real panel layouts were never exercised in this sandbox - see that entry for the trailing caveat, following this project's own established convention for client-only fixes verified in a headless test runner).

**Toolchain bootstrap**: no Flutter SDK preinstalled in this sandbox (same starting point every prior client-only session hit). `git clone --depth 1 -b master https://github.com/flutter/flutter.git` (matching `.github/workflows/client-verify.yml`'s own `channel: master` pin - Flutter 3.47.0-0.3.pre resolved), then `flutter pub get` in `client/`.

Verified for real: `flutter analyze` clean on the whole client project; full client `flutter test` suite 1096/1096 (1075 prior + 21 new/rewritten across `part_screen_test.dart`, `pattern_panel_test.dart`, `mirror_panel_test.dart`, `triad_test.dart`, 7 skipped - the same pre-existing, environment-only Impeller-unavailable skip this project's CI has already flagged as sandbox flakiness, not a regression). No backend test run - this session touched no Python files. Not yet confirmed on a real device - no phone attached to this sandbox session, per every other client-only phase's own identical caveat in this file.

## 2026-08-04 — Gear Design Workstreams 1-2: gear math core + `GearFeature` (external/internal spur gears)

New session, `claude/gear-design-tool-vo9d81` branch (continuing a long design-scoping conversation - see `docs/gear-design/` for the full per-workstream scope, split into a `README.md` index, `00-conventions.md` for cross-cutting decisions, and one file per workstream specifically so a future session implementing one workstream doesn't need to read the whole plan).

**Workstream 1 - `backend/app/document/gear_math.py`** (pure Python, zero OCCT dependency, per that split's own established convention - see `app.document.mesh_import`/`app.document.sweep`'s own pure-math-vs-OCCT-construction precedent): involute-of-a-circle sampling and its inverse, resolved spur gear dimensions (base/pitch/addendum/dedendum radii, tooth thickness) from module/tooth-count/pressure-angle/profile-shift/backlash, a full tooth outline and full closed-gear-profile assembler (`full_gear_profile_points`, plus `full_gear_profile_by_tooth` grouping by flank for Workstream 2's own wire construction), an undercut-risk threshold, rack tooth trapezoid geometry, external and external-internal pair centre-distance formulas, and planetary assembly validation (planet tooth count is derived, `N_planet = (N_ring - N_sun) / 2`, not a free input - an invalid combination raises rather than warns, since there's no valid gear to draw). Domain failures raise `GearGeometryError` (a plain `ValueError` subclass, mirroring `app.sketch.models.NoIntersectionFoundError`'s own pattern) - no FastAPI dependency in this module at all.

35 reference-value tests (`backend/tests/test_gear_math.py`), checked against known standard gear dimensions (module-2/20-tooth/20-degree spur gear, rack tooth trapezoid, external/internal pair centre distances, planetary assembly conditions) rather than just "it runs" - this genuinely caught two real bugs before they reached Workstream 2: an algebra error in the planetary orbit-radius derivation (a dropped factor, would have under-detected real planet-to-planet collisions) and a wrong assumed mirror-axis in the test suite's own first draft of the tooth-symmetry check (the code was right, the test's own expectation was backwards) - both found by insisting on hand-verifiable numbers rather than trusting either side blindly.

**Workstream 2 - `GearFeature`** (external + internal spur gears), following the same six-part Feature checklist every other Feature type in this codebase uses:
- `models.py`: new `GearType` enum (Boss/Cut, mirroring `ExtrudeType`) and `GearFeature` dataclass. Unlike `ExtrudeFeature`, a `GearFeature` has no backing `SketchFeature` at all (this whole project's own "gear teeth are not Sketch entities" decision - `Spline`'s real `py-slvs` solver backing makes it unsuitable for hundreds of involute-curve points per gear) - it owns a `plane_ref: PlaneRef` directly instead, defaulting to the fixed XY plane at the router layer when omitted (a real gap found by walking a full user flow end to end during the design/scoping conversation, not scoped up front - see `docs/gear-design/02-gear-feature.md`'s own "Positioning" section for the full reasoning, since it applies to every other gear-producing Feature type too, not just this one).
- `graph.py`: new `_gear_dependencies` (mirrors `_mirror_dependencies`' exact shape - `target_body_ids` plus `_plane_ref_dependency(feature.plane_ref)`), wired into `build_feature_graph`.
- New `app/document/gear.py` (the OCCT-dependent half): each tooth flank becomes one real `Geom_BSplineCurve` edge (`GeomAPI_Interpolate`) rather than a polyline - the only choice that keeps STEP export genuinely smooth, per this whole project's own locked-in decision - connected by straight tip-land and root-gap edges into one closed wire per gear, extruded via the same `BRepPrimAPI_MakePrism` path `ExtrudeFeature` already uses. Internal gears build as one annulus (`BRepBuilderAPI_MakeFace(outer_rim_wire).Add(tooth_wire)`, the identical face-with-holes idiom `app.document.extrude.face_for_profile` already uses for a Sketch profile's own inner loops, including the same winding-direction check via `_wire_normal`). Optional root fillet (`root_fillet_radius > 0`) rounds the correct axial edges via `BRepPrimAPI_MakePrism.Generated()` mapping the original wire's root-corner vertices through the extrusion, then `BRepFilletAPI_MakeFillet` (the same class `app.document.fillet` already uses) - best-effort, falls back to the unfilleted solid with a warning if it doesn't converge, since an unfilleted gear is still valid.
- `extrude.py`: `resolve_feature_tool_shape` gained a `GearFeature` branch (unlike the three existing Sketch-backed branches, `resolve_gear_from_bodies` always raises a structured `HTTPException` rather than returning `None` - no backing Sketch means no "currently has nothing to build" state to tolerate); `compute_part_bodies`'s own topological loop gained a matching branch, reusing the existing `_apply_boss_or_cut` Boss/Cut/register machinery unchanged.
- `schemas.py`/`router.py`: `GearFeatureCreate`/`Update`/`Response`, added to the `FeatureResponse` union; `create_gear_feature`/`update_gear_feature`/`_get_gear_feature_or_404` follow `create_mirror_feature`'s exact validate-then-construct-then-eagerly-resolve-then-persist shape. `_validate_target_body_ids`/`_PATTERN_MIRROR_SOURCE_FEATURE_TYPES` both widened to include `GearFeature`, so a gear Body is immediately targetable by a later Cut/Fillet/Pattern/Mirror exactly like any other Body - confirming in code what the design doc had only asserted "by construction."

**Honest verification status at the time this was first written - no `pythonocc-core` in this sandbox** (this project's own recurring caveat, stated above, applies in full here): `gear_math.py` itself is fully, genuinely tested (35 passing tests). Everything touching real OCCT (`gear.py`, and the `extrude.py`/`router.py` wiring around it) could only be checked via `py_compile`/`ruff` (clean) and, for the OCCT-free parts of the import graph, actually importing them after installing `fastapi`/`pydantic` via pip (`graph.py` imports cleanly end to end, confirming `GearFeature`'s dependency-graph wiring is structurally sound; `pythonocc-core` itself has no pip-installable path, conda-forge only, so `gear.py`/`extrude.py`/`router.py`'s own OCCT-touching code was never actually executed). One real bug was caught and fixed by careful self-review despite this - a wire-vertex-traversal ambiguity in the root-fillet edge-picking logic (`TopExp_Explorer` over a whole wire's vertices doesn't guarantee one entry per unique vertex, the exact reason this codebase's own existing code reaches for `TopTools_IndexedMapOfShape` elsewhere) was rewritten to walk edges instead, which have no such ambiguity.

**Update, same session: bootstrapped a real `pythonocc-core` environment and re-verified for real.** `micro.mamba.pm` was blocked by this session's own egress policy (403, same shape as the 2026-07-31 Phase 8 session's own identical block), but unlike that session's fallback, this session's proxy allowed direct GitHub *release-asset* downloads (`objects.githubusercontent.com`/`github.com/.../releases/download/...` all reachable, only the GitHub *API* JSON endpoints and `conda-forge.org`'s marketing site were blocked) - downloaded `micromamba` directly from its GitHub Releases download URL (not the API), then built the real `didsacad` conda-forge env from `backend/environment.yml` (`conda.anaconda.org` reachable) - `pythonocc-core` and all.

New `backend/tests/test_gear_feature.py` (19 tests, real HTTP requests through `TestClient(app)` against genuine OCCT, mirroring `test_stage_f_revolve.py`'s own shape): external and internal gears actually produce real mesh geometry with sane radii/depth, the fixed-XY-plane default and an explicit custom plane (XZ) both resolve correctly, every `gear_math`-raised validation surfaces as the right structured 422, Boss/Cut and update (PATCH) both work, STEP export produces a real file, a gear Body is genuinely cuttable by a later Feature (the keyway/fixing-holes workflow `00-conventions.md` had only asserted "works by construction" - now actually exercised), and - the one test written specifically to rule out a silent no-op - a filleted gear's mesh is confirmed to genuinely differ from an unfilleted one, not just "didn't crash."

**A real, second bug was found this way**, exactly the kind self-review couldn't catch: `app.document.native_format` (this app's Save/Open native-format serializer) never gained a `GearFeature` branch in either `_feature_to_dict` or `_feature_from_dict` - surfaced first as an incidental failure in `test_stage_native_format.py` (this test suite's process-global Document store carries every Part created earlier in the same pytest session, so that file's own native-export test choked on `GearFeature`-containing Parts `test_gear_feature.py` had already created), then given its own direct, deliberately-isolated regression test (`test_native_export_import_round_trips_a_gear_feature`, following `test_stage_native_format.py`'s own save/restore-global-state convention so it doesn't leak into other test modules either) once the fix (mirroring `MirrorFeature`'s exact export/import shape, including reusing `_plane_ref_to_dict`/`_plane_ref_from_dict`) was in. This was a genuine, real-user-facing gap - Save would have broken for any Part containing a gear - not a test-isolation artifact; a repo-wide grep for every other exhaustive per-Feature-type `isinstance` dispatch (`graph.py`, `extrude.py`, `router.py`) confirmed no further gaps of the same shape.

Verified for real: full backend `pytest` suite 1253/1253 (1234 prior + 19 new) against genuine `pythonocc-core`, including every test above. `gear.py`'s OCCT construction - the real BSpline tooth flanks, the internal-gear face-with-a-hole annulus, and the root-fillet edge-picking via `BRepPrimAPI_MakePrism.Generated()` - all worked correctly on the first real run, no fixes needed there; the one geometry-adjacent bug this pass found was the native-format serialization gap above, not a `gear.py` construction bug.

Remaining gear-design workstreams (rack, helical/herringbone + general Loft, gear chains/compound/planetary, DXF export/import, the client entry screen, presets, bevel) are unstarted - see `docs/gear-design/README.md` for the full delivery order.

---

## 2026-08-04 — Gear Design Workstream 3: `RackFeature` (standalone rack)

Same session, continuing straight on from Workstreams 1-2 above. `docs/gear-design/03-rack.md`: a standalone rack - a straight-sided trapezoidal-tooth profile over a derived length (`tooth_count * tooth_pitch`, not entered directly, same "derived, not entered" treatment `GearChainFeature`'s centre distance already gets), extruded `face_width` deep. Pairing a rack with a pinion stays explicitly out of scope here.

`gear_math.py` additions (pure math, no new module needed - `rack_tooth_geometry`/`rack_tooth_profile_points` already existed from Workstream 1): `rack_length`, `full_rack_profile_points` (assembles `tooth_count` copies of one tooth's profile, evenly spaced and centred on the origin), `default_rack_backing_height` (`2 * module`). 8 new reference-value tests in `test_gear_math.py` (43 total), covering the assembled-length formula, point count, centring, even spacing, and that consecutive teeth genuinely share a flat root land rather than leaving a gap.

Kept as its own Feature type rather than a `GearFeature` variant flag - a rack has no pitch/base/addendum/dedendum radii, no `is_internal` concept, and needs its own `backing_height` field a round gear has no use for; this project's own "each Feature type owns its own fields" convention. Caught and fixed one bug pre-testing, by self-review alone: `RackFeature.backing_height` was first written as a plain `float = 0.0` default, which would silently close the tooth profile into a zero-area (degenerate) rectangle - changed to `float | None = None`, resolved to `default_rack_backing_height(module)` at build time when omitted, with the reasoning spelled out in the field's own docstring so the same mistake isn't repeated on a future re-read.

Full six-part Feature checklist, following `GearFeature`'s exact shape throughout: `RackType` enum (Boss/Cut) + `RackFeature` dataclass in `models.py`; `_rack_dependencies` in `graph.py`; new `app/document/rack.py` (the OCCT-dependent half) - simpler than `gear.py`, since every edge of a rack's profile is a straight line (no involute/BSpline curve fitting needed at all), so this is just `BRepBuilderAPI_MakePolygon` over the assembled outline points, the same primitive `app.document.extrude.wire_for_profile` already uses for a plain Line-chain Sketch profile; `extrude.py`'s `resolve_feature_tool_shape` and `compute_part_bodies` both gained matching `RackFeature` branches (same "always raises a structured `HTTPException`, no backing Sketch to tolerate" shape as Gear); `schemas.py`'s `RackFeatureCreate`/`Update`/`Response`, added to the `FeatureResponse` union; `router.py`'s `create_rack_feature`/`update_rack_feature`/`_get_rack_feature_or_404`, `_validate_target_body_ids`/`_PATTERN_MIRROR_SOURCE_FEATURE_TYPES` both widened to include `RackFeature`.

**Deliberately avoided repeating the Workstream 2 `native_format.py` omission this time**: `RackFeature` branches were added to both `_feature_to_dict` and `_feature_from_dict` as part of the same pass that wired the rest of the checklist, not as an afterthought found via a later failing test - confirmed via `grep -l "GearFeature" app/document/*.py` as a reference checklist of every file that needed a matching `RackFeature` counterpart before considering the workstream done.

Verified for real against genuine `pythonocc-core` (this session's own bootstrapped `didsacad` conda-forge env, rebuilt mid-session after a full container/scratchpad reset wiped the one built during Workstream 2's own verification pass - same build recipe, no new issues). New `backend/tests/test_rack_feature.py` (15 tests, real HTTP requests through `TestClient(app)`, mirroring `test_gear_feature.py`'s own shape): a rack produces real mesh geometry with the expected length/height/depth, the fixed-XY-plane default and an explicit XZ plane both resolve correctly, a custom `backing_height` genuinely changes the mesh, every `gear_math`-raised validation (including the zero-`backing_height` case specifically) surfaces as the right structured `invalid_rack_parameters` 422, Boss/Cut and update (PATCH) both work, STEP export produces a real file, a rack Body is genuinely cuttable by a later Feature, and native export/import round-trips a rack-containing Part with the mesh unchanged (the direct regression guard for the omission called out above). Full backend `pytest` suite: 1276/1276 (1261 prior + 15 new), no regressions.

Remaining gear-design workstreams (helical/herringbone + general Loft, gear chains/compound/planetary, DXF export/import, the client entry screen, presets, bevel) are unstarted - see `docs/gear-design/README.md` for the full delivery order.

---

## 2026-08-04 — Gear Design Workstream 8: entry screen + `/gear/preview` (v1 - external/internal/rack only)

Same session, continuing on `claude/gear-design-tool-vo9d81` from Workstreams 1-3 above. `docs/gear-design/08-entry-screen-and-preview.md` describes a gear-type selector spanning nine types plus a multi-gear/chain/planetary preview extension and `GearGroup` colour-coding - scoped down deliberately here to exactly the two Feature types that exist (`GearFeature`/`RackFeature`; Workstreams 4/5/10/11 are still unstarted, so helical/herringbone/chain/planetary/bevel have no backing Feature to build UI for yet), and single-gear preview only (no chain/planetary/`GearGroup` colour-coding - nothing to colour-code with only one implicit group). `gear_kind` (backend) / `GearDesignKind` (client) is the discriminator naming the split - a future type widens this by one literal value plus one branch, not a new endpoint/screen.

**Backend - `GET`/`POST /document/gear/preview`** (`app/document/router.py`, schemas in `app/document/schemas.py`): runs only `gear_math` (no OCCT, no tessellation, no Part/Feature involved at all) - `_gear_preview_response` dispatches on `gear_kind` (`"external"`/`"internal"` via `spur_gear_geometry`+`full_gear_profile_points`, `"rack"` via `rack_tooth_geometry`+`full_rack_profile_points`), returning the tooth-outline polyline plus pitch/base/addendum/dedendum/outer reference-circle radii (gear) or pitch/addendum/dedendum line y-values + overall length (rack - a rack has lines, not circles). `root_fillet_radius` is deliberately excluded from the request - cosmetic only at OCCT-construction time, `gear_math.tooth_profile_points` never reads it, so it can't affect this endpoint's output. Undercut risk (`minimum_tooth_count_without_undercut`, external gears only) surfaces as a `warnings: list[str]` entry per `00-conventions.md`'s non-blocking-banner convention; a `GearGeometryError` (no valid geometry at all - bad tooth count, missing `outer_diameter` on an internal gear, non-positive `backing_height`) is the doc's own stated blocking exception instead, a 422 with the same `{"type": "invalid_gear_preview_parameters", "detail": ...}` shape `gear.py`/`rack.py`'s own structured errors already use. Both GET (query params) and POST (JSON body) share the same `_gear_preview_response` core - GET exists for a convenient no-JSON manual check, POST is what the client actually calls on every debounced keystroke.

**Client - new `GearDesignScreen`** (`client/lib/gear/`), reached via a new "Gear Design" tile on `ToolChooserScreen`: closer in shape to `SketchScreen` than a `ResizableToolPanel` per the doc's own note - `Scaffold` + `LayoutBuilder` splitting into a `GearPreviewCanvas` (a plain `CustomPainter`, no `flutter_scene`/OCCT dependency at all) alongside a scrollable form, side-by-side above 700px width and stacked below it. `GearDesignKind.external/internal/rack` drives a `SegmentedButton` (mirrors `ExtrudePanel`'s `ExtrudeType` pattern); `StandardValueField` (new, `client/lib/gear/standard_value_field.dart`) is the reusable "dropdown of standard values + custom override" widget `00-conventions.md`'s field input style calls for, used for module and pressure angle. Every field edit reschedules a 500ms debounce (`PartScreen`'s own existing `Timer`-per-field convention, just self-contained in this screen instead of spread through `PartScreen`'s giant state) that calls `DocumentApiClient.previewGear` and repaints the canvas from the response; a structured 422 sets a red blocking banner and disables Create, a `warnings` entry sets an amber non-blocking one that leaves Create enabled - both banners share one `_Banner` widget. "Create" calls `createPart` (this screen is always reached fresh from `ToolChooserScreen`, so there's never an existing Part to reuse) then `createGearFeature`/`createRackFeature`, and hands off via `Navigator.pushReplacement` to the real `PartScreen(initialPartId: ...)` - the ordinary 3D-viewport flow from there on, nothing gear-specific.

**Positioning scoped down from `00-conventions.md`'s full `PlaneRef` picker**: that picker (the one Mirror/Create Plane use) needs an existing Part's Body faces/`CreatePlaneFeature`s to pick from, and this screen is always reached before any Part exists - there's nothing beyond the three fixed reference planes to offer yet. The plane field here is a plain XY/XZ/YZ dropdown instead, still always shown and pre-filled to XY (never silently defaulted) per that doc's actual requirement, just without the Body-face/Plane-feature options a Part-less entry point can't meaningfully serve. Recorded here as a deliberate scoping call, not an oversight.

**`document_api_client.dart` additions**: `GearPreviewDto` (the wire response), `previewGear`/`createGearFeature`/`createRackFeature`. One small, low-risk convention fix alongside these: `_detailOf`'s error-parsing only ever handled a plain-string `detail` field: every gear/rack-feature failure (`gear.py`/`rack.py`'s `_invalid_gear_parameters`/`_gear_failed`/`_rack_failed`/`_invalid_rack_parameters`, now joined by this endpoint's own `_invalid_gear_preview_parameters`) actually sends a structured `{"type": ..., "detail": "..."}` dict - previously this would have surfaced as the raw JSON blob in any error banner. Fixed to unwrap the nested `detail` string when present; safe because no client caller had ever hit this exact shape before (Gear/Rack Features had zero client wiring until this workstream).

Verified for real, not just `flutter analyze`: bootstrapped both a real `pythonocc-core` conda-forge env (`micromamba`, same recipe the Workstream 1-2 session used - `micro.mamba.pm`/conda-forge's own site blocked again, GitHub Releases/`conda.anaconda.org` reachable, same as before) and a real Flutter SDK from scratch (neither existed in this fresh container) - **`channel: master`, not stable**, confirmed necessary firsthand: `flutter analyze`/`flutter test` against downloaded stable 3.44.8 hard-failed on `flutter_scene`/`flutter_gpu` Dart compile errors (`VertexLayout`/`TextureCompressionFamily`/`mipLevelCount` and others missing), exactly the incompatibility `.github/workflows/client-verify.yml`'s own comment already documents from a prior CI run - re-cloning `flutter/flutter` on `master` fixed it immediately, no code changes needed.

Backend: new `backend/tests/test_gear_preview.py` (8 tests, real HTTP requests through `TestClient(app)` against genuine `pythonocc-core`) - known reference-value radii for a module-2/20-tooth external gear (matches `test_gear_math.py`'s own numbers), the undercut warning firing without blocking (6-tooth case) vs. a genuinely-invalid combination blocking with a 422 (3-tooth case), GET and POST agreeing on identical output, internal gear's `outer_diameter` requirement and `outer_radius` echo, and rack's pitch/addendum/dedendum-line-not-circle shape plus its own `backing_height` validation. Full backend `pytest` suite: 1284/1284 (1276 prior + 8 new).

Client: new `client/test/gear_design_screen_test.dart` (5 widget tests, real `flutter_test` `pumpWidget`/`tap`/timer-advance against a faked `MockClient` - no real network) - the debounced preview firing on load, the undercut warning banner leaving Create enabled, a blocking 422 disabling it and showing the real detail text (confirming the `_detailOf` fix above actually works end to end), switching to Internal revealing the required outer-diameter field, and Create genuinely posting `createPart` then `createGearFeature` in sequence. Plus new `DocumentApiClient`/`GearPreviewDto` coverage in `document_api_client_test.dart` (previewGear's request shape for external/internal, the structured-422-detail unwrap, createGearFeature/createRackFeature's request shape). `flutter analyze`: clean, zero issues, whole project. Full client `flutter test`: 1121/1121 passed (7 pre-existing skips, unrelated to this work), no regressions.

**Actually run, not just tested against a fake backend** - the strongest verification this workstream got, and the one that found real bugs the mocked widget tests and `flutter analyze` both missed entirely: `flutter build linux --debug` (after installing `libgtk-3-dev`/`libepoxy-dev`/`libgl1-mesa-dev`, none of which were present in this fresh container either) compiled the real app to a native binary; launched it under a real `Xvfb` display with a genuine `uvicorn` server (the same bootstrapped `pythonocc-core` env) as its backend, drove it with `xdotool` (mouse/keyboard events into the real X server, not a test harness), and captured real framebuffer screenshots via Pillow's `ImageGrab` to actually look at what rendered - installing `xdotool`/`pillow` too, since neither existed here either.

This caught two real, on-device-only bugs, both fixed and re-verified by rebuilding and re-running the same way:
1. **`GearPreviewCanvas`'s `CustomPaint` had no intrinsic size when it had no `child`** - `Expanded` only forces tight *width* inside a `Row`, not height, so the whole preview canvas silently collapsed to zero height the instant a real preview loaded (the "enter valid parameters" placeholder `Text` happened to force a height while `preview == null`, which is exactly why every widget test - which never asserts on rendered *pixels*, only on the widget tree - passed regardless). Invisible in a screenshot until compared against the known-dark `Container` colour actually sampled via Pillow. Fixed with `SizedBox.expand`.
2. **The reference-circle overlay rendered as filled bounding squares, not circles** - `canvas.drawCircle` under this toolchain's specific combination (Impeller's `OpenGLESSDF` software backend, required because `flutter_scene` needs Flutter's *master* channel - `.github/workflows/client-verify.yml`'s own "volatile channel, can break for reasons unrelated to this repo" caveat, confirmed literally true here) didn't rasterize a stroked circle correctly. The tooth outline itself (a `Path` of the same sampled points, drawn through the identical `scale`/`center`/`toCanvas`) rendered perfectly, isolating the fault to `drawCircle` specifically rather than to any of this workstream's own scale/data math. Fixed by rebuilding the overlay from sampled `Path`s (circles as 72-point polylines, rack reference lines as two-point paths) instead of `drawCircle`/`drawLine` - the same primitive already proven correct, sidestepping the toolchain's own drawing-call bug rather than depending on it.

A third finding, in `gear_math.py` itself (Workstream 2, out of this workstream's own scope to fix): an internal gear's addendum points *inward* (`spur_gear_geometry`'s `is_internal` sign flip), and at this screen's own default module(2)/pressure-angle(20°), a 20-tooth internal gear's addendum radius already sits *inside* its base circle - `sample_involute_flank` can't sample that (raises, correctly surfaced here as a blocking 422 per `00-conventions.md`'s own no-valid-geometry exception, not a bug in the error-handling itself). Real, but not this workstream's fix to make - worked around at the UI layer instead: `_onKindChanged` now also bumps the tooth-count field to 40 (confirmed clear of the same fault at 34+ via direct backend calls) the first time Internal is picked, alongside the outer-diameter seed it already did, so the tool's own defaults don't walk a first-time user straight into it. A real fix (e.g. `spur_gear_geometry` itself rejecting this combination as `invalid_gear_parameters` the same way it already rejects a non-positive external dedendum) belongs to whichever future session touches `gear_math.py`/`02-gear-feature.md` next - noted here rather than silently patched over.

Golden path actually exercised end to end against the real stack (real backend, real X server, real screenshots, not a description of what should happen): connect screen -> `ToolChooserScreen`'s new "Gear Design" tile -> external gear preview (tooth outline + all four reference circles, correctly proportioned) -> Create -> real `GearFeature` (confirmed via backend log: `POST .../gear-features` 201) -> `PartScreen` -> **a real, genuinely-extruded 20-tooth 3D gear solid rendered in the actual 3D viewport** (screenshot sent to the user). Internal gear preview (40 teeth after the seeding fix, correct inward-pointing teeth + outer rim circle). Rack preview (correct trapezoidal teeth + pitch/addendum/dedendum reference lines) -> Create -> real `RackFeature` (confirmed via backend log: `POST .../rack-features` 201) -> `PartScreen` (3D viewport not screenshotted for this one - reached before the `FLUTTER_ENGINE_SWITCHES=1`/`FLUTTER_ENGINE_SWITCH_1=enable-flutter-gpu=true` fix for this Linux-binary-launched-outside-`flutter run` environment quirk was in place; the identical code path already confirmed working for the external gear above makes a second screenshot redundant). The Linux desktop 3D viewport needing that engine-switch workaround at all (unrelated to gear design - the same gap this project's own `README.md` already documents for Windows's `runner/main.cpp`) is itself a real, if pre-existing and out-of-scope, finding worth a future session's attention.

`flutter build`/`flutter test`/`flutter analyze` all re-run after both fixes (see the numbers above) - no regressions from either change.

---

## 2026-08-04 — Gear Design: fixed a real internal-gear tooth-taper bug (dovetail), added pitch-diameter/length display, field help tooltips, pinch-zoom/pan

Same session, continuing straight on from the Workstream 8 entry above. On-device follow-up: the user tried an internal gear on a real device and reported the teeth "look like they should be on an external gear... rotated 180 degrees... like a dove tail joint" - narrower at the root (far from centre) than the tip (close to centre), backwards from a normal tooth's taper.

**First response was wrong, corrected by insisting on ground truth rather than trusting a screenshot glance.** Initial pixel-measurement of the reported screenshot (a naive colour-run scan across a wide x-window) mis-measured which end was wide and concluded the shape was correct - directly contradicted once challenged and re-checked three independent ways: (1) the raw `tooth_profile_points` output (root half-width 0.51mm vs. tip half-width 2.02mm at module 2/40 teeth - tip wider, confirmed backwards), (2) a real OCCT solid built via `resolve_gear` and queried directly with `BRepClass3d_SolidClassifier` (a binary search for the material/hole boundary angle at increasing radius - not just the wire, the actual solid), and (3) the real tessellated mesh through the full router/HTTP path. All three agreed: genuinely backwards, a real bug in `gear_math.py` (Workstream 2, already merged - predates this workstream, first surfaced by this session's own on-device testing since no prior test had ever asserted tooth width *direction*, only that geometry existed at all).

**Root cause**: `spur_gear_geometry`'s `is_internal` sign flip correctly repositions *which* radius is addendum vs. dedendum (tip inward/root outward for internal, the reverse of external) but doesn't correct for a real, unavoidable property of *any* flank sampled via `tooth_profile_points`'s existing construction (external or internal alike): angular tooth width is a strictly *decreasing* function of radius, always, for radius >= base_radius (`involute_function`'s own monotonicity, not a choice this module makes). For external (tip beyond pitch, root below it) that law already gives the correct shape - wide root, narrow tip, matching every verified external render. For internal (tip *below* pitch, root *above* it - the sign flip's whole point) the same law then gives *narrow* root and *wide* tip: the exact dovetail reported.

**Fix** (`gear_math.py`, `_internal_tooth_profile_points`, new): build the flank as if generating an *external* gear with addendum/dedendum radially mirrored through the pitch radius (`_mirror_radius_through_pitch`, `r -> 2*pitch - r`) - reuses `tooth_profile_points`'s already-correct external construction completely unchanged (same base circle, tooth thickness, pressure angle), then mirrors every resulting point's own radius back through the pitch radius (angle untouched). Radially mirroring inverts which end of the flank sits closer to/further from the base circle, which inverts the direction of the same universal narrowing law - confirmed via the identical three-method check that found the bug: `tooth_profile_points` now gives root>tip width, the real OCCT solid's own material/hole boundary width now increases monotonically with radius (1.42mm at the tip up to 4.46mm near the root, module 2/40 teeth), and the rendered 3D solid (screenshot sent to the user) shows a normal, correctly-tapered ring gear.

New tests: `test_gear_math.py` - symmetry still holds for `is_internal=True` (the mirror construction only rescales radius, can't break left/right symmetry) and a direct monotonic-taper assertion against the pure-math profile points (confirmed this fails on the pre-fix code, root_half_width 0.51 < tip_half_width 2.02, before passing on the fix). `test_gear_feature.py` - the same taper assertion against the real tessellated mesh through the full router/OCCT path (module 2/60 teeth: filtering to one tooth's own angular slice, not just "x > 0" as the first draft of this test wrongly did, catching a real bug in the *test* itself before it could give a false pass on the still-buggy code - confirmed failing pre-fix, passing post-fix). Full backend suite: 1287/1287, no regressions. A prior attempt at an additional "meshing constraint" test (internal tooth width should equal a same-spec external gear's width at the mirrored radius) was dropped rather than shipped once it revealed the fix's angle-preserving mirror doesn't satisfy that exact identity - the fix's *primary*, directly-verified claim (correct taper direction, confirmed against real OCCT geometry) stands on its own; getting the fully rigorous meshing-accuracy formula right (AGMA-style, likely a sign-flipped involute-function term evaluated directly at each true radius rather than via a geometric mirror) is a real, separate question worth a future session's closer look before this tool starts validating real meshing pairs (`GearChainFeature`), not something to overclaim here.

**Three unrelated, explicitly-requested additions to `GearDesignScreen`/`GearPreviewCanvas`**, same session:
- **Pitch diameter / rack length shown directly on the preview** - a small overlay in the canvas's own top-left corner, `IgnorePointer`-wrapped so it doesn't steal the new pinch/pan gestures below. Gear kinds show `Pitch diameter: {2 * pitch_radius} mm`; rack shows `Length: {rack_length} mm` (a rack has no pitch circle at all).
- **A tappable "?" next to every numeric field**, showing what it means (module, pressure angle, tooth count, face width, profile shift, root fillet radius, backlash, outer diameter, backing height, plane). First attempt (a bare `Icon` wrapped in `Tooltip(triggerMode: tap)`, per that API's own stated purpose) silently failed on-device - tapping it just opened the surrounding `DropdownButtonFormField`'s menu or focused the surrounding `TextField` instead, since the field's own tap-to-open/tap-to-focus gesture won the arena every time. Fixed with `FieldHelpIcon` (new, `field_help_icon.dart`): a real `IconButton` (the same kind of independently-hit-tested interactive widget a decorated field's own "clear text" suffix button already is) whose `onPressed` calls `ensureTooltipVisible()` on a `Tooltip(triggerMode: manual)` - confirmed on-device this actually pops the bubble, on both a dropdown field (Module) and a plain one (Tooth count), with neither side effect the first attempt had.
- **Pinch-zoom and two-finger pan on the preview** - wrapped the canvas in `InteractiveViewer` (the stock Flutter widget for exactly this gesture pair, also picks up trackpad pinch/scroll on desktop) rather than hand-rolling gesture tracking. Real multi-touch pinch isn't exercisable through this sandbox's X11/mouse-only input, but panning (the same underlying gesture recognizer family) was confirmed working on-device via a real click-drag, which moved the rendered gear by exactly the drag delta.

New tests: `gear_preview_canvas_test.dart` (pitch-diameter/length label per gear kind, absent with no preview yet, `InteractiveViewer` present) and one more in `gear_design_screen_test.dart` - tapping the Module field's help icon shows its tooltip text and leaves the dropdown's own option list closed (confirmed this fails against the first, `Tooltip(triggerMode: tap)`-only attempt, passes against the `FieldHelpIcon` fix). `flutter analyze`: clean. Full client `flutter test`: 1126/1126 passed (7 pre-existing skips, unrelated), no regressions.

All of the above re-verified on-device (real backend, real Xvfb X server, real screenshots) the same way as the Workstream 8 entry above - screenshots of the corrected internal gear (3D solid) and all three UI additions sent to the user.

---

## 2026-08-04 — Workstream 5 Spike 1: `GearChainFeature` bent-path positioning + interference checking (investigate/prototype only)

New session. Per `05-gear-chain-and-planetary.md`'s own "plan for two separate spikes before committing to the full build" note, this session ran the first of those two spikes - `GearChainFeature`'s own bent-path positioning and interference-check design (Spike 2, compound-station geometry, is separate and not attempted here). Investigate/prototype scope explicitly: no schema/router/OCCT work, a throwaway pure-Python script only, findings written back into `05-gear-chain-and-planetary.md` itself (new "Spike 1 findings" section appended at the end).

**Bent-path positioning**: confirmed the turtle-graphics resolution rule works cleanly - segment 0's direction is the chain-level `start_direction` field; segment `k` (`k>=1`)'s direction is segment `k-1`'s direction plus `stage[k].turn_angle`, reusing `gear_math._rotate`'s existing CCW-positive convention directly (already the same right-hand-rule sign `RevolveFeature.angle`/OCCT's `gp_Ax1` use, so nothing new invented). Hand-verified against a 5-stage module-2 chain (external ×4 then internal, one 90° turn then a -30° turn back): every centre-distance (`35, 25, 35, 35`) and every segment direction (`0°, 0°, 90°, 60°`) reproduced exactly by the prototype and matched independent hand/calculator arithmetic, including the final stage's centre `(77.5, 65.310889)` = `60+35·cos(60°), 35+35·sin(60°)`.

Real gap found: `stage[N-1]`'s (the last stage's) `turn_angle` field is geometrically inert - it would steer segment `N-1`, which doesn't exist. Not a blocker, but worth Workstream 8 greying out/omitting that one field on the last stage row rather than accepting a value that silently does nothing. Also flagged, not resolved here: a rack stage's position needs an orientation (its length axis) as well as a reference point, and the doc doesn't yet specify how that derives from the connecting segment's turtle direction - the worked example above deliberately used four gears, no rack, to keep the turtle math itself unambiguous; real remaining design work before the full build, smaller than either headline problem.

**Interference checking**: confirmed the doc's own "topology split, not a tolerance value" design is cleanly implementable as one shared signed-gap convention (negative = overlap, `[0, margin)` = near-miss, `>= margin` = clear) across all three bounding-shape pairs that can occur (external addendum circle / internal outer-rim circle / rack oriented-bounding-rectangle): `circle_circle_gap` (exact), `circle_rect_gap` (exact box signed-distance function), `rect_rect_gap` (Separating Axis Theorem over 4 axes - overlap detection exact per SAT's iff property for convex polygons; near-miss magnitude a conservative lower bound for a corner-to-corner closest-approach case, which is the safe direction for a clearance-margin check and doesn't compromise the zero-tolerance overlap half). Hand-verified one instance of each pair (e.g. circle-circle centres 44.1mm apart with two addendum-22 circles → `gap=0.1`, inside the 0.2mm margin; rect-rect racks 4.5mm apart → `gap=0.0` exactly, touching) plus the full 5-stage worked example above, which - not built to be collision-free - correctly surfaced genuine overlaps at non-adjacent pairs `(1,3)`, `(1,4)`, `(2,4)` while skipping every consecutive pair, a useful side-confirmation that the checker actually catches what a naive sharp-turn chain produces.

Data structures/function signatures for the real implementation (`gear_chain_math.py`, mirroring `gear_math.py`'s OCCT-free shape) and the complete worked-example numbers are written up in `05-gear-chain-and-planetary.md`'s new "Spike 1 findings" section, not repeated here. Conclusion: both halves of Spike 1 are implementable in pure Python with no OCCT dependency, unit-testable the same way `test_gear_math.py` already is - clear to proceed to Spike 2 (compound-station geometry) before committing the full Workstream 5 build.

---

## 2026-08-04 — Gear Design Workstream 10 spike: spherical involute math + `BRepOffsetAPI_ThruSections` go/no-go (investigate/prototype only, not the full workstream)

New session, `claude/gear-design-tool-vo9d81` branch, continuing straight from Workstream 9's own work above. Requested explicitly as a spike, not an implementation: `10-bevel-gear.md` is flagged as this project's single highest-risk workstream (a bevel tooth flank has no flat 2D profile at all - a genuinely 3D spherical involute curve on a cone, unlike every other gear type here), so before `11-bevel-pair.md` or `06-dxf-export.md`'s bevel flat-pattern question commit to depending on it, this session answered two concrete questions: does the spherical involute math check out against anything independently verifiable, and does `BRepOffsetAPI_ThruSections` (already scoped for Workstream 4's Loft) actually hold up for lofting a tooth flank between two spherical-involute space curves, self-intersection risk on a tight cone included. Full findings appended to `10-bevel-gear.md` itself (its own "Spike findings" section) - this entry is the short version.

**Math** (`app/document/bevel_math.py`, real, committed, OCCT-free per the established split): pitch cone half-angles (general shaft angle plus the doc's own `Sigma=90deg` reduction), base cone half-angle (`sin(gamma_b) = sin(gamma)*cos(alpha)`, derived here via Napier's rules for right spherical triangles), the spherical involute curve itself, and full bevel cone-geometry resolution (addendum/dedendum/face/root cone angles, cone distance, a non-blocking `max_recommended_face_width = cone_distance/3`). 87 tests, `backend/tests/test_bevel_math.py`.

**A real bug was caught by this test suite, exactly the scenario the doc's own test requirement worried about.** The first working draft of the spherical involute curve had its generating great circle's tangent direction backwards (wrong handedness). It passed every self-consistency check tried first - lies exactly on the unit sphere, colatitude grows monotonically, and even a full independent re-derivation via Napier's rules matched it to machine precision - because none of those checks actually exercise curve handedness. What caught it: reducing the same general "involute of a curve on a surface" construction to the flat-plane case (where geodesics become straight lines) and comparing bit-for-bit against `gear_math.involute_point`'s own already-shipped planar formula - the wrong-signed version diverged immediately, the corrected sign matched to 13 significant figures. Independently reconfirmed with a from-scratch rigid-body rolling simulation sharing no code with the closed form. Both cross-checks are now permanent regression tests. Lesson written up in `10-bevel-gear.md` for whoever extends this module next: for a genuinely new curve construction with no in-codebase precedent, checks need to be independent *in derivation method*, not just numerous - a from-scratch second derivation of the same formula (this session's own first attempt at "independent" verification) can still share the same underlying bug and agree with itself perfectly.

**OCCT construction spike: GO on `BRepOffsetAPI_ThruSections`.** Bootstrapped a real conda-forge `pythonocc-core` 7.9.3 env (micromamba from GitHub Releases, `backend/environment.yml` - this project's own established recipe, `micro.mamba.pm` blocked as usual, GitHub Releases/`conda.anaconda.org` reachable). Sampled outer/inner spherical-involute flank curves, fit real `Geom_BSplineCurve`s, and built `ThruSections` between them across a battery of cases from a moderate 90-degree 20T/40T pair down to a 6T/80T pair at a 4.3-degree pitch cone angle (tighter than any real bevel pinion would use, and past `gear_math`'s own undercut-avoidance threshold anyway) - valid, correctly-shaped, non-self-intersecting surfaces in every realistic case, confirmed three ways (`BRepCheck_Analyzer`, a self-section, and a real fine-grid parametric-injectivity fold detector - the first two both passed on a case later shown to genuinely fold, which is why the grid check was necessary at all). Cross-checked against a hand-built `GeomFill_BSplineCurves` ruled surface in every case - identical results, confirming that as a working fallback if `ThruSections` is ever rejected for unrelated reasons.

**The one genuine fold found was precisely characterized, not just observed**: not tight cone angle alone, not low tooth count alone, but face width pushed toward the standard `cone_distance/3` rule-of-thumb bound specifically on a tight cone - probed the exact threshold (table in `10-bevel-gear.md`) and found the moderate 20T/40T pair never folds even at the full bound, the 6T/80T pair folds starting at 95% of the bound. Mechanism identified: a large face width shrinks the inner curve's sphere radius a lot relative to the outer curve's while both sweep the same angular range, and on a tight cone the ruled surface's straight "rungs" between corresponding points start crossing. Concrete implication for a future `BevelGearFeature`: the existing face-width warning-banner threshold is fine as a general rule of thumb but isn't sufficient on its own to guarantee valid geometry on a tight pitch cone - a real implementation should run an actual fold check at construction time for tight-cone gears, not rely on the ratio alone.

Not attempted, correctly out of scope for a spike: stitching N teeth into a closed shell/solid gear body (real BRep kernel work, its own future spike). Full backend suite re-run against this same bootstrapped env: 1374/1374, no regressions. `ruff check` clean on both new files.

---

## 2026-08-04 — Gear Design Workstream 4 spike: helical/herringbone technique + general Loft wire-reordering (investigate/prototype only, no shipped code)

New session, `claude/gear-design-tool-vo9d81`, pulled latest (Workstreams 1/2/3/8 above already merged). Scoped explicitly as investigate/prototype - not a build pass. Resolves the two open questions `docs/gear-design/04-helical-herringbone-loft.md` flagged as needing a real spike before Workstream 4 gets built for real; full detail (numbers, exact test setups) lives in that file's own new "Spike findings (2026-08-04)" section, not duplicated here.

**Toolchain bootstrap**: same recipe every prior session's own entry above used - `micro.mamba.pm`/the GitHub API blocked, but micromamba's own GitHub Releases download-asset URL and `conda.anaconda.org` both reachable. Built the real `didsacad` conda-forge env from `backend/environment.yml` (`pythonocc-core=7.9.3=novtk*`) and ran every test below against genuine OCCT construction, not reasoning about the API.

**Headline result, and the one worth flagging loudest**: `04-helical-herringbone-loft.md`'s own premise for 4b ("achieving [twist control] means reordering each profile's own wire edge-traversal start... and matching winding direction across profiles") turned out to be **wrong for this codebase's actual OCCT build**. Direct testing - cyclically re-indexing a wire's own starting vertex (no coordinate rotation at all), separately flipping winding direction (built-reversed and via `TopoDS_Wire.Reversed()`), across a real 20-tooth gear profile, a hand-checkable asymmetric hexagon, and a genuinely dissimilar square-to-hexagon general-loft case - found `BRepOffsetAPI_ThruSections`' vertex correspondence completely unaffected in every single case, confirmed not a broken measurement (real coordinate rotation *does* move the same measurements every time, and the wire's own topological "first vertex" genuinely does change under re-indexing when checked directly via `TopExp_Explorer`). Practical upshot for helical teeth: twist between two copies of the same tooth profile just needs the top copy's real coordinates pre-rotated before its wire is built - no reordering step, no winding correction, simpler than the doc assumed. Left genuinely open (not resolved this pass): whether the general `LoftFeature`'s own "user picks a reference point on each of 2+ *dissimilar* profiles" UI needs some other OCCT mechanism (`AddVertex`/`ParType` flagged as candidates) since reordering doesn't drive correspondence there either.

**Self-intersection stress test** (the doc's other explicit ask - "stress-test self-intersection risk... between two rotated copies of the same profile, confirm... not just worked for one easy case"): the real 20-tooth module-2 gear profile survived twist 5°-1080° (past 360° the test's own periodicity makes higher numbers uninformative, noted honestly rather than claimed as evidence) with zero self-intersection by every check tried, including a direct mid-height-section area/simplicity check built specifically because the two OCCT checkers this codebase would otherwise reach for both turned out weak: `BOPAlgo_CheckerSI` failed to flag an obviously self-intersecting bowtie-profile prism at all, and neither checker caught anything wrong anywhere in the sweep. A deliberately much deeper 6-point star (40:1 tip:root ratio, far beyond any real gear tooth's ~1.1-1.3:1) also came back clean up to 340° twist. Self-intersection **is** real and detectable for other shapes, proving the check methodology isn't just reporting "fine" no matter what - a thin far-off-axis "blade" profile genuinely broke (up to 5.5x expected cross-sectional area, one fully degenerate 0-area case) starting around 60-110° twist. Conclusion: self-intersection risk for actual gear-tooth-shaped profiles is low well past any twist a real helical gear would use.

**Sweep-along-helix vs Loft, the doc's actual decision to confirm/overturn**: building a real helix path curve in OCCT (`Geom2d_Line` on a `Geom_CylindricalSurface`, `BRepLib.BuildCurves3d`) worked correctly first try - not the hard part. Getting `BRepOffsetAPI_MakePipeShell` to sweep the tooth profile along it *without distorting the cross-section* turned out to be the real cost: the default trihedron mode visibly tilts the profile to track the helix's own tangent (10% aspect-ratio distortion on a test circle - wrong, since a real helical tooth's cross-section must stay perpendicular to the gear axis, only rotating, never tilting); the most promising alternative mode found (`SetMode` with a fixed binormal along the gear axis) passed a symmetric circular-profile check but still showed a genuine ~14% cross-sectional-area distortion once re-tested with an asymmetric profile that can actually reveal incorrect rotation - not solved by picking one of the four built-in trihedron modes, would need a custom rotation law. Compared directly against the loft's own known approximation error over the same twist range (1-7%, smaller and better-understood).

**Overturned the doc's original recommendation** (sweep-as-primary, Loft-as-fallback) based on the above: build the general `LoftFeature` (4b) as planned regardless (independently useful), and use loft-between-two-rotated-copies as the *primary* helical-tooth technique, not a fallback - self-intersection risk is low for real tooth profiles at realistic helix angles, twist control is simpler than assumed (no reordering needed), and the "more correct" sweep alternative needs real unresolved additional spike time to actually beat the loft's own accuracy - the exact "too costly" condition the doc's own fallback language anticipated, now confirmed by prototyping instead of assumed going in. `SweepFeature`'s path-model extension (procedural helix paths, not just picked Sketch-entity chains) deferred/dropped for this use case as a result.

No shipped code this pass, per the task's own explicit scope ("investigate/prototype only - do not build the full Workstream 4") - prototype scripts were scratch-only (conda-env-local, not committed). Findings written up in `docs/gear-design/04-helical-herringbone-loft.md`'s own new section (implementation sketch for the rotated-copy twist technique included there) plus this entry. Workstream 4 itself (`LoftFeature`, `GearFeature.helix_angle`/herringbone wiring) remains unstarted - this session only resolved the two open technical-risk questions blocking it.

---

## 2026-08-04 — Gear Design Workstream 5 Spike 2: compound-gear join geometry (investigate/prototype only)

New session, `claude/gear-design-tool-vo9d81`, pulled latest (Workstream 5 Spike 1, and the Workstream 10/4 spikes above, all already on the branch). Scoped explicitly as investigate/prototype only, per `05-gear-chain-and-planetary.md`'s own "plan for two separate spikes" note - this is the second and final one, resolving the "Compound gears" section's own two unspiked unknowns (structural transition between two coaxial diameters; DXF export not reducing to one profile) plus confirming `MergeMode`'s existing plumbing actually covers the coaxial case. Full findings, exact numbers, and the resolution written into `05-gear-chain-and-planetary.md`'s own new "Spike 2 findings" section (appended after Spike 1's); this entry is the short version.

**Toolchain bootstrap**: same established recipe every prior spike above used - `micro.mamba.pm` 403'd through this sandbox's own proxy, but micromamba's GitHub Releases download-asset URL and `conda.anaconda.org` both reachable. Built the real `didsacad` conda-forge env from `backend/environment.yml` (`pythonocc-core=7.9.3=novtk*`) and ran every check below against genuine OCCT construction. Prototype script (throwaway, not committed) built solids via `app.document.gear`'s own real internals (`_gear_outline_wire`/`_gear_face`/`_apply_root_fillet`/`spur_gear_geometry`) with a hand-built `ResolvedPlane` offset along world Z standing in for a compound member's own axial-stacking-offset position - a simplified stand-in for `resolve_gear_from_bodies`, since a full `Part`/`compute_part_bodies` apparatus isn't needed to test the join itself.

**Structural transition: minimum-thickness alone is not sufficient - the real risk is silent disconnection, not degeneracy.** External+external members never degenerate at any tested module ratio (up to 16x, module 0.5<->8) - always backed by a continuous solid disc to the shaft centre (`_gear_face`'s own wire-alone construction, no bore), fused volume equal to the unfused sum to the mm^3 in every case, `BRepCheck_Analyzer`/`Fuse.IsDone()` both clean throughout. But three real, silent failure modes were found, none flagged by either of those checks: (1) a 0.1mm axial gap between two otherwise-touching members produces 2 disconnected solids, not 1 fused body; (2) an internal ring member (module 2/40 teeth, outer rim 45mm, inward addendum reach 38mm) paired with an external member whose own addendum radius doesn't reach that 38mm bore produces a floating, disconnected pinion inside the ring (confirmed radius-dependent: 5mm/24mm addendum both disconnected, 46mm addendum connects) - a connectivity problem specific to internal-member compounds the original "step/overhang" framing didn't anticipate at all; (3) a 1-3mm axial overlap stays a single valid solid but loses real volume (321mm^3/963mm^3 short of the unfused sum) as the larger member's footprint silently swallows the smaller member's tooth geometry. Resolution: a post-join connected-solid count (reusing `app.document.extrude._explode_solids`, already called by every `_register_solids` site - no new topology code needed) catches cases 1 and 2 as a **blocking** error when `merge=FUSE_INTO_ONE` produces >1 solid (there's nothing valid to register as "the" fused body); case 3 (volume-loss, still one valid solid) fits the ordinary **non-blocking** warning convention instead. A literal minimum-thickness check is still worth keeping, but as an unrelated, simpler check on one member's own `face_width` in isolation.

**Optional join fillet: mechanism generalizes, convergence doesn't - not worth building now.** Locating the join edges directly on the fused shape (edges flat in the member-A/member-B transition plane) works cleanly, after an initial dead end (tracking member A's original top face through the fuse's own `Modified()`/`IsDeleted()` history breaks down when member B's cross-section is larger - the exposed shoulder is a face newly carved from member B's own bottom face, not a survival of member A's). But `BRepFilletAPI_MakeFillet` convergence around that whole tooth-profile loop is narrow and non-monotonic with module difference: a modest diff (module 1->2) converges 0.05-0.5mm, fails at 0.8mm+; a large diff (module 1->5) converges 0.05-0.8mm (a *wider* window, not narrower, than the modest case), fails only at 1.0mm. Usable but unpredictable, and even where it converges the radii are too small relative to a real multi-millimetre step to matter structurally. Not worth building as v1 scope, matching root fillet's own much cleaner precedent (`docs/gear-design` Workstream 2 history above - "worked correctly on the first real run").

**DXF two-files resolution confirmed, simpler than originally framed.** No DXF export code exists in this codebase yet (Workstream 6, confirmed unstarted by direct search) - this is a scoping conclusion, not a code test. `06-dxf-export.md`'s own existing scope already writes a gear's DXF straight from its stored generative parameters, never by slicing the 3D solid - so whichever `MergeMode` a compound station used makes literally no difference to DXF export; "two per-member files" is the same per-member path an ordinary N-stage chain already needs, just 2 files for one stage instead of 1. Two concrete refinements flagged for `06-dxf-export.md`'s own future scope (not implemented here, Workstream 6): per-gear file naming needs a compound-aware sub-index (member A/B), and combined-layout export needs a deliberate choice for two coaxial members sharing the same `(x, y)` centre (they'd otherwise land exactly on top of each other in a flat top-down drawing).

**`MergeMode` confirmed sufficient for the coaxial case, no new field needed.** Ran a real `TopoDS_Compound` of two touching coaxial solids through `app.document.extrude`'s own `_explode_solids`/`_register_solids` (Pattern/Mirror's own `KEEP_SEPARATE` machinery) directly, not just reasoned about it: correctly split into 2 independent Bodies (`#0`/`#1`), no special-casing needed for "touching" vs. Pattern/Mirror's own original spatially-disjoint-copies case. `FUSE_INTO_ONE` on a well-formed stack registers 1 Body correctly; on a 0.5mm-gap mistake it silently registers 2 (`#0`/`#1`) despite `Fuse.IsDone()`/`BRepCheck.IsValid()` both staying `True` - the exact gap the connected-solid-count check above closes, confirmed through the real `MergeMode` plumbing rather than in the abstract.

**Both of Workstream 5's spikes are done. Workstream 5 is clear to move to a full build** - remaining items (Spike 1's last-stage inert-field UI handling and rack-orientation convention; this spike's connected-solid-count check and the two `06-dxf-export.md` refinements) are small, scoped implementation decisions, not further open technical risk.

---

## 2026-08-05 — Gear Design Workstream 10 spike: bevel shell/solid assembly (investigate/prototype only, not the full workstream)

New session, `claude/bevel-gear-shell-solid-nhai2s` branch (rebased onto `claude/gear-design-tool-vo9d81`'s own tip to pick up `bevel_math.py`/the 2026-08-04 bevel spike below, since the designated branch had none of that history yet). Picks up exactly where that spike explicitly stopped: single-tooth flank construction was proven (`ThruSections` GO, per that entry above), but "stitching N teeth into a closed shell, capping with addendum/dedendum cone surfaces, and producing a valid solid gear body" was named as its own future spike. This session is that spike. Full findings in `10-bevel-gear.md`'s own new "Spike findings (2026-08-05)" section (following its 2026-08-04 section); short version here.

**Toolchain bootstrap**: same recipe every prior session's own entry above used - `micro.mamba.pm` blocked, micromamba's own GitHub Releases download-asset URL and `conda.anaconda.org` both reachable. Built the real `didsacad` conda-forge env from `backend/environment.yml` (`pythonocc-core=7.9.3=novtk*`). Full pre-existing backend suite re-run against it before starting: 1374/1374, no regressions (nothing in this session touches committed code - scratch scripts only, matching this project's own spike convention).

**Go/no-go: GO on full shell/solid assembly**, via `BRepBuilderAPI_Sewing` across `4N + 2` faces (2 flank + 1 tip-land + 1 root-land per tooth, plus 2 end-caps) followed by `ShapeFix_Shell` (on the whole sewn shell, not per-face) then `MakeSolid` then `BRepLib.OrientClosedSolid`. Validated for both the prior spike's moderate 20T/40T (module 4) pair and its tight 18T/90T (module 2.5, 11.3deg pitch angle) pair, at multiple face-width ratios each. Tip-land/root-land faces reuse the *exact same* `ThruSections`-between-two-wires technique flanks already use (arcs instead of splines - both are ruled surfaces of lines through the apex, so no separate cone-primitive construction was needed). Undercut handling needed zero new code: both test cases land in mild undercut territory and the existing `bevel_tooth_flank_pair`'s own `max(root_cone_angle, base_cone_angle)` clamp already determines where the root-land face belongs.

**The one genuinely new technique, and two real dead ends**: the 2 end-caps (spherical, bounding the whole `4N`-edge zigzag tooth-outline rim on each sphere, mirroring `gear.py`'s own planar whole-outline-as-one-face pattern) needed hand-built pcurves - `BRepBuilderAPI_MakeFace(Geom_SphericalSurface, wire)` given only 3D edges (no pcurves) silently returns a *wrong* face (`IsDone() == True`, but `BRepCheck_Analyzer` invalid and area exactly 0.0 - the API's own docstring warns of this, easy to miss). `BRepOffsetAPI_MakeFilling` (sidesteps pcurves by fitting its own surface) is a second, measured dead end for this shape specifically: it reports done *and* `BRepCheck_Analyzer`-valid, but the resulting area is ~3.2x a naive upper-bound estimate - wrong, and both weaker checks missed it. What worked: closed-form (azimuth, latitude) pcurves via each 3D point's own `atan2`/`asin` (matches `Geom_SphericalSurface`'s default parametrization exactly), a hand-rolled azimuth-unwrap (matching `numpy.unwrap`'s own algorithm, no numpy dependency) to keep the pcurve's U parameter monotonic all the way around N teeth, and - the second real gotcha - `BRepLib.BuildCurves3d(wire)` called *before* `MakeFace`, not after: edges built from a pcurve+surface alone carry no 3D curve at all until that call.

**`BRepCheck_Analyzer` is wrong again, in a new way - continuing this project's own recurring lesson.** The final solid still reports `IsValid() == False` (`BRepCheck_UnorientableShape` on the two end-caps specifically, whose wires graze the same latitude circle once per tooth - genuinely ambiguous to OCCT's own per-face orientation heuristic even though the shell-level fix already applied the correct orientation). Confirmed correct three independent ways instead: an independent divergence-theorem volume computed over a real `BRepMesh_IncrementalMesh` tessellation agreed with `BRepGProp`'s own analytic volume to 0.08-0.47% across every case tested (mesh-noise-level, not a red flag); `BOPAlgo_CheckerSI` clean on the final solid in every case; `BRepTopAdaptor_FClass2d` correctly classifying a point at a tooth's own centre as inside. Same lesson as the 2026-08-04 spike's own flank-validity finding, this time for a false-negative about *orientation* rather than a missed self-intersection.

**Face-width fold risk re-tested at assembly scale, per this session's own scoping question**: re-derived (not literally reused - the prior spike's own script no longer exists, scratch-only by that spike's own convention) a grid-based fold detector and validated it against a positive control (pushing face width to the physical limit, confirming it correctly fires there). At the *previously-documented* threshold ratios (6T/80T at 0.95, 18T/90T at 1.00) this session's own re-implementation found no fold by either signal - a genuine, honestly-flagged discrepancy this session could not reconcile without the original script to compare against. What **is** solidly answered regardless: at every ratio tested, the full-ring assembly and the single isolated flank track each other's pass/fail status exactly, including at the ratio where the flank's own grid check first signals near-coincidence but the assembled solid's own volume/`BOPAlgo_CheckerSI` checks do not yet catch it - a second confirmation that those whole-solid checks are weaker than the grid-based one, not just pre-assembly but post-assembly too. No evidence that stitching N teeth into a ring shifts the safe boundary or adds a new failure mode; the per-flank check remains the right, sufficient gate, run once before assembly.

Implementation sketch (new `app/document/bevel.py` helpers, face inventory, validation split, undercut passthrough) written up in `10-bevel-gear.md`'s own new section, not repeated here. No shipped code this pass, per the task's own explicit scope - `bevel.py` itself remains unstarted; this session only resolved the shell/solid construction risk blocking it, plus flagged one open item (the fold-threshold discrepancy above) for whoever builds the real feature to re-check against committed code.

---

## 2026-08-05 — Gear Design Workstream 4: helical/herringbone teeth + general `LoftFeature` (full build)

New session, `claude/gear-design-tool-vo9d81` branch, continuing straight from the 2026-08-04 Workstream 4 spike above (whose two open questions this session deliberately did not re-derive - see `04-helical-herringbone-loft.md`'s own "Spike findings" section). Real build this time, not a spike: `LoftFeature` (4b) and `GearFeature.helix_angle_degrees`/`herringbone` (4a), following the same Feature-tree six-part checklist `00-conventions.md` describes and using `RackFeature` (Workstream 3) as the file-by-file checklist - every file `grep -l "RackFeature" backend/app/document/*.py` names (`models.py`, `graph.py`, `extrude.py`, `schemas.py`, `router.py`, `native_format.py`) got a matching `LoftFeature` entry, plus a new `app/document/loft.py` module mirroring `rack.py`'s own role.

**Toolchain bootstrap**: same established recipe every prior session above used - `micro.mamba.pm` blocked, micromamba's own GitHub Releases download-asset URL and `conda.anaconda.org` both reachable. Built the real `didsacad` conda-forge env from `backend/environment.yml` (`pythonocc-core=7.9.3=novtk*`). Full pre-existing backend suite re-run against it before starting any new code: 1374/1374 (matching the prior 2026-08-05 bevel spike's own baseline count above), confirming the toolchain and establishing a clean baseline.

**4a - helical/herringbone teeth, built directly in `app/document/gear.py`, not through `LoftFeature`.** Reuses `gear_math.full_gear_profile_by_tooth`/`_gear_outline_wire`/`_gear_face` completely unchanged - the only new piece is `_twisted_basis`, which shifts a `ResolvedPlane`'s origin along its own normal and rotates its `x_axis`/`y_axis` about that same normal by the twist angle, then hands that *rotated basis* (not rotated points) to the existing wire-building code. This is mathematically identical to the spike's own confirmed "pre-rotate the profile's real (x, y) coordinates" technique (`basis_point_to_world`'s embedding is linear, and rotation commutes with a linear map applied in the same plane) while reusing the flank/tip-land/root-gap wire construction verbatim rather than duplicating it. `helical_twist_angle` (new, `gear_math.py`, OCCT-free, unit-tested) derives the twist angle from the standard `face_width * tan(helix_angle) / pitch_radius` relation. `helix_angle_degrees=0.0`/`herringbone=False` (the defaults) take the exact original `BRepPrimAPI_MakePrism` code path, unchanged - confirmed byte-identical output against a fresh Part in `test_helical_herringbone_gear.py`, not just "the field defaults to 0". Herringbone builds two separate half-face-width `ThruSections` lofts (bottom-to-midplane at `+half_twist`, midplane-to-top at `-half_twist`) and fuses them, rather than one 3-section loft through a non-monotonic twist sequence - a deliberate lower-risk choice not directly covered by the spike's own 2-section-only testing (a 3-section loft with an interior twist "peak" is genuinely untested territory this session chose not to gamble on). Internal helical/herringbone gears build the twisted tooth-boundary as its own solid and boolean-cut it out of a plain (untwisted) outer-rim prism, rather than attempting a face-with-a-hole through `ThruSections` (which builds from wire sections, not faces). Root fillet is not supported for a helical/herringbone tooth (no `ThruSections` equivalent to `BRepPrimAPI_MakePrism.Generated()`'s vertex-tracking) - tolerated with a logged warning, same best-effort convention an unconverged straight-gear fillet already falls back to.

**4b - general `LoftFeature`, in new `app/document/loft.py`.** Lofts 2+ Sketch-Profile `sections` (each its own `sketch_feature_id`/`profile_refs`, possibly a different Sketch - mirrors `SweepFeature.path_refs`' own cross-Sketch allowance) via `BRepOffsetAPI_ThruSections(isSolid=True)`. Holes (`profile.inner_loops`) are rejected outright for v1 (`invalid_loft_section`) rather than silently only lofting the outer boundary - the same per-hole correspondence problem as the headline question below, not solved here either. **Resolved the one thing the spike explicitly left open** ("if the general Loft's own reference-point-per-profile UI is meant to align genuinely *different* profiles, plain wire reordering won't drive that either"): `LoftSection.reference_point` (an optional Sketch Point, restricted to this section's own Sketch) triggers an *explicit pre-alignment transform* - if both the first section and this section name a reference point, this section's whole wire is rotated (via `BRepBuilderAPI_Transform` about the section's own local origin/normal - the world-space equivalent of rotating its real (x, y) coordinates) so its own reference point's local angle matches the first section's. This is exactly one of the two candidates the spike itself flagged (an explicit pre-alignment transform, as opposed to `AddVertex`/`ParType`), confirmed for real against a genuinely dissimilar-orientation case (two identical-shape, differently-labelled rectangles - a rectangle was needed, not a square, since a square's 90-degree symmetry made an earlier test draft's own rotation a silent no-op on the resulting geometry).

**Self-intersection validation, per the spike's own Result 2 finding** ("`IsDone()`/`BRepCheck_Analyzer`/`BOPAlgo_CheckerSI` are all weak signals - build a real geometric check"): `LoftFeature` gets its own best-effort `_mid_section_warnings` (non-blocking, surfaced on `LoftFeatureResponse.warnings`) - cuts the lofted solid at the representative plane between the first/last sections' own origins, reassembles the loose section edges into a wire, and flags a warning if that reassembly fails, doesn't bound a simple face, or the resulting face is degenerate (near-zero area relative to the solid's own bounding-box scale) - the two concrete failure modes the spike's own "blade" profile actually exhibited. Deliberately does *not* attempt an "expected area" comparison the way the spike's own gear-specific check could (two rotated copies of the *same* profile share a known, rotation-invariant area at any twist) - a general Loft between two *different* profiles has no such reference value, so this is an honestly partial check (would miss a section that stays a single simple loop but is grossly inflated), documented as such rather than overclaiming full coverage.

**Backward compatibility and native round-trip**, following Workstream 3's own explicit "use the existing type as a checklist" discipline (its own status entry describes the exact bug this caught for Workstream 2 - `native_format.py`'s export/import branches silently missing a new Feature type entirely): `test_native_export_import_round_trips_helix_angle_and_herringbone` and a companion legacy-dict-with-no-`helix_angle_degrees`-key import test guard `GearFeature`'s two new fields specifically; `test_native_export_import_round_trips_a_loft_feature` covers the new Feature type. `LoftFeature` also threaded through every other place a Body-producing Feature type has to appear: `resolve_feature_tool_shape`/`compute_part_bodies`'s topological loop (`extrude.py`), `_validate_target_body_ids`/`_PATTERN_MIRROR_SOURCE_FEATURE_TYPES` (so a Loft Body is a valid Boss/Cut target and a valid Mirror/Pattern source), and `graph.tool_feature_qualifies` (so a Loft is a valid Mirror/Pattern `tool_feature_id`).

**Real-OCCT test results**: two new test files (`test_helical_herringbone_gear.py`, `test_loft_feature.py`) plus additions to `test_gear_math.py` - real HTTP integration tests via `TestClient(app)`, mirroring `test_rack_feature.py`/`test_gear_feature.py`'s own shape (known-volume checks for a straight-prism Loft, not just "it runs"; angular-position-of-a-specific-tooth-tip checks, precomputed from `helical_twist_angle` rather than blind max-radius search, for the helical/herringbone twist). Two real, on-device-against-genuine-OCCT test-design bugs found and fixed, not just "wrote tests and they happened to pass": the first helical-twist test draft's own "max-radius vertex at height z" measurement was ambiguous (every one of a gear's teeth shares the identical addendum radius, so it could silently match a *neighbouring* tooth once its own twist landed close to the angular tooth pitch) - fixed by checking radius at a specific, `helical_twist_angle`-predicted angle instead of a blind search, with test parameters chosen to keep that prediction safely clear of any neighbouring tooth. The first Loft reference-point-alignment test draft used a square profile rotated 45 degrees for its "genuinely different orientation" case - silently a no-op, since a square only has 90-degree rotational symmetry and the picked reference angles happened to cancel out the alignment rotation entirely - fixed by switching to a 10x4 rectangle (only 180-degree symmetric) with reference points on two different corners. Full backend suite: 1374 pre-existing + 30 new = 1404 passing, zero regressions (re-confirmed with a final full re-run after every fix below landed).

`docs/gear-design/README.md`'s workstream table updated (Workstream 4 marked done, same style as Workstream 8's own "v1 done" annotation). Client-side UI for helical/herringbone/Loft entry remains out of scope here, same as every other still-unstarted workstream's own UI (Workstream 8's own scoped-down v1 note already defers it).
