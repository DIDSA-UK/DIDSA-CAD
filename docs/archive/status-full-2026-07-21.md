# DIDSA-CAD Status (Consolidated) — full-detail archive, superseded 2026-07-21

Archived verbatim on 2026-07-21 when `docs/status.md` was condensed (~60% smaller) to stay under Claude's own file-read size limits. This full version has every entry's complete reasoning/investigation narrative; `docs/status.md` has the terse current version - same entries, same order, tightened prose. Update `docs/status.md` going forward, not this file.
...
Two more real bugs, each found by reading the actual failure rather than assuming the previous fix was enough:

- The `AlertDialog`-scoping fix (above) worked for 2 of 3 remaining cascade-delete tests, but exposed one more real ambiguity in the third: after making the preview call succeed, the same closing-sheet/opening-dialog overlap could also hit a *second*, distinct ambiguity once the dialog and a stray still-present "Delete" landed in the same frame differently across tests - fixed with the same `AlertDialog`-scoped tap everywhere it was still using a plain text search, plus a genuine timing gap: `_cascadeDeleteFeature`'s un-hide bookkeeping can land a frame after the deleted Feature's row disappears from the tree, so the visibility-icon assertion needed its own extra settle pump.
- The "pre-selected Sketch" test's `find.byTooltip('Add')` (inside the shared `tapAddFeatureExtrude` helper) had the exact same unreliable-tooltip-position issue as `Exit Sketch` - switched to `find.widgetWithIcon`. That surfaced one more layer: the "Add" FAB carries `heroTag: 'add-fab'`, so while the pop transition's Hero flight is still in progress, a temporary in-flight copy of the FAB can coexist with the destination route's own static one - a fixed 300ms settle pump wasn't reliable against that. Fixed by waiting for the flight to actually finish (exactly one `Icons.add`-bearing FAB left) instead of guessing a duration.

**Final confirmed result: 534 passed, 1 failed** - and that one is `part_viewport_test.dart`'s already-diagnosed CI-sandbox GPU-init flakiness (the exact same "Flutter GPU requires the Impeller rendering backend" error reproduced again, not a hang, not new). All 26 originally-flagged pre-existing failures are resolved for real, confirmed by CI itself rather than assumed - this took nine total CI round-trips across `part_screen_test.dart`'s 14, `sketch_controller_test.dart`'s 4, `orbit_camera_test.dart`'s 3, and the five single-file failures, catching (and then re-catching) mistakes in this session's own fixes along the way rather than declaring victory early.
...
Everything learned above about writing/fixing Flutter widget tests correctly - `find.byTooltip`'s tap unreliability, `pumpAndSettle()` vs. permanently-running Tickers, the `_pumpUntil` pattern, proxy-signal waits, unscoped finders, Hero-flight duplicates, fake-backend endpoint coverage, and telling a stale test apart from a real bug - is distilled into `docs/flutter-widget-test-lessons.md`, matching this project's existing convention of splitting reusable how-to knowledge (see `docs/live-preview-pattern.md`) out of these dated narrative entries. Read that file first before touching any widget test in this repo.
...
Every status entry above about CI progress checked the test-run step's own pass/fail count, but never the `Analyze` step's own conclusion - which had actually been failing (`flutter analyze` exits 1 on any issue, including `info`-level ones) on every single run in this branch, including the ones already reported as "534 passed, 1 failed." The real cause: 3 pre-existing `avoid_print` lint findings in `part_viewport.dart`'s `[RenderDebug]` diagnostic logging (deliberately `print`, not `debugPrint`, per that code's own comment - see the widget-test-lessons/live-preview-pattern precedent of trusting a comment's stated intent). **Fixed**: added `// ignore: avoid_print` above each of the 3 calls rather than removing them, since the code's own reasoning for using `print` there is still valid. Confirmed via the real next CI run: `Analyze` step now `success`, and the only remaining failure is the exact same, already-diagnosed `part_viewport_test.dart` GPU-init flakiness (identical error text, nothing new).

**Branch merged to `main` via PR #94** once this was confirmed - closing out the full lighting/shading-upgrade branch (PBR lighting/shading rollout, mesh viewer decimation/materials/Up-axis/mirroring fixes, native Save/Load and STEP/STL/OBJ/glTF export/import, Revolve/Sweep features, the new Flutter CI workflow, and all 26 pre-existing test failures it surfaced) into the main line.

## 2026-07-14 — Polygon promoted from a client-only shortcut to a real, persisted entity (`claude/sketcher-roadmap-tuning-7z3shf`)

Resumed the sketcher-tuning package's last deferred item: reinterpreting a Polygon vertex drag as an edit to its circumradius, instead of an unconstrained 2D point move. Investigating first confirmed the constraint graph alone couldn't reliably identify "this Point is a Polygon vertex" - `EqualRadiusConstraint` has no discriminating field between Polygon's own raw-point creation path and Arc/Slot's entity-based path, making any heuristic fragile. Given three options (heuristic matching, a new discriminator field on the constraint, or a real persisted entity), the user picked the most thorough: **a real Polygon entity**, matching how Arc/Ellipse/Slot are already modeled.

**Backend** (`app/sketch/models.py`): new `Polygon(SketchEntity)` dataclass (center Point, `sides` vertex Points, edge Lines, plus its own `radius_constraint_id`/`equal_radius_constraint_ids`/`equal_length_constraint_ids`/`angle_constraint_ids`) and `Sketch.add_polygon`/`polygons()`/`delete_polygon`, mirroring the Arc/Ellipse five-endpoint router shape (`schemas.py`, `router.py`), `native_format.py` round-trip support, and `store.py`'s entity-ref table. `add_polygon` creates everything atomically in one call - the whole regular-polygon constraint chain (one real circumradius `DistanceConstraint`, `sides-1` `EqualRadiusConstraint`s, `sides-1` pairs of `EqualLengthConstraint`+`AngleConstraint`) - replacing the old client-orchestrated multi-call sequence. 16 new backend tests (`test_stage_polygon_entity.py`); full suite still at the established 9 pre-existing, unrelated baseline failures.

**Client**: `SketchApiClient` gained `PolygonDto` + list/create/update/delete methods; `SketchController` replaced the old session-only `PlacedPolygon` bookkeeping with a real `SketchPolygonView` map loaded from the API, `_clickPolygonTool` now makes one `createPolygon` call instead of orchestrating points/lines/constraints individually, and delete/undo gained full Polygon support (capture-before-delete, dependency-ordered restore, matching Arc/Ellipse/Circle's own pattern). `beginPointDrag`/`updatePointDrag`/`endPointDrag` now detect a Polygon vertex and, once its circumradius `DistanceConstraint` is confirmed, PATCH that constraint's value to `distance(center, cursor)` instead of the vertex's raw position - reusing the existing throttled mid-drag-solve infrastructure (Phase 3's incremental-resolve work) so the equal-length/equal-radius/angle chain doesn't get hammered on every pointer-move tick.

Verified: `flutter analyze` clean, `flutter test` at the established 442/-19 sandbox baseline (19 pre-existing `flutter_scene`/`flutter_gpu` sandbox-only incompatibilities, unrelated). Four commits pushed (`56bc412` backend entity, `63717a0` client entity wiring, `9ffeefd` drag reinterpretation, plus tests each step).
...
The user reported, from real on-device use: adding an "across flats" dimension to a Polygon over-constrained it, with nothing visibly explaining why - "is there something hiding in the background?" Investigation (a background research pass, then direct root-causing) confirmed it: the drag-as-circumradius-edit fix above called `updateConstraintValue` on the circumradius `DistanceConstraint` on *every* drag tick, and `update_constraint_value` unconditionally clears that constraint's `provisional` flag as a documented side effect - so an ordinary "nudge this to look right" drag silently confirmed a real, DOF-removing dimension the user never explicitly set. Adding a second, explicit dimension (the across-flats one) on top of that then genuinely over-constrained the sketch, and the now-confirmed circumradius constraint wasn't rendering as a readable dimension to explain why - `isRadiusDistanceConstraint` only recognized Circle/Arc, so it fell through to a generic two-point linear dimension between the center Point and vertex 0 (an internal line with no drawn edge), easy to miss as clutter.

**Fixed** (`d5ee1b8`): a vertex drag is now only reinterpreted as a dimension edit once the circumradius constraint is *already* confirmed - while still provisional (the common case for a Polygon nobody has explicitly dimensioned), it already removes zero DOF, so an ordinary raw point drag already resizes the whole shape correctly via the equal-radius chain, with nothing to confirm. Also taught `isRadiusDistanceConstraint`/added `polygonForDistanceConstraint` to recognize a Polygon's own circumradius constraint, so once confirmed it renders as a proper radial leader (matching Circle/Arc) instead of the misleading generic dimension.

Same on-device report also flagged that a Polygon's auto-created angle ties (a "45.0°" label at every vertex, visible the instant the shape is placed) and equal-length glyphs are implicit structure, not user dimensions, and should only surface if the shape is ever broken. **Fixed** (`42a789d`): new `SketchController.isImplicitPolygonEdgeTie(line1Id, line2Id)` - true when both Lines are edges of the same still-existing Polygon (identified by Line membership, mirroring how `circleForDistanceConstraint`/`arcForDistanceConstraint` already identify "mine" without the API exposing raw constraint ids) - wired into both the paint path and hit-testing so these ties are neither rendered nor tap-selectable while the Polygon is intact. Deliberately forward-looking: `delete_polygon` currently cascades every Line/Constraint away together, so "hidden" and "shape broken" can't diverge yet, but a future trim/extend tool that removes just one edge without deleting the whole Polygon would make them reappear as real information.

Also asked to add a trim/extend tool to scope (not implement it yet). Researched the actual gap: no line-line/line-circle/line-arc intersection-point math exists anywhere in this codebase today (the client's own `_lineIntersectionScreen` in `sketch_canvas.dart` is private, screen-space, line-line-only, and exists only to lay out the angle-ghost arc), profile detection's connectivity graph is by shared Point id and doesn't help find geometric crossings, and naively moving a shared endpoint via the existing `update_point` PATCH would silently drag every other entity anchored to that Point (a chain corner, a Polygon vertex). Written up as **Phase 11** in `docs/sketcher-overhaul-scope.md` (`6944fd1`) - a fresh-Point-on-trim design to avoid that, Line/Circle/Arc-only scope for v1 (Ellipse/Spline need curve-specific root-finding), and the open question of how trimming one Polygon edge should interact with the entity's own bookkeeping.

Verified: `flutter analyze` clean throughout; `flutter test` at 447/-19 (established baseline, 5 new tests added across the two fix commits). All changes pushed to `claude/sketcher-roadmap-tuning-7z3shf`.

## 2026-07-14 — Removed the broken 3D backdrop, added New Sketch on Face, reworked the sketch-start camera sequence

Four more on-device-feedback requests, same branch. First, a status audit (asked directly): confirmed Spline/Text tools, Phase 7.1's 2D/3D pan-zoom sync, and Phase 4.3 v2 (external edge refs) are all fully shipped despite `sketcher-overhaul-scope.md` still describing them as open in places - genuinely open work is Phase 5's reference-axis alignment (pick-an-edge-as-Y-axis was never built, only discrete flip/rotate) plus Phases 8/9/10/11 (DXF import, Convert/Translate, thin-profile extrude, trim/extend - all scope-only, zero implementation).

**Removed the shaded-body backdrop behind the flat 2D sketch canvas** (`d543b55`) - on-device feedback confirmed it never worked right, and root-caused why: its camera was necessarily perspective (`flutter_scene` has no orthographic camera at all), synced to the 2D canvas's own pan/zoom at only one target depth - anything off that plane showed real perspective foreshortening a flat orthographic canvas can never reproduce. Removed outright: `SketchScreen._hasBodyBackdrop`/`_backdropViewportKey`/`_syncBackdropCamera` and Canvas Transparency (whose sole purpose was revealing it) are gone; the sketch's own `closedProfileFills` already shades the enclosed area (untouched, unconditional), and Orbit View remains the only place real Body geometry shows, unaffected since its own camera and geometry are both genuinely 3D.

**Added "New Sketch on Face"** (`febca0e`) - selecting a single Body face now offers this alongside the existing "Create Plane": one tap creates a zero-offset `CreatePlaneFeature` flush against the face and immediately starts the same orientation-confirm flow a plane-based new sketch already gets, instead of requiring create-plane-then-separately-start-sketch as two steps.

**New sketch-start camera sequence** (`d854346`) - requested: New Sketch (any method) → animate to isometric for orientation definition → confirm → animate to the chosen orientation → sketcher. Previously the camera cut straight to the plane's own face-on view before orientation-confirm even appeared (custom planes got no animation at all - there was no `orientationFacingPlane` equivalent for an arbitrary basis). New `OrbitCamera.isometricOrientation()` (true CAD isometric angle, `asin(1/sqrt(3))` ≈ 35.264°, plane-independent) now plays first for every new sketch, fixed or custom-plane alike; confirming now animates to the actual chosen plane/flip/rotation before opening the sketcher, so the sequence ends exactly where the sketcher opens rather than cutting there. Scoped to new-sketch only - redefining an existing sketch's orientation keeps its own existing camera behavior.

Verification note: none of `orbit_camera.dart`/`part_viewport.dart`/`part_screen.dart`/`selection_actions.dart`/`sketch_screen.dart` or their test files can be executed in this sandbox - they transitively import `flutter_scene`, which doesn't compile against this sandbox's Flutter SDK (the same pre-existing, documented limitation `flutter analyze`/CI history above already established). Every change here is `flutter analyze`-clean and manually reviewed (plus a hand-verified isometric-angle unit test for the one pure-math piece), but real on-device or CI confirmation is still outstanding for all four items.

## 2026-07-14 — Phase 11 implemented: trim/extend a Line

Same branch, same day - after scoping Phase 11 (above) and a round of feature-rollout suggestions, the user asked to build it. Resolved the scope doc's three open design questions via documented code comments rather than re-asking: multiple crossing candidates resolve to whichever is nearest the dragged endpoint's current position (one endpoint, one nearest-wins rule, no separate trim/extend mode flag needed); reach is capped at 10000 sketch units (mirrors the client's own `_maxAngleIntersectionDistance` precedent); and a Polygon's own edges are rejected as trim targets outright for v1 - demoting a Polygon to loose geometry is real scope, deliberately deferred rather than half-built.

**Backend**: new `app/sketch/intersections.py` - plain-tuple `line_vs_segment`/`line_vs_circle`/`line_vs_arc` geometry, ported from the client's own private `_lineIntersectionScreen` algebra for the line-line case, standard quadratics for circle/arc (Ellipse/Spline explicitly out of scope, no closed-form intersection). `Sketch.trim_or_extend_line` (`models.py`) scans every other Line/Circle/Arc, picks the nearest valid crossing, and moves the dragged endpoint - in place if it's provably unshared, otherwise via a fresh Point (reusing `_point_deletion_blocker`, generalized with an `exclude_entity_id` kwarg, for the "is this shared" check rather than duplicating its entity-traversal logic). New `POST /sketches/{id}/lines/{id}/trim`; a distinct `NoIntersectionFoundError` gets its own 422 rather than folding into the generic 400. 20 new tests (`test_stage_trim_extend.py`); full suite still at the established 9 pre-existing, unrelated failures.

**Client**: `SketchApiClient.trimLine` + `LineTrimResultDto`; a new `SketchMode.trim` (FAB → Trim/Extend, new `action_trim.svg`) whose tap handler hit-tests only Lines (never Points/Circles/Arcs - those are intersection targets, not picks here), reinterprets the tap as "which end is closer" the same way every mainstream CAD trim/extend gesture does, and stays active across repeated picks rather than exiting after one. Undo of an in-place move is a plain position revert; undo of a shared-endpoint trim (no API exists to repoint a Line's endpoint id directly) deletes the trimmed Line and its new Point and recreates the original Line fresh - the same fresh-id-on-recreation convention `_restoreDeletedEntities` already uses elsewhere in this class. 7 new controller tests against a small real (not canned) line-line intersection faked into the test backend.

Verified: backend 20/20 new tests passing, full suite unchanged at 9 pre-existing failures; client `flutter analyze` clean, `flutter test` at 454/-19 (7 new tests, same established 19 pre-existing `flutter_scene`/`flutter_gpu` sandbox-only failures). Two commits pushed (`4ac4ed9` backend, `e6e9b00` client).

## 2026-07-14 — On-device round: seven fixes surfaced by real use of the trim/extend + New-Sketch-on-Face rollouts

Same branch, same day - a batch of on-device screenshots/reports covering the trim/extend and New-Sketch-on-Face/backdrop-removal work above. Each was root-caused individually rather than patched blind; six were fixed and pushed in this round (`be02b91`, `687d5c4`, `dbd1899`, `174858b`, `c010faa`), one (angle-not-offered) needed a follow-up clarifying question before its real cause was found.

**"New Sketch" missing when selecting a Plane** (`be02b91`) - New Sketch on Face's own `contextActionsFor` addition only ever offered "New Sketch" for a Body *face*; a lone reference/created Plane selected via the persistent Selection-mode context panel still only got bare "Create Plane" (the sibling tap-to-sheet flow already had it, just unreachable from that mode). Mirrored the face case.

**Orbit View gone for a custom-plane sketch** (`be02b91`) - traced precisely: Orbit View's camera/rendering was hard-wired to a fixed `ReferencePlaneKind` from Phase 4.1/4.2 onward, a dormant gap until New Sketch on Face made reaching a custom (Feature-anchored) plane sketch a one-tap path - combined with the backdrop's removal, such a sketch had *no* way to see body context at all. Generalized the whole path to a `SketchPlaneBasis`: new `orientationFacingBasis`/`PartViewport.initialViewBasis`/`PartViewportState.animateToBasis` work for any basis, not just a fixed plane; `PartScreen` now threads the sketch's own fully-oriented basis into `SketchScreen` for both cases uniformly. Also fixed a related pre-existing bug (`_sketchPlaneBasisFor`) where a custom-plane sketch's own flip/rotation was silently dropped on re-open, affecting both Orbit View and the main 3D viewport's own rendering of it.

**Polygon still breaks when dragging an edge, not just a vertex** (`687d5c4`) - `beginLineDrag`'s ordinary rigid-body translation (both endpoints by the same delta) can't generally keep two Polygon vertices on the same circumradius circle the way a single-vertex drag's reinterpretation already can - a straight-line translation of a chord almost never equals a pure rotation about the center. Redirected: dragging a Polygon's own edge now redirects to `beginPointDrag` on its start vertex, reusing the already-correct scaling gesture. `updateGrabbedPosition`/`dropGrabbedEntity` already dispatched generically on whichever of `draggingPointId`/`draggingLineId` ends up set, so no other call site needed to change.

**Over-constrained / stale-fully-constrained until re-entering the sketch** (`dbd1899`) - root cause: `update_constraint_value` re-solves server-side (its response is a fresh `SolveResultResponse`), but three client call sites that PATCH an *already-existing* constraint's value - `setLineLength`, `updateSelectedConstraintValue`, `confirmGhostValue`'s point-based branch - only called `_solveAndTrackDof()` in their sibling "create a brand new constraint" branch, never on re-confirm. The client's own cached dof/converged state (everything `isFullyConstrained`/`isUnderConstrained`/the green-red coloring reads) kept showing whatever the *previous* solve reported until some unrelated mutation, or a full re-adopt via closing/reopening the sketch, forced a fresh one. All three now solve unconditionally, matching `confirmGhostValue`'s angle/lineDistance branches and `endPointDrag`'s polygon-radius branch, which already did.

**Angle dimension not offered between two Lines** (`174858b`) - initially suspected ghost-specific (a materialized body-edge Line), but the user confirmed it failed for two *ordinary* Lines too, and traced their own instinct ("aren't connected by a common point?") to the real cause: `_lineIntersectionScreen`'s rejection threshold was a large *absolute* screen-pixel constant, guarding only against near-parallel numeric blow-up - two Lines positioned far apart with only a shallow angle between them could produce a virtual intersection technically under that constant yet many times farther than the Lines themselves, landing the whole arc outside the visible canvas. The angle ghost's layout now falls back to the existing straight-line-to-midpoint layout whenever the intersection is unreasonably far from either Line's own midpoint, scaled to the two midpoints' own on-screen distance - degrades gracefully regardless of zoom or how the Lines happen to be positioned. (The underlying ghost-building logic itself was already correct, confirmed by a pre-existing passing test - this was purely a rendering-layout bug.)

**Dimension-mode picks weren't highlighted** (`174858b`) - the canvas's `isSelected()` check only ever read `SketchMode.select`'s own `selectionSet`; an entity picked into `dimensionSelection` while in Dimension mode rendered with no highlight at all, only the dimension bar's chip list showed what was picked. Now checks both (never populated simultaneously, so safe).

**Ghost/projected body outline never shaded** (`c010faa`) - "shade the area enclosed by the lines projected onto the canvas" was the original ask when the backdrop was removed, but only the Sketch's own drawn profile (`closedProfileFills`) ever got wired up - the projected ghost wireframe stayed unfilled. `referenceGhostSegments` carries no id/topology (a flat, unordered bag of coordinate pairs from every visible Body's mesh edges merged together), unlike the backend-resolved profile loops `closedProfileFills` already has - new `closedGhostLoops` (public specifically for direct testing) recovers real closed loops itself: snap-merge nearby endpoints into shared nodes, keep only edges between two degree-2 nodes (excludes T-junctions/open chains without needing to know which edges belong to which loop up front), walk each remaining component back to its start. v1 scope, documented as such: no nested-hole punch-out, and a stray edge touching an otherwise-clean loop excludes it rather than guessing. 8 new unit tests directly exercising the loop-recovery algorithm (square, triangle, open chain, float-wobbled corners, two disjoint loops, a T-junction, a zero-length segment).

Verified throughout: `flutter analyze` clean at every step; `flutter test` climbed from the established 454/-19 baseline (trim/extend's own end state) to 467/-19 across this round's 5 commits - 13 new tests (2 for the polygon edge-drag redirect, 3 for the dof-staleness fix, 8 for `closedGhostLoops`; the New-Sketch/Orbit-View fix and the angle-ghost/dimension-highlight fix touch only code this sandbox's `flutter_scene` limitation or Dart's own privacy rules keep untestable here - both `flutter analyze`-clean and manually reviewed instead) - same 19 pre-existing `flutter_scene`/`flutter_gpu` sandbox-only failures throughout, no regressions at any point.

## 2026-07-14 — Two more on-device findings from the same round

Same branch, same day - two more issues from continued on-device use.

**Materialized Body-edge references were solid geometry, not construction** (`be6d74c`) - `create_external_edge_reference`'s own `sketch.add_line(start_point.id, end_point.id)` never passed `construction=True`, so a projected Body edge picked into a Sketch rendered/behaved as real drawn geometry (eligible for profile/extrude detection) instead of a reference to dimension against. One-line fix plus a new backend assertion and a matching client fake-backend/test update.

**"Face the plane" camera animations never recentred, only reoriented** (`d2988f2`) - the second animation in the new-sketch camera sequence didn't land looking straight at the sketch plane; the same misalignment happened leaving Orbit View via "Return to Default View". Root cause: `_animateOrientationTo` (shared by `animateToPlane`/`animateToBasis`) only ever slerped `OrbitCamera.orientation`, never touched `OrbitCamera.target` - if the user had panned at all before either animation ran, the camera resolved to the mathematically correct facing direction while staying centred on wherever they'd last panned to. Both methods now also animate `target` back to the plane's own `basis.origin` (zero for the three fixed reference planes, a custom Plane's real offset otherwise) alongside the orientation slerp. `animateToIsometric` is untouched - there's no single "correct" center yet during orientation-definition itself.

Verified: backend change confirmed against a real conda pythonocc-core environment (3 passing, the same 2 pre-existing OCCT-unrelated failures this file already had); client change is `flutter analyze`-clean with a passing regression test. The camera fix touches `part_viewport.dart`, which - like every `flutter_scene`-importing file in this project - can't be loaded by `flutter test` in this sandbox; verified by `flutter analyze` and manual review only, real on-device confirmation still outstanding.

## 2026-07-14 — Slot "fully constrained too early" root-caused and fixed

Same branch, same day. Two more on-device reports about the Slot tool: (1) a freshly-drawn Slot showed the whole-sketch padlock as fully constrained with only a Horizontal constraint on its centerline, before its radius had ever been signed; (2) a screenshot of an extrude built from a Slot sketch showing a wrong body - two full circles joined by a twisted saddle surface instead of a proper stadium prism.

Investigated both together since they looked likely to share a cause. First correction to a prior assumption in this file: pythonocc-core **is** actually importable in this sandbox's conda env (`OCC.Core.*` imports fine, and `pytest` here already runs the full real-OCCT suite - the persistent "needs a real environment, not available here" caveat on OCCT-touching tests earlier in this file was stale/overcautious, not currently true). This let both reports be reproduced and root-caused directly against the real backend rather than by static reading alone.

**Root cause of #2 (wrong extrude shape) was a bug in my own reproduction script, not the product** - an early repro rebuilding `_clickSlotTool`'s sequence via list-index lookups (`client.get(...).json()[-2]`) instead of capturing each Line's id from its own creation response grabbed the *construction centerline* instead of the intended perimeter Line, wiring a `TangentConstraint` between an Arc and a line that passes through its own center - genuinely unsatisfiable except by collapsing the radius to 0, which is exactly the collapsed geometry observed. Once the repro was fixed to capture ids the same way the real client does, extrude produced a correct stadium body (verified via a real mesh: y-extent exactly ±radius, x-extent exactly length ± radius) every time, including after adding a Horizontal constraint. No product bug found here - flagging in case the same "wrong extrude" shape resurfaces on-device, since it would point at *how* the Slot's Lines/Arcs got connected, not at the solver.

**Root cause of #1 (fully constrained too early) was real** and reproduces from Slot creation alone, before any Horizontal constraint is even added. `solve_sketch`'s existing `REDUNDANT_OKAY` override (the one documented for a Slot's own mathematically-redundant Tangent+EqualRadius web, `solver.py`) trusts `system.Dof` even though py-slvs's naive param-count-minus-equation-count is exactly the thing that override exists to route around: for this specific redundant system it consistently reports `dof: 0` regardless of whether the shape's radius has ever been confirmed - a real, unconfirmed degree of freedom the count simply doesn't see, confirmed directly against a real solve of the exact `_clickSlotTool` construction (dof stayed 0 both right after creation and after adding Horizontal, while the Point positions themselves stayed correct throughout - this is a reporting bug, not a geometry bug).

**Fixed** (`solver.py`): whenever `solve_sketch` converges and any `DistanceConstraint` in the sketch is still `provisional` (unconfirmed), `dof` is floored to `max(system.Dof, 1)` rather than trusted as-is. Harmless everywhere else - a plain Arc/Circle/Polygon/Ellipse's own provisional constraint already reports a correct nonzero `dof` from a clean (non-redundant) solve, so the floor is a no-op there; it only changes anything for the specific redundant-and-still-unconfirmed case this bug report was about.

2 new tests in `test_bugfix_provisional_size_constraints.py` (`_build_slot`, mirroring `_clickSlotTool` exactly): one confirms `dof > 0` both immediately after Slot creation and after adding Horizontal, plus that every rim Point stays exactly `radius` from its own Arc's centre (guards against the collapse-shaped bug turning out to be real after all); one confirms `dof <= 0` once the radius and centerline length are both explicitly confirmed. Full backend suite: 799 passed (was 797 - 2 new), same 9 pre-existing unrelated failures, no regressions.

## 2026-07-15 — Session close-out: wrote a standalone sketcher architecture/UX-rethink scoping document

Requested directly: the user is still unhappy with sketcher UX - specifically how entities/shapes resolve when moving things around - and asked for a separate document (not this file, not `sketcher-overhaul-scope.md`) covering every tool's full functionality, every design decision and its rationale, and a menu of options for a dedicated LLM scoping session on a possible structural rethink. Three concrete ideas were named to evaluate: moving sketch solving onto the client with a push to the backend only on exit; changing how translations work when moving entities/editing dimensions; and making sure shapes are created with the correct relationships from the start.

Researched via two parallel deep-dive passes (client tool flows across `sketch_controller.dart`/`sketch_canvas.dart`; backend entity/constraint/solver architecture across `models.py`/`constraints.py`/`solver.py`/`profile.py`) rather than reconstructed from memory, so the round-trip counts and mechanism descriptions in the new doc are traced against the actual current code, not approximated.

**New file**: `docs/sketcher-architecture-ux-scoping.md`. Covers the full entity/constraint/solver model, every shape tool's exact tap-by-tap client/backend flow with measured round-trip counts (Slot is the most expensive at ~20-25 round trips per placement, directly tied to it - unlike Polygon - having no real backend entity of its own), a full deep-dive on the drag/move system (why the throttled incremental mid-drag solve exists, what it costs), a round-trip/latency inventory flagging the `_refreshAllPoints` N+1-per-point-GET pattern as the single biggest scaling cost in the whole client, a design-decisions log tying today's architecture back to prior scoping-doc discussions (including the previously-rejected "defer solving to sketch-exit" idea, cross-referenced directly against the new "move solving to client" ask so the scoping session isn't blindsided by prior art), and a final options section addressing all three of the user's named ideas plus lower-risk round-trip-reduction items that don't depend on the larger architectural question being resolved first.

Also added a short pointer to the new doc under `roadmap.md`'s "Sketcher tuning package" section, flagged as under consideration, not decided or started. No code changes this entry - documentation only.

## 2026-07-16 — PR #95's first real CI run: fixed 20 client test failures, all stale for the sandbox's own reasons

The user asked to merge this branch and checked CI first. This was this branch's (and, per the client test file's own long-standing doc comments, effectively this whole project's) first real execution of every `part_screen_test.dart`/`sketch_screen_orbit_view_test.dart`/`sketch_orientation_indicator_test.dart` test - these files transitively import `flutter_scene`, which cannot load under `flutter test` in any sandbox session to date (confirmed still true here too - `flutter_gpu`'s bundled SDK headers don't match this `flutter_scene` version's expectations). Every prior session's "passing" claim for these files meant `flutter analyze` plus manual review only; real CI was the first environment able to actually run them, and it surfaced 20 failures accumulated silently across several earlier sessions' work.

Root-caused each individually rather than patching blind. Two shared root causes covered the bulk of them: (1) task #71's SVG-icon migration (`Icon(Icons.X)` → `SvgIcon(assetPath)` for the FABs and feature-tree leading glyph) left several tests' `find.byIcon`/`find.widgetWithIcon` finders stale, since an `SvgIcon` isn't a named `IconData`; (2) the orientation-confirm step ahead of every new-Sketch creation (predates this session) meant several older tests timed out waiting for `'DIDSA-CAD Sketch'` without ever tapping the new "Continue" button first. A `part_viewport_test.dart` initialViewBasis rename (this session's own earlier task #120 work) broke one more test expecting the old `initialViewPlane` field. Task #95's orientation-UI relocation (hamburger → Feature-tree long-press) left a whole 5-test group in `sketch_screen_orbit_view_test.dart` silently testing a menu entry that no longer exists anywhere in that widget - removed with a comment documenting the relocation and flagging that no replacement coverage exists yet for the relocated flow (`_FakeSketchBackend` has no stub for the orientation PATCH endpoint), a real, acknowledged gap rather than a silently dropped one.

The last failure (`Prompt D - a pre-selected, already-eligible Sketch skips the picker entirely`) took three CI round-trips to actually fix, each teaching something real about this file's testing constraints:
1. A widget-count check plus a guessed 300ms settle pump after popping back from a Sketch still tapped the wrong target - the 'add-fab' FAB is the *same persistent widget* across the whole push/pop (`PartScreen`'s State is never rebuilt, only covered/uncovered by the pushed route), so counting it is satisfied before the pop's own route transition even starts.
2. Swapping in `pumpAndSettle` timed out outright - `PartViewport`'s Scene render loop keeps scheduling frames indefinitely in this sandbox/CI (the same reason `pumpAndSettle` is already avoided everywhere else in this file, for `PartScreen`'s own initial mesh-loading spinner - it turns out that reason isn't unique to initial load).
3. Fixed: many small manual pumps (20×50ms, not one big jump) after the pop, so intermediate frames' post-frame callbacks - where the Overlay actually drops the finished route's RenderObjects - get a chance to run before the tap.

Verified via real CI on the final commit (`1ba8e58`): client `analyze-and-test` 699 passed, 1 failed (the pre-existing, accepted `part_viewport_test.dart` "Fix 4" Impeller/GPU CI-sandbox flake - confirmed identical across every run this session, unrelated to any of these changes); backend `build-and-test (linux/amd64)` 799 passed, 9 failed (the same pre-existing baseline documented in the 2026-07-14 Slot entry above, unchanged). Four commits (`ff4dfb0`, `7454722`, `0f385db`, `1ba8e58`) pushed to `claude/sketcher-roadmap-tuning-7z3shf`; PR #95 is CI-clean and ready to merge.
...
Per the rollout sequence's step 3 ("Implement 'new sketcher' in the main repo") and `docs/sketcher-restructure-plan.md`'s Phase 0/Phase 1, executed against a concrete milestone plan (A: round-trip reduction; B: native foundation; C: Dart solver port; D: Android build wiring; E: wire into the live drag path; F: on-device confirmation). This is the session the environment itself changed to make the rest possible: Miniforge/pythonocc-core got installed on this machine specifically so the backend test suite could actually run here (previously only possible in the cloud sandbox's now-stale conda env), and this turned out to be the same physical Windows laptop the spike used - Flutter, the Android SDK/NDK, and `adb` were already present.

**Milestone A (Phase 0, backend + client).** `_refreshAllPoints`'s N+1 per-point `GET` loop replaced with a single `listPoints` call. New `POST /sketches/{id}/solve-and-refresh` endpoint (`SketchStateResponse` schema) bundling solve + points + constraints + profile into one response; `_solveAndTrackDof` now calls it, collapsing ~26 call sites' `solve` + `_refreshAllPoints` + `_refreshConstraints` triples down to one call each. Deleted the now-dead `_refreshAllPoints`/`_refreshProfile` methods. Backend `pytest`: 801 passed (798 baseline + 3 new), 10 failed - confirmed via a stash/reapply that all 10 are pre-existing and unrelated (this Windows-native pip `py-slvs==1.0.6` build apparently disagrees with the Linux CI baseline on one extra test - `test_slot_leaves_radius_unconstrained_until_confirmed` plus circle/ellipse/arc solving-after-move cases - worth a closer look later, not chased down this session). Client: `flutter analyze` clean, 698/699 passed (the one failure is the same pre-existing `part_viewport_test.dart` "Fix 4" flake noted above).

**Milestone B (native foundation).** Vendored `realthunder/solvespace` as a pinned git submodule (`client/native/slvs/vendor`, commit `73a181b`, matching the spike exactly). Wrote `slvs_ffi_shim.cpp`/`.h` as real production code: 30 `extern "C"` functions forwarding to the fork's `System` C++ class (`src/swig/slvs_swig.hpp`), each catching at the boundary per the spike's own documented finding (a C++ exception must never unwind across `dart:ffi`). Two-step CMake build (`client/native/slvs/CMakeLists.txt`, documented in its own header): build `vendor/`'s `slvs_static` as its own standalone project first (`add_subdirectory` doesn't work here - the vendored CMakeLists.txt hardcodes `CMAKE_SOURCE_DIR` assuming it's always the top-level project), then build the shim linking against the resulting archive directly (`find_library` also doesn't work against the NDK's sysroot-restricted search - construct the path directly instead). Two Windows-specific snags fixed along the way, neither hit by the spike (Android doesn't have them): `SLVS_STATIC_LIB` must be defined when statically linking on Windows (`slvs.h` otherwise emits `dllimport` prototypes with no import stub to satisfy them) and MinGW doesn't auto-export DLL symbols the way ELF `.so`s do (added an explicit `__declspec(dllexport)`/`visibility("default")` macro). Host (Windows) build succeeds cleanly. **Desktop parity harness** (`client/native/slvs/desktop_parity_harness/`, a separate throwaway Dart package) reproduces both of the spike's on-device parity cases against real backend ground truth, now provable on a desktop dev loop instead of only a phone: the simple two-point/distance case (`result_code=0, dof=3, distance=50.0`) and the harder Slot case (2 Arcs, 2 Lines, Tangent+EqualRadius, `result_code=5, dof=0`, positions unchanged from their exactly-tangent seed) - the Slot ground truth was captured fresh from a real py-slvs run on this machine (a one-off script mirroring `_build_slot`'s exact construction), not assumed from the spike's own numbers, and matched them anyway.

**Milestone C (Dart solver port, `client/lib/sketch/local_solver/`).** `slvs_bindings.dart` (full FFI surface), `solver_builder.dart` (`SolverBuilder` interface + `NativeSolverBuilder`, a literal port of `_PySlvsBuilder` including its handle-memoization caches, the sign-preserving `_projectDistance` logic, and the angle-supplement disambiguation), `local_sketch_solver.dart` (constraint dispatch for all 15 `ConstraintDto` types - reusing the client's *existing* DTOs rather than a duplicate parallel hierarchy - plus `solveSketchLocally`, a port of `solve_sketch`/`_solve_sketch_once` including the redundancy-safe-type convergence override and the provisional-DOF floor). Not yet ported: `_fix_circle_cardinal_point_signs` (circle-specific, out of scope for this single-Point-drag landing, flagged in the code rather than silently dropped). Verified with 4 new pure-Dart tests (`client/test/local_solver_test.dart`) against the host-built library, including the same Slot construction end to end through the real port (not just the raw shim) - all pass. Full client suite: 703/704 (same pre-existing flake).

**Milestone D (Android build wiring).** Didn't use Gradle's `externalNativeBuild` - it doesn't fit the two-step recipe cleanly, and the spike's own proven pattern (drop a prebuilt `.so` into `jniLibs/`) is simpler and already de-risked. Built `slvs_static` then the shim for `arm64-v8a` via the NDK toolchain (matching the spike's exact `-DANDROID_PLATFORM=android-24` flags); confirmed via `llvm-readobj`/`llvm-objdump` that the result depends only on `libc.so`/`libdl.so`/`libm.so` and exports all 30 symbols correctly - the same clean result the spike got on-device. Copied to `client/android/app/src/main/jniLibs/arm64-v8a/`; a real `flutter build apk --release` succeeded and the built APK was inspected directly to confirm `lib/arm64-v8a/libdidsa_slvs_ffi.so` is actually bundled (492KB stripped). The `.so` itself isn't committed (regenerable from the already-vendored source; `jniLibs/` is gitignored) - building it is a manual pre-step for now, not yet wired into any automated build graph; worth automating before this is a true zero-step `flutter build apk` for the next person.

**Milestone E (wired into the live drag path, `sketch_controller.dart`).** `updatePointDrag`'s mid-drag reflow (`_maybeSolveDuringDrag`/`_solveDuringDrag`'s throttled `solve` + `listPoints` round trip) now tries the in-process solver first via `_trySolveDuringDragLocally` - no throttle needed since it isn't a network call - falling back to the existing server round trip whenever the native library isn't loadable (lazily attempted once, cached) or the local solve throws for any reason. `endPointDrag`'s final PATCH + `solveAndRefresh` stays a real server round trip unchanged - the backend remains the persistence source of truth, matching the restructure plan's own recommended default. Deliberately scoped to `updatePointDrag` only, per the plan's "one narrow, real interaction path first" - `updateLineDrag`'s own mid-drag solve is untouched. A test-only `localSolverBindings` constructor parameter lets a test inject the host-built library; a new test confirms the actual local-solve-engaged path (not just its fallback): dragging a Point with a live `DistanceConstraint` reflows the other Point to the exact expected distance with zero `/solve`-family requests logged. Every one of the 703 pre-existing tests (none inject a native library) continues to pass unchanged, confirming the fallback is exactly as invisible as it needs to be.

**Not done this session, deliberately: Milestone F.** Real on-device confirmation ("does the drag feel instant") on the actual Adreno 740 test device, per the restructure plan's own §6 phase-gate discipline - the same thing both spikes' own verdicts ultimately rested on, not sandbox argument. Needs the user to connect the device.

## 2026-07-17 — Milestone F: real on-device confirmation, and a genuine release-build networking bug found along the way

Connected the actual Adreno 740 test device (S23 Ultra) over wireless ADB and installed a real `flutter build apk --release` build - the same one Milestone D's build already confirmed bundles `libdidsa_slvs_ffi.so` correctly. First attempt to connect the app to the real backend (`https://cad-api.snail-shell.uk`, the production Cloudflare-Tunnel-fronted Pi) failed with a generic "Could not reach server" error.

**Root-caused rather than guessed at**, since the generic error string covers any exception in `ConnectionScreen._handleConnect`. Ruled out, in order, each with real evidence rather than assumption: the backend itself (curled directly from the Pi with the real key - `200 {"status":"ok",...}`); the stored API key (same curl, same key, works); a stale/autofilled key in the app's own field (cleared and pasted fresh - no change); Private DNS (already off); the VPN/meshnet the phone reaches this machine through (disabled entirely - no change); per-app Wi-Fi/mobile-data restrictions and battery optimization on this Samsung device (checked, unrestricted). None of it. A temporary diagnostic `print` in the catch block (release builds strip Flutter's own debug logging, so a plain `print()` - which the engine still forwards to logcat - was the only way to see the real exception) surfaced the actual error: `SocketException: Failed host lookup: 'cad-api.snail-shell.uk' (OS Error: No address associated with hostname, errno = 7)` - a DNS resolution failure specifically for this app's own process, while Firefox on the same phone, same network, resolved the identical hostname fine at the same moment.

**Actual root cause: the app has never had the `INTERNET` permission in a real release build.** `android/app/src/main/AndroidManifest.xml` (the base manifest, which applies to every build type) never declared it; it only ever existed in `android/app/src/debug/AndroidManifest.xml` and `.../profile/AndroidManifest.xml`, each with their own comment explaining it was added there specifically for the Flutter tool's hot-reload/DevTools connection during development. `flutter run` (debug mode) has therefore always worked - it gets `INTERNET` from the debug overlay - but a genuine `flutter build apk --release`, which is what every phase gate in this project's own discipline actually calls for, has never had network access at all. Confirmed directly both ways: `aapt2 dump permissions` on the pre-fix APK showed only an auto-generated `DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, no `INTERNET`; after adding `<uses-permission android:name="android.permission.INTERNET"/>` to the main manifest and rebuilding, the same dump showed it present. This is why "the last full build I tried before the big changes yesterday worked" - that was a debug-mode run, not a release build; this is the first time in the project's history a real release APK has been built and network-tested against a live backend, and it surfaced a gap nobody had hit before for exactly that reason.

**Verified, real, on-device**: with the fixed build installed, the connection succeeded against the production Pi backend, and dragging a plain Point (with other geometry/dimensions in the sketch for context) was confirmed noticeably smoother - the actual thing Phase 1's whole restructure exists to fix. Also confirmed, and expected rather than a regression: dragging a Polygon vertex or a Slot's own points still feels exactly as slow as before, since Milestone E deliberately only wired the local solver into `updatePointDrag`'s plain-Point path - a Polygon vertex drag reinterprets through `_maybeUpdatePolygonRadiusDuringDrag` (a separate branch that returns before ever reaching the new code), and neither that path nor any other beyond the single-Point case has been widened yet, per the plan's own "one narrow interaction path first, before widening to the full constraint vocabulary." Widening coverage to more drag paths is real, separate follow-up work, not a bug in what shipped this session. The user explicitly deferred chasing this further: "let's finish the changes we're already doing before solving this" - i.e. finish Phase 2 (below) first.

## 2026-07-17 — Phase 2 (plane-embedded 3D sketching), milestones P1-P5: tap-to-place Point/Line lands inside Orbit View itself

Per `docs/sketcher-restructure-plan.md`'s Phase 2 - moving sketch interaction into the same 3D viewport/camera as Orbit View, instead of a separate flat 2D canvas. Sequenced right after Phase 1 (above) per the plan's own ordering. Scoped via a dedicated planning round: researched first (confirmed the backend needs zero changes - `Point.x`/`Point.y` are always flat local 2D regardless of the Sketch's own `Plane`; the solver never sees 3D at all) and that Orbit View's embedded `PartViewport` backdrop already existed half-built, just read-only, gated on `flutter_scene` having no orthographic camera at the time - precisely what Spike B's Track 2 (see the 2026-07-08-ish spike docs) had already resolved. Four scoping decisions were confirmed with the user before writing code: rollout via a **persisted setting** (not a live-only toggle or an outright 2D deletion - the user's own proposed third option, chosen specifically so the 2D sketcher stays available until 3D is proven); **Point + Line only** for this landing; **tap-to-place only** (no drag/edit of 3D-embedded geometry yet, sidestepping the orbit-vs-drag gesture ambiguity entirely); and **orthographic as the camera default**. That last one grew in scope mid-conversation: wiring a real orthographic camera meant `OrbitCamera.isPerspective` - previously a no-op flag, since `flutter_scene` had no orthographic camera to give it - stopped being a no-op everywhere it's read, not just in the new embedded sketch view. Flagged to the user directly rather than silently narrowing it back down; they confirmed explicitly ("let it apply everywhere... orthographic as default, perspective as an option") - this was the first time orthographic rendering was actually possible, so the rollout goes app-wide as originally intended, not scoped down to just this feature.

**P1 (settings).** New `SketcherPreferences` (`client/lib/sketch/sketcher_preferences.dart`) and `SketcherSettingsScreen`, mirroring the existing `MeshViewerPreferences`/`MeshViewerSettingsScreen` pattern exactly (`shared_preferences`-backed, one bool: `use3DSketcher`). Reached via a new `SettingsHomeScreen` behind the connection screen's existing gear icon (now a small menu of "Mesh Viewer" / "Sketcher" rather than jumping straight to Mesh Viewer's own settings). `SketchScreen`'s existing `_orbitViewActive` toggle - previously always `false` on open - now seeds from this preference, reusing all of Orbit View's already-built camera/gesture/rendering infrastructure instead of standing up a parallel 3D entry point.

**P2 (real orthographic camera, app-wide).** Promoted `OrthographicProjection`/`OrthographicCamera` out of the throwaway spike file (`b1_tap_test_screen.dart`) into production (`client/lib/viewport3d/orthographic_camera.dart`) unchanged - the spike's own header comment already confirmed they needed no changes to work. `OrbitCamera.cameraFor()` widened from always returning `PerspectiveCamera` to the base `Camera` type, actually branching on `isPerspective` for the first time. Every call site reading a `cameraFor()` result that only used base-`Camera` members got its own signature widened to match (`triadAxes()`, `SketchOrientationIndicator`, `screen_projection.dart`'s `worldToScreen`) rather than special-cased; `MeshViewerScreen` (a separate feature, no perspective/orthographic UI of its own) pins `isPerspective = true` explicitly to keep its current behavior unchanged. Removed two now-stale "orthographic not yet available" UI comments/subtitles.

**P3 (ray-vs-sketch-plane).** New `hitTestSketchPlane` (`client/lib/viewport3d/sketch_plane_hit_test.dart`) - deliberately simpler than the existing `hitTestReferencePlanes` (which tests 3 candidate planes and picks nearest): the embedded sketch view only ever has one plane, the current Sketch's own already-resolved `SketchPlaneBasis` (fixed or custom alike). Pure function, unit-tested directly (4 tests: straight-down hit, plane-parallel miss, behind-the-ray miss, an arbitrary tilted/offset custom plane cross-checked against the existing `worldPointToSketch` round-trip) - the same "pure math, testable without touching `flutter_scene`" pattern the Phase 1 local-solver work already established for this sandbox's known limitation.

**P4 (wire it up for real).** `PartViewport` gained `sketchPlaneBasis`/`onSketchPlaneTap`; its tap handler now tries `hitTestSketchPlane` before falling through to the existing background-tap path. `SketchScreen._handleEmbeddedSketchTap` converts the resulting world-space hit back to the Sketch's own local (x, y) via the already-existing `worldPointToSketch` and feeds it straight into `SketchController.handleCanvasTap` - the exact same entry point every 2D-canvas tap already funnels through, confirmed by this session's own research, so zero controller/tool logic needed to change. `SketchSpeedDial` gained `restrictToPointAndLine`, tagging each tool action with its `SketchTool` and filtering both the categories row (hiding Dimensions/Trim, both reliant on 2D-only ghost-picking) and the tool list down to Point/Line while embedded. Rewired `SketchScreen`'s FAB layout: the speed dial now stays visible (restricted) instead of being replaced by "Return to Default View" while in Orbit View - that button moved to the top-right FAB column instead, alongside the Orbit View toggle. `_enterOrbitView()` now resets the active tool to Point on entry. One pre-existing test (`sketch_screen_orbit_view_test.dart`) asserted the old replace-not-restrict behavior; updated to match the new intent (speed dial present and restricted, not absent) rather than deleted.

**P5 (snapping/live-rendering - verified already-correct, no code needed).** Confirmed by reading the code rather than assuming: `SketchController.handleCanvasTap` already snaps to nearby existing Points purely from local (x, y) proximity (`snapRadius`, the same value the flat 2D canvas has always used) via `_pointIdAt`/`_existingPointIdNear` - since P4's `_handleEmbeddedSketchTap` converts the 3D ray-plane hit into that same local coordinate space *before* calling `handleCanvasTap`, every existing snap path (a Line's start/end endpoint reuse, standalone-Point auto-coincidence, midpoint snapping) already applies identically and automatically to embedded 3D taps - no separate ray-based snap logic was needed, and adding one would have duplicated this rather than added anything. Live rendering was likewise already wired: `_buildBaseLayer()` (which recomputes `sketchGeometry3DFrom` from the controller's current Points/Lines/etc.) sits inside an `AnimatedBuilder(animation: _controller, ...)`, so every `notifyListeners()` a tap triggers - which every draw-mode tap already does - repaints the embedded 3D geometry immediately.

Verified throughout: `flutter analyze` clean at every milestone; `flutter test` at 708 total (705 + the updated P4 test), only the one pre-existing, established `part_viewport_test.dart` "Fix 4" Impeller/GPU sandbox flake failing - no regressions. P4/P5's actual interactive behavior (tap accuracy, snap feel, orthographic rendering judged by eye) is real-device-only, same known limitation as every prior 3D-touching phase in this project - **P6 (on-device confirmation) is the remaining, user-gated step**, not yet done.

## 2026-07-17 — P6 confirmed on-device; P7-P10: real Bodies now shape the sketch-plane surface, grid, and Dimension-mode picking

**P6.** Connected over wireless ADB, installed the release build, confirmed directly: "sketcher now feels connected to the model and viewer" - tap-to-place Point/Line inside Orbit View works, and bodies are visible behind the sketch plane for the first time (a genuinely new capability, not just a performance win). Phase 2's original 6-milestone scope (P1-P6) is complete.

That on-device pass immediately surfaced three real follow-on needs the flat-2D-canvas era's design choices never had to consider, since nothing 3D ever sat behind the canvas before: canvas transparency finally has something worth seeing through it; the embedded view's background should match whatever the user already set for the main 3D viewport; and the sketch plane needs some visual structure (a grid) now there's no flat white canvas under it. Scoped and planned as a follow-up batch (P7-P11) via a dedicated planning round (two Explore passes grounded the design against real code rather than assumption) before any of it was built, per the user's own "let's scope and weigh any trade offs" request - also folding in a related ask: real Body edges/vertices should be directly pickable, starting with the Dimension tool (Convert Entity/Offset Entity don't exist as tools yet, and are explicitly out of scope for this batch - Offset in particular would need new backend geometry support).

**P7 (background colour sync).** `SketchScreen`'s embedded `PartViewport` was silently discarding `ViewPreferences.bgColourHex` (never passed at all, always falling back to the constructor default) - now loads `ViewPreferences` defensively alongside `SketcherPreferences` in `_loadInitialOrbitViewPreference` and passes the real persisted value through.

**P8 (sketch-plane surface + adjustable transparency).** There was no rendered surface at all for the active Sketch's own plane while embedded-sketching (the 3 fixed reference planes are explicitly hidden there) - new `buildSketchPlaneSurfaceNode`/`doubleSidedSketchPlaneQuadBuffers`/`sketchPlaneBorderPoints` (`sketch_geometry_3d.dart`) build a translucent fill + opaque border for an *arbitrary* `SketchPlaneBasis` (fixed or custom), mirroring `reference_planes.dart`'s `buildReferencePlaneNode` but built directly in world space via `sketchPointToWorld` rather than a local quad + rotation matrix, since a custom plane has no fixed rotation table to key off. New `PartViewport` node (`_sketchPlaneSurfaceNode`, synced the same remove/rebuild/add way `_planeNodes`/`_sketchNodes` already are) with colour/opacity threaded from new `SketchScreen` state (`_orbitCanvasColourHex`/`_orbitCanvasOpacity`, defaulting to a subtle `0.18` alpha) and two new "Canvas Colour"/"Canvas Transparency" entries in the Orbit View menu, reusing the exact existing `showColourSwatchSheet`/`showBodyOpacitySheet` helpers Body Colour/Transparency already use one section up.

**P9 (grid lines).** No grid-rendering precedent existed anywhere in the client. New pure `sketchGridLinesFrom(basis, {spacing, extent})` (fixed, finite extent - a camera-following/fading "infinite grid" look would need dynamic per-frame regeneration or a shader, neither of which exists here, explicitly deferred) plus GPU-facing `buildSketchGridNode`, mirroring `sketchGeometry3DFrom`/`buildSketchGeometryNode`'s own pure-function/GPU-builder split so the geometry math stays unit-testable (4 new tests, `test/sketch_grid_test.dart`, including one confirming the grid honors an arbitrary tilted/offset custom plane the same way the P3 ray-hit-test's own test does). New `_orbitGridVisible` (defaults **on**, per the ask) with a `SwitchListTile` in the Orbit View menu.

Both P8's surface and P9's grid sit exactly on the same plane as the Sketch's own real Points/Lines (and the ray-hit target `hitTestSketchPlane` itself resolves to) - rendering them at that same literal depth risks z-fighting against genuinely-drawn geometry, so both are pushed a small epsilon backward along `-basis.normal` for rendering purposes only (surface furthest back, grid in between, real geometry unaffected at true zero offset) via a new `_offsetSketchPlaneBasis` helper - never fed into hit-testing or coordinate conversion, purely a draw-order/z-fight mitigation.

**P10 (edge/vertex tap priority for Dimension mode).** `SketchController` never sees screen or ray coordinates - `handleCanvasTap` only takes already-resolved sketch-local `(x, y)` - so the new priority decision lives in `PartViewport`/`SketchScreen`, mirroring the flat 2D canvas's own existing Dimension-mode priority exactly (`sketch_canvas.dart`'s `_dispatchTap`: real sketch entity wins first, then a ghost/Body pick, then nothing). New `PartViewport.preferEntityPick`/`onSketchEntityTap`/`hasEntityNearSketchTap` (the last a callback-injected predicate, the same "backend-shaped predicate as a callback" pattern `sketchLineLoopGroup` already uses, since this widget has no dependency on `sketch_controller.dart`); `SketchScreen` gates it on a single boolean expression (`_controller.mode == SketchMode.dimension`) deliberately kept as one line so a later Convert Entity/Offset Entity mode is a one-line addition, not a redesign. On a miss against the current Sketch's own entities, `_handleTap` now runs the already-existing, already-tiered `hitTestBodies` (vertex/edge only - explicitly excluding `face`, since there's no `pickReferenceGhostFace`-equivalent method and leaving face-hits enabled would otherwise silently swallow the tap instead of correctly falling through to the plane-tap miss path) and, on a hit, calls the *already-existing* `SketchController.pickReferenceGhostVertex`/`pickReferenceGhostEdge` directly - both already do the full materialize-cache-and-select job the flat canvas's own ghost-pick uses, so this needed zero backend or controller changes, only a new trigger point. Confirmed low-risk to other tap flows before writing any code (via the planning round's own research): the embedded-sketch `PartViewport` is a structurally separate widget instance from `part_screen.dart`'s own (which drives "New Sketch"/"New Sketch on Face"/reference-plane picking), with `onPlaneTap`/`onBackgroundTap` hardcoded no-ops and `selectionMode` always false there - nothing in this change touches those other flows. (The plan's own optional cosmetic cleanup - renaming `pickReferenceGhostVertex`/`pickReferenceGhostEdge` to drop the now-inaccurate "ghost" naming, since the embedded-3D path calls them against real geometry directly - was skipped: touches 7 files for naming clarity alone, no behavior change, not worth the risk against this session's remaining scope.)

Verified: `flutter analyze` clean throughout; `flutter test` at 711 total (708 + 3 new grid tests), same one pre-existing `part_viewport_test.dart` "Fix 4" Impeller/GPU sandbox flake, no regressions. P8/P9/P10's actual rendering/feel (transparency level, grid look, edge-pick accuracy against a real Body) is real-device-only per this project's established phase-gate discipline - a fresh release build was installed on the test device; on-device confirmation itself is still outstanding.
...
Same day, immediate on-device feedback on the P7-P10 build above. The user also explicitly deprioritized the flat 2D canvas ("it will probably get killed off after the 3d sketcher had been worked up") - resolving, by simply mooting it, the "what should toggling Orbit View off do" architecture question raised earlier in the session (a locked-normal-view redesign is no longer worth chasing while the 2D canvas itself is headed for retirement).

**Grid/surface fade + border removal.** The P8 surface's hard-edged border didn't suit a surface meant to be seen *through*, and both the surface and grid stopped abruptly at their own extent instead of fading. `UnlitMaterial.baseColorFactor` has no per-vertex gradient, so both fixes approximate a gradient using only constant-alpha primitives (this codebase's existing rendering vocabulary): `buildSketchPlaneSurfaceNode` now layers 5 concentric, progressively smaller squares (same alpha each, composing under `AlphaMode.blend` so the centre reads closer to the full chosen alpha, tapering outward) instead of one flat quad + border; `buildSketchGridNode` now splits each full grid line into 6 short pieces, each with its own alpha faded by that piece's own Euclidean distance from the plane's origin (Chebyshev/local-coordinate math wasn't needed - straight world-space distance from `basis.origin`, normalized by `extent`, was enough). Grid spacing's own default widened from `1.0` to `2.5` sketch units per the ask, keeping primitive count reasonable now each line is itself subdivided into 6 pieces. `sketchGridLinesFrom` (the pure, tested layer) is unchanged in its own signature/behaviour - only its GPU-facing counterpart changed.

**Tap priority (edge/vertex pick vs. plane-tap) - real bug found and fixed.** `hitTestBodies`'s `filter` param defaults to `face: true` - the P10 call left it at that default, so a Dimension-mode tap landing on a Body *face* (not near any edge/vertex) resolved to a `SelectionEntityKind.face` hit, which `_handleEmbeddedSketchEntityTap`'s switch has no case for - silently swallowing the tap instead of falling through to the ordinary plane-tap miss behaviour. Fixed by passing an explicit `SelectionFilterState(vertex: true, edge: true, face: false, body: false)` - there's no `pickReferenceGhostFace`-equivalent method, so a face was never a valid target to begin with.

**Feature-tree "long-press a Plane row > Hide" wasn't wired at all** (`feature_tree_panel.dart`). `_buildFeatureTile` (the "Features" section's own row for every Feature, including a `create_plane` one) has always had `onLongPress: () => widget.onFeatureLongPress(feature)`; `_buildPlanesSection`'s own separate shortcut row for the same Feature (in the "Planes" section) never did - `onTap` only. Added the same `onLongPress`, plus the `hidden`-state `Opacity`/`visibility_off` trailing icon `_buildFeatureTile` already shows, for full parity between the two rows referencing the same Feature.

**New: origin/reference planes auto-hide after the first real Body.** They're a placement aid for an empty Part; once a Body exists they're clutter. `PartScreen._refreshMesh` (the single place `_bodies` genuinely changes, per its own doc comment) now sets `_referencePlanesHidden = true` the first time `_bodies` transitions from empty to non-empty, guarded by a new one-time `_hasAutoHiddenReferencePlanes` flag (mirrors `PartViewportState._hasFramedCamera`'s own "one-time auto-behaviour" pattern) so a later delete-everything-then-rebuild cycle never fights a user who explicitly re-showed the planes themselves.

**New: default view orientation rebuilt to a Z-up CAD convention.** Explicit ask: world Z+ should read as pure screen-up, X+ as down-and-left, Y+ as up-and-left, for both the cold-start default view and the new-sketch isometric preset (`animateToIsometric`). `OrbitCamera`'s old `_defaultOrientation`/`isometricOrientation` composed `Quaternion.axisAngle` rotations targeting a Y-centric scheme (X+ read as pure right, Y+ up-left, Z+ up-right - not what was wanted). Both are now built from the desired world-space screen axes directly (`_zUpOrientation(elevation)`, parametrized so the two presets keep their existing different "steepness" - ~30° pleasant default vs. ~35.264° true isometric - while sharing the same axis convention), converted via `Quaternion.fromRotation(Matrix3.columns(right, up, back))`. Hit, and fixed, a genuine `vector_math` (2.4.0) quirk along the way, confirmed by a throwaway numeric probe rather than assumed: `Quaternion.rotate()` computes `conjugate(this) * v * this`, not the more commonly assumed `this * v * conjugate(this)` - a bare `fromRotation(...)` rotated local axes to the *wrong* world directions entirely (confirmed reproducible, not a one-off); `.conjugated()` on the constructed quaternion compensates. Two `orbit_camera_test.dart` tests updated to check the new axis convention directly (dot-product signs against world X/Y/Z) rather than the old azimuth-based assertions, plus a new shared `expectZUpAxisConvention` helper covering both presets.

Verified: `flutter analyze` clean; `flutter test` at 712 total (711 + 1 new default-orientation test alongside the rewritten isometric one), same one pre-existing sandbox flake, no regressions. All of this is rendering/camera-feel, real-device-only per usual - a fresh release build was installed on the test device for the next on-device round.

## 2026-07-17 — Cursor/select mode for the embedded 3D sketcher (P12-P14), and a real depth-sort bug found and fixed

Same day. The user asked to "start rolling out all the features from the 2D sketcher to the viewport sketcher," starting with a cursor/select mode toggle including dynamic hover highlighting. Scoped via a dedicated planning round (two Explore passes) before writing code - the key finding reframed the task from "build a selection system" to "wire up one that already exists": `PartViewport`'s Selection-mode hover/highlight machinery (`_buildEntityHighlightNode`/`_syncSelectedEntityNodes`) already has real, working cases for `sketchPoint`/`sketchLine`/`sketchCircle`, gated purely on `sketchGeometries` (already passed by the embedded instance since P4/P5) - and `SketchController`'s own selection-driven actions (`deleteSelected`, `availableConstraintOptions`/`applyConstraintOption`, `setSelectedConstruction`) are already screen-agnostic, operating purely on sketch-local ids with no notion of 2D vs 3D. `SketchRibbon` (the action UI these drive) was already built and screen-space-only - just conditionally hidden during Orbit View for no reason beyond nothing being selectable there yet.

**P12 (the toggle).** The 2D canvas's existing mode-indicator pill (tap-to-`exitToSelectMode`) was un-hidden during Orbit View (dropped its `|| _orbitViewActive` condition) - no new UI needed for entry; the already-visible (P4) speed dial's Point/Line taps already exit select mode back into draw mode via existing `selectDrawTool` dispatch, unchanged. `PartViewport.selectionMode` on the embedded instance now follows `_controller.mode == SketchMode.select`, with a new restricted `SelectionFilterState` (sketchPoint/sketchLine only - Body vertex/edge/face picking stays P10's own `preferEntityPick` concern, not cursor mode's).

**P13 (the selection adapter).** `PartViewport` speaks `SelectionEntityRef` (kind + `sketchFeatureId`/`sketchEntityId`); `SketchController` speaks `SketchSelection` (`SelectionKind` + a bare id). Found the exact reusable precedent: `selectConstraint`'s own body already implements "add-to-selection-vs-replace by an already-known selection, no hit-testing" - extracted into a new public `SketchController.selectEntity(SketchSelection)`, with `selectConstraint` becoming a one-line wrapper. New `SketchScreen._handleEmbeddedSelectionToggle`/`_embeddedSelectedEntities` convert both directions.

**P14 (un-hide the ribbon).** Dropped `SketchRibbon`'s own `if (!_orbitViewActive)` gate - it's a plain screen-space overlay with no 2D-canvas coordinate dependency, already correctly stacked relative to the hamburger FAB. Brings across Delete, Make Construction/Solid, the relational-constraint chips, and Length-edit essentially for free.

**A real depth-sort bug, found and fixed along the way.** Testing the previous round's grid/surface work surfaced two on-device reports: the grid rendering *behind* the translucent surface (should be in front), and the surface's fade shape not matching the ask (should stay solid through the interior, fading only in the outer ~20%, not fade starting from the centre). The second was a straightforward rework of the fade math (see below). The first led to reading `flutter_scene`'s own `SceneEncoder._depthOf` source directly rather than guessing: the translucent pass depth-sorts by `worldTransform.getTranslation()` - a `Node`'s own transform origin - not by its actual mesh vertices. `buildSketchPlaneSurfaceNode`/`buildSketchGridNode` (and, pre-existing and unrelated to this session, `buildSketchGeometryNode` for real Sketch geometry) all left their `Node`s at the identity transform and baked world-space positions (including the small render-only z-fighting-avoidance epsilon from the previous round) straight into vertex data - so every one of their primitives sorted as if it sat at the world origin, tied with each other regardless of the plane's real position, with the actual sort order then falling to insertion-order tie-breaking rather than genuine depth. Fixed by moving *all* of a Node's position - both `basis.origin` and the epsilon - into its own `localTransform`, with vertex positions rebuilt relative to that origin instead (`_sketchPlaneNodeTransform`, `_sketchPlaneLocalQuadCorners`) - the depth-sort key now genuinely reflects each Node's real position, correctly camera-side-independent (a true "back to front from the camera" distance, not a fixed-axis offset the previous epsilon-in-vertex-data approach amounted to). `sketchGridLinesFrom` lost its now-obsolete `applyRenderEpsilon` parameter (epsilon lives in the Node transform exclusively now) - one now-redundant test removed, the other two updated to match.

**Fade shape rework** (`buildSketchPlaneSurfaceNode`/`buildSketchGridNode`): new shared `_edgeFadeAlpha(normalizedDistance)` - full alpha through 80% of the radius, linear taper to zero over the remaining 20% - replacing the previous "fades from the centre" nested-squares/distance-based approach. The surface's own 8 nested-square layers are now sized between 80%-100% of the full extent (every layer already covers the `[0, 80%]` interior, so that band composites to a constant near-full alpha; only the outer band sees progressively fewer layers as radius grows) rather than shrinking from 100% down to a point.

**Also flagged, not yet resolved: a default-camera-view discrepancy.** The user reported the actual on-device default view still didn't match a reference screenshot, worried about a possible coordinate-system misalignment. Checked directly: the reference screenshot's own triad reads exactly Z-up/X-down-left/Y-up-left, matching the previous round's fix and its passing unit tests bit-for-bit - no code path was found that could override `OrbitCamera`'s own default orientation for the main Part viewport's cold-open. Likely explanation offered (not yet confirmed): timing - the screenshot may have been taken before the fixed build finished installing. Asked for a fresh comparison on a brand-new Part (camera orientation isn't persisted anywhere, so a genuinely fresh `PartViewportState` should reflect the fix unconditionally) once this round's build lands.

Verified: `flutter analyze` clean; `flutter test` at 711 total (one now-redundant grid test removed), same one pre-existing sandbox flake, no regressions. Cursor mode's actual feel (hover tracking touch-drag, ribbon interaction, whether relational constraints apply correctly against 3D-placed geometry) and the render-order/fade fixes are real-device-only per usual - a fresh release build was installed on the test device.

## 2026-07-17 — Camera rollback, orientation-tool orbit-mode fix, grid-fade edge-distance fix

Same day. On-device feedback: the previous round's Z-up camera rewrite made things worse, not better - the user asked for a clean rollback rather than another guess, plus a suggestion for how to nail the exact desired view without more screenshot-comparison guessing.

**Camera rolled back.** `OrbitCamera._defaultOrientation()`/`isometricOrientation()` reverted to the exact pre-session code (diffed directly against `git show HEAD:...` rather than reconstructed from memory, to be certain) - back to the original `pitch * yaw` axis-angle composition (pi/4 azimuth, pi/6 elevation default; true-isometric elevation for the new-sketch preset). `orbit_camera_test.dart`'s two orientation tests reverted to the original azimuth-based assertions the same way - kept the unrelated `camera.target`-based access pattern from this session's earlier `cameraFor()`-return-type-widening work (P2), since that fix is independent of the orientation rollback and still correct/needed.

**Suggested a better way to pin down the exact desired view**: rather than more screenshot comparisons, have the app itself report the live camera's exact numbers while the user manually orbits (using the app's own existing gesture) to precisely the view they want - offered to add a temporary debug readout for this, not yet requested/built.

**Sketch-orientation tool now forces orbit mode, restoring cursor mode after.** Real, on-device-found bug: entering the orientation-confirm step (`_addSketchFeature`'s new-sketch camera sequence, and `_redefineSketchOrientation`'s "Redefine Orientation" entry) never touched `PartScreen._selectionMode` - if cursor/select mode was active beforehand, it stayed active throughout, and `PartViewport` routes every tap/drag to its selection pipeline while `selectionMode` is true (confirmed via `_onPlaneTap`'s own existing doc comment), meaning orbit gestures - exactly what's needed to judge a pending orientation from multiple angles - were silently unavailable. Fixed: both entry points now save the current `_selectionMode` into a new `_pendingOrientationPreviousSelectionMode` and force it off; both exit points (`_confirmPendingOrientation`/`_cancelPendingOrientation`, covering both confirm and cancel) restore it.

**Grid fade fixed to use edge-distance, not centre-distance.** The previous round's fade used Euclidean distance from the plane's centre - correct for a circle, wrong for a square: a corner is `extent * sqrt(2)` from centre while an edge midpoint is only `extent` away, so corners faded out well before the edges did, an asymmetry the user caught directly ("lines disappear too soon in the corners but persist closer to the edges"). Fixed in `buildSketchGridNode` (`sketch_geometry_3d.dart`) to use Chebyshev distance instead (`max(|localX|, |localY|)`, via dot products against the basis's own `xAxis`/`yAxis`) - reaches `extent` uniformly along the entire square boundary, corners included, matching the shape of the surface's own already-correct (square-layered) fade from the previous round.

Verified: `flutter analyze` clean; `flutter test` at 710 total (net -1: two Z-up-specific tests removed, one original azimuth test restored), same one pre-existing sandbox flake, no regressions. A fresh release build was installed on the test device (a wireless-ADB reconnect was needed mid-session, same recurring environment quirk as every prior round).

## 2026-07-17 — Camera calibration: a real debug tool, a real sign bug it found, and the isometric default finally correct

Same day. The user accepted the offered debug-readout idea and ran a genuine confidence test: orbit to a known orientation (checked against the on-screen triad as ground truth, e.g. "Z axis points right"), read the debug tool's numbers, confirm they match. They immediately found a real, consistent bug: **every axis's own "right" value had the wrong sign** - and separately supplied a reference capture of their actual desired default view's readout, asking for "the nearest isometric view" to those numbers.

**Root cause of the sign bug, found by deriving `triadAxes` (`triad.dart`) algebraically rather than guessing:** `OrbitCamera.right`/`.up` are the camera's own *local-frame* vectors (`orientation.rotated(_localRight)`/`orientation.rotated(_localUp)`) - not necessarily what actually renders as screen-right/up. The debug tool's first version read `OrbitCamera.right` directly. `triadAxes` instead derives its own right/up independently, via `camera.up.cross(camera.forward)` then `forward.cross(right)` - and working through the algebra (`_right × _up = _direction` for any right-handed orthonormal camera frame) shows `triadRight = -OrbitCamera.right` exactly, while `triadUp = OrbitCamera.up` unchanged. The debug tool now reproduces `triadAxes`' own formula directly instead of reading `OrbitCamera.right`/`.up`, so it reads identically to the trusted on-screen triad.

**This also explained why the earlier (already-rolled-back) Z-up rewrite looked wrong despite passing its own unit tests**: that attempt's tests checked `orientation.rotated(localAxis) == target` - genuine self-consistency - but never cross-checked against `triadAxes`' independent, negated-for-"right" convention, so a mirrored-on-screen result could (and did) pass every test.

**The new isometric default**, built and *numerically pre-validated* this time (a throwaway scratch test reproducing both the construction and the fixed debug tool's own formula, checked against the user's actual captured numbers, before touching real code - the same rigor that caught the sign bug applied proactively rather than after another on-device round-trip): world X+ and Y+ both read as screen-left (their shared `right` component negative), Z+ reads as pure screen-up - the user's own described "nearest isometric view." Also structural, not just numeric: `OrbitCamera`'s previous `pitch * yaw` axis-angle composition can *only* ever produce an `up` vector with a zero world-X component (yaw leaves local-up untouched; pitch alone can't introduce that) - it was mathematically incapable of reaching this specific corner, confirming the general vector-construction approach (`Quaternion.fromRotation(Matrix3.columns(right, up, back)).conjugated()`, the same technique from the rolled-back attempt, minus its wrong target vectors) was the right tool, not the wrong one. `_defaultOrientation` and `isometricOrientation` are now the same view (previously a deliberately different, looser ~30-degree default vs. a true-35.264-degree isometric preset) - the reference the user captured was explicitly the desired *default*, and its own numbers matched true isometric elevation almost exactly.

Since `MeshViewerScreen` never overrides `OrbitCamera`'s own default orientation (confirmed - just `OrbitCamera()..isPerspective = true`, no `.orientation` assignment), this fix reaches it automatically, addressing the user's flagged concern that "any errors in this viewport may also extend to the mesh viewer part of the app" without needing a separate change there.

Two new/rewritten `orbit_camera_test.dart` tests: one confirms `isometricOrientation`/the default now produce the identical view; one reproduces `triadAxes`' own formula directly (not `OrbitCamera.right`/`.up`) and checks against the user's own captured numbers - the same test shape that would have caught the previous round's mirroring bug had it existed then.

Verified: `flutter analyze` clean; `flutter test` at 711 total, same one pre-existing sandbox flake, no regressions. A fresh release build was installed on the test device. The debug camera-orientation readout (`PartViewport._debugShowCameraOrientation`, a single `const bool`) is still present and now fixed - left in for one more on-device confidence-check round before removal, per the user's own request to keep verifying.

The user confirmed on-device: "the values in the debugger are now correct according to the axes on the triad."
...
Same day. Three asks: (1) the connection screen's single combined "Settings" menu (Mesh Viewer + Sketcher together, behind one gear) should split into two independent menus, one per area; (2) the camera-orientation debug readout (still mid-calibration) should become a real, persisted toggle rather than a hardcoded `const bool`, in both settings screens; (3) a new Sketch's default orientation on XY/XZ planes should start pre-flipped.

**Settings split, clarified via a quick layout question** (three options with ASCII previews - the user picked restyling Connect itself into the same 80/20 stadium-split shape the Mesh Viewer bar already uses, rather than a separate bar or a small standalone icon). `connection_screen.dart`: the Mesh Viewer bar's own gear now goes straight to `MeshViewerSettingsScreen` (was `SettingsHomeScreen`); Connect is restyled from a plain `ElevatedButton` into the same `Material`/`StadiumBorder`/`Row`/`InkWell`/`VerticalDivider` shape as the Mesh Viewer bar - 80% "Connect" (kept its busy-spinner and disabled-dimming, now done manually via `InkWell.onTap: null` plus a conditionally-dimmed text colour, since `InkWell` has no built-in disabled look the way `ElevatedButton` does), 20% a new settings gear to `SketcherSettingsScreen`. `SettingsHomeScreen` deleted outright (confirmed zero remaining references first) - each area now has its own direct entry point, so the intermediary list screen served no purpose.

**`SketcherSettingsScreen` retitled "CAD Settings"** (was "Sketcher Settings") - it now also holds a setting unrelated to sketching specifically (the debug toggle below), so its own AppBar text was updated to match what it's actually reachable as ("settings regarding the cad part of the app", the user's own phrasing) - the class/file name stayed unchanged (minimal churn on the actual identifier).

**Camera debug readout promoted to a real, persisted toggle - and given to the mesh viewer too**, which has its own entirely separate camera/viewport (`_MeshViewerViewport`, confirmed via its own doc comment: deliberately not reusing `PartViewport`, so the P7-round debug overlay never reached it). The shared computation (`debugCameraOrientationText`) and a small reusable widget (`DebugCameraOrientationOverlay`) both moved into `triad.dart`, next to `triadAxes` - the natural shared home, and it keeps the "must match the trusted on-screen triad, not `OrbitCamera.right`/`.up` directly" invariant in one place rather than two copies that could drift apart. Two new, independent persisted booleans - `ViewPreferences.debugShowCameraOrientation` (read by `PartViewport`, toggle in the new CAD Settings screen) and `MeshViewerPreferences.debugShowCameraOrientation` (read by `_MeshViewerViewport`, toggle in `MeshViewerSettingsScreen`) - deliberately separate rather than one shared flag, so each viewport's readout can be toggled independently. `_MeshViewerViewport.build()` needed wrapping in a `Stack` for the first time (it was a bare `Listener > CustomPaint` before) to host the overlay.

**Sketch-orientation default flip for XZ/YZ** (`_addSketchFeature`, `part_screen.dart`): the un-flipped default reads backwards specifically on XZ and YZ (not XY, and a custom Feature-anchored plane has no "which fixed plane" concept to key off at all) - `_pendingOrientationFlip`'s initial value is now `fixedPlane == ReferencePlaneKind.xz || fixedPlane == ReferencePlaneKind.yz` instead of always `false`. (First on-device pass initially had this as XY/XZ; corrected to XZ/YZ once actually checked against all three planes.) Caught a real follow-on correctness issue while implementing this, not just a cosmetic default: `createSketchFeature` always creates a Sketch with `flip=false`/`rotation=0` server-side (it takes no orientation params) - previously harmless, since the client's own old default matched that exactly, but with a non-zero initial default now possible, confirming without ever touching the flip/rotate controls would open a Sketch whose *rendered* view was flipped while its *persisted* backend record still said `flip=false` - a real client/server mismatch that would silently revert on the next reload or Extrude. Fixed by having `_confirmPendingOrientation`'s `newSketch` branch send one `updateSketchOrientation` PATCH before opening the Sketch, but only when the confirmed flip/rotation actually differs from the just-created defaults (a plain unflipped confirm on XY, for instance, still sends nothing, matching prior behaviour exactly).

Verified: `flutter analyze` clean; `flutter test` at 711 total, same one pre-existing sandbox flake, no regressions (no dedicated test file existed for `connection_screen.dart`/the deleted `SettingsHomeScreen`, confirmed via search before deleting). A fresh release build was installed on the test device.

## 2026-07-17 — Sketch-orientation default flip: stopped guessing, computed it

The XZ/YZ default-flip fix from the previous entry got real-world feedback that oscillated ("XZ and YZ need flipping" -> "XY and XZ now require flipping... each time you fix this another breaks") - a genuine whack-a-mole signal that hand-derivation (the same failure mode already caught once this session on the camera-orientation work) was the wrong tool here too. Asked the user for exact numeric target readouts per plane (reusing the now-trusted debug camera-orientation tool) instead of continuing to iterate on verbal "flipped/not flipped" descriptions.

**Stopped hand-deriving, computed instead**: a throwaway scratch test called the real, unmodified `orientationFacingPlane` directly for all 3 fixed planes x both flip states x all 4 `rotationQuarterTurns` values (24 combinations), printing each one's triad-convention readout, and diffed the results against the user's 3 captured targets programmatically rather than by hand.

**Findings, now grounded in actual computation rather than assumption:**
- **YZ**: `flip=false` (the unmodified default) is an *exact* match for the user's target - meaning YZ needed no change at all, and every previous round's guess to flip it was wrong.
- **XY and XZ**: *no* `flip`/`rotationQuarterTurns` combination the app currently supports is an exact match for either target - `flip=true` is the closer of the two available options for both (it correctly flips the plane's own normal/"out" axis reading to match the target), but the in-plane right/up axes still don't fully match. Getting those exactly right looks like it needs a non-zero default `rotationQuarterTurns` too, which nothing currently sets for a new Sketch (always starts at `0`) - flagged as a real, separate follow-up rather than guessed at further this round.

`_pendingOrientationFlip`'s default is now `fixedPlane == ReferencePlaneKind.xy || fixedPlane == ReferencePlaneKind.xz` (YZ back to unflipped). The user separately flagged that some camera *animations* (not just static defaults) "ended up looking the wrong way" - flagged as very likely the same root cause (an animation slerping toward an `orientationFacingPlane`-computed target that was itself wrong looks wrong at the destination, without the animation mechanism itself being at fault) rather than a separate bug, pending re-confirmation once this round's fix is on-device.

Verified: `flutter analyze` clean; `flutter test` at 711 total, same one pre-existing sandbox flake, no regressions. A fresh release build was installed on the test device. Not yet resolved: XY/XZ's in-plane axis orientation (needs a rotation default, not just flip) and whether the flagged animation issue is fully explained by this or is a separate bug - both pending the next on-device round.

**Follow-up same day: the default camera view itself had been dropped.** The user's original calibration message actually contained *four* numeric targets (the default view, plus the three per-plane "first orientation" readouts) - only the latter three got investigated; the default-view target was never carried through to a code change. Caught when the user asked directly ("did you forget to update the default camera?"). Applying the same validate-before-touching-real-code discipline as every fix this round: a throwaway scratch test confirmed the target numbers first (this time, before any production edit, not after a wrong one). The new target turned out to be the *exact negation* of the previous round's own right/up (a 180-degree yaw of it) - X+/Y+ now read screen-*right* (not left), same true-isometric elevation otherwise. `OrbitCamera._isometricOrientation`'s `right`/`up` vectors flipped sign accordingly (`(-1,-1,0)` instead of `(1,1,0)`, etc.); `orbit_camera_test.dart`'s own "matches the on-screen triad exactly" test updated to the corrected numbers. `flutter analyze` clean, `flutter test` 711/same flake, fresh build installed.
...
Same day. The user gave an exact target for XY's default orientation and asked for a full audit: "There are 8 possible orientations for each plane, 3 planes. Each should have its own animation and 'normal to sketch' camera orientation" - directly naming why this matters (it determines what a Sketch's own Horizontal/Vertical constraints mean). Given the pattern of the last several rounds (fix one plane, another breaks), treated this as a signal to stop patching per-plane defaults and find the actual root cause instead.

**Found it, computed rather than guessed.** A scratch test checked, for every plane and every `flip`/`rotationQuarterTurns` combination, whether `xAxis.cross(yAxis)` actually equals `basis.normal` (the property a valid rotation needs). Result: **`flip=true` makes the basis left-handed for every plane, unconditionally** (`xAxis.cross(yAxis) == -basis.normal` whenever flip is applied). `orientationFacingBasis` (`orbit_camera.dart`) built its camera-facing matrix from `basis.normal` directly, ignoring this - but `Quaternion.fromRotation` can only represent a *proper* rotation (determinant +1); handed a left-handed (reflected) input, it silently produces an incorrect result instead of the intended mirrored view. This is why every previous round's per-plane `flip` guess kept "breaking another plane" - the function was fundamentally incapable of correctly representing any `flip=true` state at all, for any plane, so no amount of choosing *which* planes to flip could have converged.

**The fix**: derive the camera's viewing direction from the basis's own actual handedness (`xAxis.cross(yAxis)`) instead of trusting `basis.normal` blindly - a no-op for every already-correct (right-handed, unflipped) case, since they're equal there by construction; only changes anything for a flipped basis, exactly the case that was broken.

**Verified exhaustively, not just for the reported cases.** A scratch test ran the fixed function for all 24 combinations (3 planes x 2 flips x 4 rotations) and diffed against the three independently-captured on-device targets from the previous round: all three now match *exactly* - XY at `flip=true, rotation=1`, XZ at `flip=true, rotation=0`, YZ at `flip=false, rotation=0` (unchanged, confirming the fix is a true no-op there). Every other combination now forms a clean, predictable 4-rotations-x-2-sides pattern instead of the erratic, "stuck" values the broken version produced.

**Code changes**: `orientationFacingBasis` fixed as above (`orbit_camera.dart`). `PartScreen._addSketchFeature`'s per-plane default is now a proper `(bool, int)` pair via new `_defaultPendingOrientationFor` (previously only ever varied `flip`, never `rotationQuarterTurns` - which is exactly why XY and XZ were unreachable through guessing alone, no matter how many rounds). `test/orientation_facing_plane_test.dart` extended with two new groups: one checking `orientationFacingBasis`'s correctness (right=xAxis, up=yAxis, forward=the basis's *actual* handedness) across all 24 combinations generally, and one pinning the three real on-device targets specifically as a direct regression guard for `part_screen.dart`'s own defaults.

Verified: `flutter analyze` clean; `flutter test` at 738 total (27 new), same one pre-existing sandbox flake, no regressions. This also directly addresses the user's separately-flagged "some animations ended up looking the wrong way" report - every plane-facing animation (`animateToPlane`, redefine-orientation, etc.) calls this same now-fixed function, so a wrong destination was the root cause there too, not a separate animation-mechanism bug. Build succeeded; install pending a fresh wireless-ADB reconnect (the device dropped mid-session, same recurring environment quirk as every prior round).

## 2026-07-18 — Face-selection contrast, and Fillet/Chamfer directly from a selected face

Face selection was using the same mid-saturation blue as vertex/edge selection, making it hard to tell a face was actually selected - switched to a brighter, more opaque accent blue of its own. A lone selected face also offers Fillet and Chamfer directly in the context menu now, resolved against that face's own boundary edge loop, instead of being a dead end that required hunting down individual edges by hand first.

## 2026-07-18 — Convert Entities v1 (Sketcher-roadmap Phase 9): pull Body vertices/edges into a sketch as real geometry

New "Convert Entities" tool (FAB + `SketchMode.convert`) lets a sketch pick a sibling Body's vertex or straight edge and materialize it as an ordinary, editable Point/Line - a frozen, one-time copy the user can extrude/edit/delete freely, distinct from Phase 4.3's live-pinned dimensioning references. Reuses that feature's own OCCT vertex-resolution machinery (`resolve_external_vertex_position`, `edge_endpoint_vertex_refs`) via two new document-router endpoints (`convert_body_vertex`/`convert_body_edge`), so this carries no new coordinate-precision risk of its own.

`Sketch.add_or_reuse_point` lets two separately-converted edges that share a Body vertex end up sharing one real Point, so the result can register as a closed profile for Extrude - same reasoning as an earlier `trim_circle` point-reuse fix.

Scope note (v1, later superseded - see the 2026-07-21 entry): a curved Body edge converts as its own straight chord, not its true curve - same limitation the existing external-edge-reference endpoint already had. Real Arc/Circle extraction needed OCCT curve-type introspection this didn't do yet, left as an explicit fast-follow.

Docs: captured the same day's Cast (screen-casting) scoping discussion (Google Cast Application Framework, Custom Receiver, developer-account/hosting requirements) into `roadmap.md`'s open items, so it wasn't lost before the user decides on hosting/account setup.

## 2026-07-18 — Offset Entities v1 (Sketcher-roadmap Phase 9): parallel/concentric copy of a Line/Circle/Arc

New "Offset" ribbon action for a single selected Line, Circle, or Arc - prompts for a signed distance and creates a real, independently editable parallel (Line) or concentric (Circle/Arc) copy. Entirely pure 2D math (`Sketch.offset_line`/`offset_circle`/`offset_arc` in `app/sketch/models.py`, zero OCCT dependency), so every geometry test runs for real in this sandbox, unlike Convert Entities' OCCT-dependent endpoints.

`Sketch.add_or_reuse_point` (already added for Convert Entities/`trim_circle`) keeps two collinear offset Lines connected at their shared join. A wrong assumption caught and fixed while writing the tests: two Lines meeting at a real angle do *not* share a point after independent offsetting (no corner-join logic exists yet) - confirmed as the correct v1 scope boundary via a dedicated test, not silently left untested (corner-joining itself landed the next day as Offset Entities v2, below).

Backend responses bundle each newly-created Point's coordinates alongside the offset entity (`OffsetLineResponse`/`OffsetCircleResponse`/`OffsetArcResponse`) - the client has no other way to learn a fresh Point's position, same "bundle everything the client needs in one round trip" convention the external-edge-reference endpoint already established.

## 2026-07-18 — Convert Entities v2: associative/live-linked Body vertex/edge references

`convert_body_vertex`/`convert_body_edge` now create real, non-construction geometry with associative endpoint Points (`Sketch.add_or_reuse_external_vertex_reference`, new) instead of frozen, one-time copies - reusing Phase 4.3's existing external-references/pinning/`refresh_external_references`/`has_lost_reference` machinery verbatim, since none of it is construction-status-aware. Staleness detection and the feature-tree "lost reference" indicator now work for Convert-Entities-created geometry with zero additional code. Re-pick idempotency is now identity-based (exact Body vertex match) rather than position-epsilon based, which also makes two separately-converted adjacent edges share their common corner Point deterministically rather than by floating-point luck.

This replaced v1 at the same endpoints/wire shapes - no client protocol changes needed at the time. Known, inherited limitation (not introduced or fixed here): `dragTargetPointIdAt` has no exclusion for external-reference Points, so dragging a converted Point visually works but snaps back on the next solve, same as any other pinned reference.

## 2026-07-19 — Auto-prune orphaned Points on delete/trim (on-device feedback)

"When deleting lines, curves, trimming I end up with floating, redundant points" - deleting an entity had always deliberately left its own defining Points behind (a Point might still be shared with something else), correct as a default but leaving genuinely orphaned Points around forever once nothing else references them. Most visible via `trim_circle`: converting a Circle to an Arc only ever reuses the center Point, so the old radius Point and (if drawn via the bare-radius/cardinal tool) all four cardinal Points were left behind permanently.

`Sketch._prune_orphaned_points` (backend, OCC-free, real tests) now runs after `delete_line`/`circle`/`arc`/`ellipse`/`polygon`/`spline`/`text` and `trim_circle`, removing each of the entity's own defining Points that `Sketch._point_deletion_blocker` (the same check `delete_point` already uses) finds no remaining reference to - never touches a Point still shared with anything else, by construction.

The 7 DELETE endpoints changed from a bare 204 No Content to 200 + a `pruned_point_ids` list (a real wire-contract change); the client's `deleteSelected` folds these into its existing capture/undo system rather than treating them as a separate concern, so undo still recreates everything a cascade delete removed, auto-pruned Points included.

## 2026-07-19 — Backend crash fixed: forward-referenced response schemas broke server startup

Pydantic evaluates field type annotations at class-definition time (no `from __future__ import annotations` in `app/sketch/schemas.py`) - the previous entry's `OffsetCircleResponse`/`OffsetArcResponse` had been placed near the top of the file, before `CircleResponse`/`ArcResponse` are defined further down, causing a `NameError` on every server startup. This is exactly the forward-reference pitfall `SketchEntityRefSchema`'s own doc comment already documents for this file - missed because the only verification available for OCC-dependent files in this sandbox was AST syntax parsing, which checks grammar, not name-resolution order. Fixed by moving both classes after `CircleResponse`/`ArcResponse`, and added a script-based ordering-aware check (walks the file top to bottom, flags any class field or function signature referencing a name not yet defined) that would have caught this - run clean against every file touched in this stretch of work.
...
**Real bug, predating this stretch of work (not caused by Convert Entities):** Dimension mode's own body-edge/vertex picking had silently stopped working once Orbit View's cursor-precision model (`drawCursorMode`) started covering Dimension mode. `PartViewport._commitDrawCursor` (the tap handler actually used whenever `drawCursorMode` is true) was never taught to consult `preferEntityPick`/`hitTestBodies`/`onSketchEntityTap` - only the older `_handleTap` path (unreachable while `drawCursorMode` is true) did. Fixed at the root in `_commitDrawCursor` itself, so Convert Entities and the new Offset mode below get real-Body picking for free too; corrected a stale doc comment that had asserted the opposite of what the code did.

**Fix: "tapping a sketch in the tree opens the old 2D editor."** There was never a second route - `SketcherPreferences.defaultUse3DSketcher` was still `false`, so any device that had never visited Settings > Sketcher (or a fresh install) landed every Sketch, new or existing, in the flat 2D canvas. Flipped to `true` now that the 3D sketcher has real feature parity; 2D stays reachable via the in-sketch Orbit-View-toggle FAB.

**New: Offset mode** (`SketchMode.offset`) - the cursor is now available to pick a Line/Circle/Arc directly, via a new `SketchController.pendingOffsetTarget` hand-off the UI layer reacts to with the same distance dialog the ribbon's single-selection Offset action already used.

**New: "Tools" FAB category** groups Dimensions/Trim/Extend/Convert Entities/Offset one level down, mirroring "Sketch Entities"' own two-level shape instead of four flat top-level entries, plus a persistent "Finish" button (mirroring the existing chain/spline one) for all four Tools modes, exiting to Select - previously the only way out was tapping the mode label in the toolbar.

## 2026-07-19 — On-device feedback: hide the draw cursor while dragging an entity in Orbit View

"When I grab something to perform a drag, the cursor should disappear and it should feel like I'm now moving the entity around. After dropping the entity, the cursor should reappear at the drop location." New `PartViewport.suppressDrawCursor`, true while `SketchScreen`'s drag mode has something actually grabbed (a Point, Line, or constraint label) - hides the `drawCursorMode` crosshair entirely so the moving entity itself reads as what's being dragged, instead of a separate reticle riding alongside it. The crosshair's own screen position keeps updating throughout regardless (it's what drives `updateGrabbedPosition`), so it simply reappears at wherever it already is - the drop location - the moment the drag ends, with no extra plumbing needed.

## 2026-07-19 — Convert Entities picks a whole face's edges; Offset picks a Body edge directly; a real body-visibility toggle

**Convert Entities:** selecting a Face now converts every one of its own boundary edges in one tap (sequential, not concurrent - two adjacent edges share a Body vertex, and the backend's own point-reuse only works correctly if the first edge's conversion has actually completed before the second one's request goes out). Reuses the same `faceEdgeIds` resolution `part_screen.dart`'s own Fillet-on-face convenience already established. New `PartViewport.preferEntityPickIncludesFace` widens the real-Body hit filter to include Face for this mode only - Dimension mode still excludes it (no `pickReferenceGhostFace`-equivalent exists there).

**Offset mode** can now pick a Body edge directly ("select edges from other bodies to create sketch geometry offset from the body edges as if they were projected onto the sketch plane") - converts the edge (same associative-Line mechanism Convert Entities itself uses) then hands the result straight to `pendingOffsetTarget`, so one cursor tap reaches the distance prompt instead of requiring a separate Convert-then-Offset session.

**"Show/Hide Reference Body"** now also toggles the real 3D body meshes in Orbit View, not just the 2D canvas's own projected ghost overlay ("give user a clear view of the sketch"). New `PartViewport.bodiesHidden` suppresses only the rendered mesh/edge Nodes - real-Body hit-testing (Convert Entities/Dimension/Offset picking) and camera framing both stay fully intact while bodies are hidden.

## 2026-07-19 — Fix: linear dimensions sliding along the line during camera orbit

On-device feedback: linear dimension labels visibly drifted each frame while orbiting in the 3D-embedded sketcher, because their dragged offset was stored as a raw screen-pixel delta re-interpreted through a normal that rotates with the camera. Mirrors the earlier radial-dimension fix: stores a camera-independent, sketch-local perpendicular distance instead (`SketchController.setLinearOffsetDistance`/`linearOffsetDistanceFor`), with `PartViewport` resolving it directly via a sketch-plane raycast (no screen-space math needed, unlike radial's angle resolve) and the renderer re-deriving a fresh screen offset each frame from the live camera. Scoped to the default (non axis-locked) linear dimension case only, per the reported symptom - a follow-up round (below) widened coverage to the remaining cases.

## 2026-07-19 — A pulsing "glow" added to the selected-face highlight, then reverted the same day

On-device feedback: even after an earlier brightness/contrast pass on the selected-face highlight color, a static tint was still hard to notice. Added a repeating brighten-toward-white pulse (`pulseTowardWhite`, tested) driven by an `AnimationController` that only mutates the already-bound `UnlitMaterial.baseColorFactor` in place each tick - no geometry rebuild, avoiding the GPU-rebuild-storm class of bug fixed earlier for the Rectangle tool. The very next on-device round (below) found this read as "moving", not "lit up" as intended, and reverted it in favor of a palette-distance approach.

## 2026-07-19 — Offset Entities v2: chain-aware, corner-joining multi-entity Offset

On-device feedback: "offset should allow the selection of multiple entities and should operate intuitively. eg offset two connected lines results in two connected lines... if the origin lines are connected, the offset lines should be connected effectively trimming or extending the new lines to their intersect."

**Backend:** new `Sketch.offset_chain` offsets each given Line/Arc independently (same math/sign convention as `offset_line`/`offset_arc`), then for every original Point shared by exactly two of the given entities, resolves their new intersection (`line_vs_line` - new - /`line_vs_circle`/`circle_vs_circle`) and uses it as the shared corner, nearest-to-original tiebreaking when a curve gives two candidates. A branch/T-junction point (shared by 3+) or an unshared end just keeps its raw offset - no error. New `POST /sketches/{id}/offset-chain` endpoint + `OffsetChainRequest`/`OffsetChainResponse` schemas. 16 new, fully executable pytest cases (this module has no OCCT dependency) cover the corner-join math itself, including hand-verified exact corner coordinates.

**Client:** `SketchMode.offset`'s cursor pick now accumulates Line/Arc taps into `selectionSet` (toggling on a repeat tap) instead of showing the distance dialog immediately - a Circle tap still goes straight to the existing single-entity dialog, since it has no chain endpoints to join. The Tools flyup's Finish button (`finishOffsetChain`) submits the picked set: zero picks exits offset mode, exactly one uses the existing fast single-entity `offsetLine`/`offsetArc` path, two or more hand off to a new `pendingOffsetChainTargets` dialog backed by the backend's offset-chain endpoint (`SketchController.offsetChain`).
...
**Face highlight:** reverted the pulsing "glow" animation from earlier the same day (wrong read - the user wanted "lit up", not motion) in favor of picking whichever of a small high-saturation palette is furthest (RGB distance) from the user's own Body Colour, so it can no longer collide with an arbitrary chosen body color the way the old single fixed blue could.

**Real occlusion bug fixed:** `hitTestBodies` let a vertex/edge on the far side of a shaded body win purely on 2D screen-space proximity, with no regard for a nearer face actually rendered in front of it - "picking edges through faces". Added an opt-in face-occlusion check (only when the render mode actually shows filled faces and bodies aren't hidden) that drops a vertex/edge candidate sitting behind the nearest face along the ray.

**Linear dimension camera-slide, part 2:** the original fix only covered the general-direction case of a point-to-point dimension. Extended the same camera-independent offset-magnitude technique to the vertical/horizontal-orientation case (very likely what "one end still slides" actually was) and to the separate Line-to-Line distance dimension kind, which the original fix never touched at all.

**Fixed Convert Entities never showing a cursor:** `drawCursorMode`'s own gate was missing `SketchMode.convert` entirely, despite an existing doc comment claiming it had been added - a real regression, not a design gap. The existing orbit/cursor toggle FAB (already unconditional whenever Orbit View is active) needed no separate change once this gate was fixed.
...
**Hover highlighting fixed for Dimension/Convert Entities/Offset:** the `PartViewport` selection filter feeding `_recomputeHover` was a permanently vertex/edge/face-off constant, so nothing but Sketch entities ever hover-highlighted in those modes even though the tap path already targeted real Body geometry via its own separate filter. Made it mode-aware to match what's actually tappable.

**Offset tool overhauled:** replaced the modal distance dialog with a non-modal bottom fly-up bar (`OffsetValueBar`, mirroring `SketchDimensionBar`'s own "taps still reach the canvas" shape) driven by a live `SketchController.offsetPreviewDistance`, and added a live dashed ghost preview (`offsetPreviewGhosts`, reusing the existing `LineGhost`/`CircleGhost`/`ArcGhost` draw-tool ghost types and their rendering in both the 2D canvas and the embedded 3D view) so flipping the typed value's sign visibly flips which side the offset lands on. Unified the previous single/multi-entity `pendingOffsetTarget`/`pendingOffsetChainTargets` one-shot hand-off into one `offsetPreviewTargets` list, since the value bar needs to stay populated for the whole editing session, not just one frame.

**Tools flyup** now lays out in two rows of two, matching Sketch Entities' own two-row grid instead of one long vertical column.
...
**"Show/Hide Reference Body" FAB fixed:** it flipped `bodiesHidden` but `PartViewport`'s `didUpdateWidget` never diffed it, so neither `_syncMeshNode` nor `_syncEdgesNode` ever re-ran - the toggle updated state with no visible effect at all. Added it to both trigger conditions.

**Root-caused "dynamic face highlight isn't working":** the embedded 3D sketcher's Orbit View defaults Body opacity below 100%, which routes the Body's own material onto `flutter_scene`'s translucent pass - the same pass this codebase has already found has an unreliable on-device depth test (see `buildMeshEdgesNode`'s own doc comment on the identical bug for edges). A face highlight sits at the exact same depth as the Body's own surface, so the translucent Body could redraw over the opaque highlight underneath it. Added `biasTrianglesTowardCamera` (mirroring the existing `biasSegmentsTowardCamera` fix for edges) and applied it everywhere a face highlight is built, so it sits at a real, if small, distance in front of the surface instead of depending on that depth test.

`kEdgeStrokeWidth` bumped `1.1 -> 1.4`.

## 2026-07-21 — Curved Body edges convert as real Arcs instead of always flattening to a chord

On-device feedback: "when I offset a curved edge it creates a straight line." Convert Entities'/Offset's edge conversion (`convert_body_edge`) had always flattened a circular Body edge to a straight chord between its two endpoints - the v1 scope note from the 2026-07-18 Convert Entities entry above, never revisited until now.

**New pure-math layer** (`app.document.plane_geometry`, deliberately OCCT-free so every test runs for real in this sandbox): `signed_distance_to_plane` (a point's off-plane distance along the basis normal); `resolve_planar_circle` (whether a circle's centre/axis actually lie flat in a given `ResolvedPlane`, returning its local-2D centre/radius or `None` if not coplanar - both an axis-parallel check and a centre-on-plane check, since either alone is insufficient: a circle in a parallel-but-offset plane would pass an axis-only check); `resolve_ccw_arc_endpoints` (a real OCCT edge sweeps whichever direction it actually sweeps, with no guarantee it matches `Sketch.add_arc`'s own always-CCW convention - resolved by sampling a genuine third point partway along the real curve and checking which of the two candidate sweep directions it actually falls on). 10 new tests (`backend/tests/test_feature_curved_edge_conversion.py`), all executable in this sandbox.

**OCCT-bound glue** (`resolve_circular_edge_arc`, `app.document.extrude`): pulls a Body edge's real curve type (`BRepAdaptor_Curve.GetType() == GeomAbs_Circle`), circle centre/axis/radius, and a genuine mid-parameter sample point, then delegates every actual decision to the pure-math layer above - `None` for anything not circular or not coplanar, falling back to the existing chord-Line behaviour exactly as before.

**Wiring:** `convert_body_edge` tries the Arc path first, falling back to the chord Line only when it returns `None`. `ConvertEdgeResponse` now carries exactly one of `line`/`arc` (plus `center_point`, only present alongside `arc` - a v1 limitation of its own: unlike the associative `start_point`/`end_point`, the centre is a plain, non-associative Point, since nothing currently pins a circular edge's own centre the way a vertex reference pins a corner). A full circle (both topological endpoints the same Body vertex) still 422s as `degenerate_edge` before ever reaching curve-type detection - real Circle extraction is a separate, not-yet-built follow-up.

**Client:** new `ConvertEdgeResultDto` (line-or-arc, mirroring the backend schema) replaces the old always-a-Line DTO; `_convertBodyEdgeToLocalState` now returns a kind-aware `SketchSelection` instead of a bare Line id, with full undo support for the Arc case (deletes the Arc, its centre Point, and any freshly-created endpoint Points). Also fixed, found while touching this path: picking a Body edge for Offset used to hand the result straight to `offsetPreviewTargets`, immediately opening the value bar - correct back when Offset was single-entity only, but never updated once Offset Entities v2 added multi-entity chain picking. Now accumulates into the same chain pick set as any other Line/Arc tap, so further Body-edge or Sketch-entity taps keep adding to the same pick set before Finish opens the value bar. Sketch line thickness nudged up slightly per further feedback (2D canvas `1.5 -> 1.8`/`2.25 -> 2.7`; embedded 3D `1.2 -> 1.5`).
