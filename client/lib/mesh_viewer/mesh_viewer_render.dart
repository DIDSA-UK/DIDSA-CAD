...
/// One contiguous triangle range (already clamped to
/// [_kMaxTrianglesPerBatch]/[_kMaxVerticesPerBatch]) tagged with the
/// [MeshBody] id it belongs to - the STEP multi-body counterpart of
/// [_buildMeshViewerBatches]' plain, body-less ranges. A pure data class
/// (no `flutter_scene`/`flutter_gpu` types at all) so
/// [computeBodyAwareBatchRanges] is directly unit-testable without a GPU
/// context - see `test/mesh_viewer/step_body_visibility_test.dart` -
/// matching `mesh_data.dart`'s own pure/GPU-independent split, extracted
/// specifically for that purpose even though it lives in this
/// otherwise-GPU-touching file.
class BodyBatchRange {
  final int bodyId;
  final int startTriangle;
  final int triangleCount;
  const BodyBatchRange({required this.bodyId, required this.startTriangle, required this.triangleCount});
}

/// Splits each `(bodyId, startTriangle, triangleCount)` entry in
/// [bodyRanges] further wherever it exceeds [maxTrianglesPerBatch] - a
/// body's own triangles stay taggable as one unit (one [BodyBatchRange])
/// unless they're too many for a single `flutter_scene` batch, in which
/// case that body's own range splits across multiple [BodyBatchRange]s, all
/// sharing the same [BodyBatchRange.bodyId] - never merging two different
/// bodies' triangles into one range, and never splitting a batch across a
/// body boundary.
List<BodyBatchRange> computeBodyAwareBatchRanges(
  List<({int bodyId, int startTriangle, int triangleCount})> bodyRanges, {
  int maxTrianglesPerBatch = _kMaxTrianglesPerBatch,
}) {
  final out = <BodyBatchRange>[];
  for (final body in bodyRanges) {
    for (var start = 0; start < body.triangleCount; start += maxTrianglesPerBatch) {
      final end = math.min(start + maxTrianglesPerBatch, body.triangleCount);
      out.add(BodyBatchRange(
        bodyId: body.bodyId,
        startTriangle: body.startTriangle + start,
...
/// Builds one [Node] list per body id (via [computeBodyAwareBatchRanges] +
/// [_buildMeshViewerBatches]) - the STEP multi-body counterpart of
/// [buildMeshViewerNodes]'s per-[MeshMaterialGroup] case, sharing the one
/// [material] across every body (STEP/B-rep has no per-body texture concept
/// the way a glTF material group does). The returned map is what
/// [addBodyNodes]/[removeBodyNodes] key off of to toggle a single body's
/// visibility in [Scene] with no geometry rebuild at all - see those
/// functions' own doc comments.
Map<int, List<Node>> buildBodyTaggedMeshViewerNodes(
  DecodedMesh mesh,
  PhysicallyBasedMaterial material,
  List<({int bodyId, int startTriangle, int triangleCount})> bodyRanges,
) {
  final result = <int, List<Node>>{};
  for (final range in computeBodyAwareBatchRanges(bodyRanges)) {
    final nodes = _buildMeshViewerBatches(
      mesh,
      material,
      startTriangle: range.startTriangle,
      triangleCount: range.triangleCount,
    );
    result.putIfAbsent(range.bodyId, () => []).addAll(nodes);
  }
  return result;
}

/// Adds [bodyId]'s own [Node]s (from [buildBodyTaggedMeshViewerNodes]) to
/// [scene] - O(1) per body, no geometry rebuild, mirroring
/// `mesh_viewer_screen.dart`'s existing `_syncFacetsAndWireframe` add/remove
/// pattern for the Facets/Mesh View-menu toggles. A no-op if [bodyId] isn't
/// in [nodesByBody] (nothing built for it - shouldn't happen for a real
/// body id, but harmless either way).
void addBodyNodes(Scene scene, Map<int, List<Node>> nodesByBody, int bodyId) {
  for (final node in nodesByBody[bodyId] ?? const []) {
    scene.add(node);
  }
}

/// [addBodyNodes]'s inverse - removes [bodyId]'s own [Node]s from [scene]
/// (the Mesh Viewer's tap-to-hide/checkbox-list "hide this body" action).
void removeBodyNodes(Scene scene, Map<int, List<Node>> nodesByBody, int bodyId) {
  for (final node in nodesByBody[bodyId] ?? const []) {
    scene.remove(node);
  }
}
