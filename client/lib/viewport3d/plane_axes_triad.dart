...
/// (see [buildAxesTriadNode]) - the geometric counterpart to `triad.dart`'s
/// screen-space, orientation-only world compass. Where that compass always
/// shows the *world*'s own X/Y/Z, this shows one specific plane's *own*
/// embedded local X/Y axes, drawn at that plane's real position/orientation
/// in the same scene - so the two can be compared directly, by eye, in a
/// single view: does this plane's red arrow point the same way the world
/// compass's red arrow does (it should, for any plane whose local X truly
/// is world-aligned in the way its own code assumes)?
///
...
/// Builds the [Node] rendering one plane's own X(red)/Y(green) axes as two
/// short world-space line segments from [origin] - [xAxis]/[yAxis] are
/// assumed already unit-length and orthonormal (guaranteed by every
/// `ResolvedPlane`/`SketchPlaneBasis` this app produces, fixed or custom
/// alike), so no normalization happens here.
///
