...
void main() {
  ...
  testWidgets(
    'A2: double-tap-then-drag in selection mode calls onSelectionToggle via box hit-test',
    (tester) async {
      var toggleCount = 0;
          ...
          home: Scaffold(
            body: SizedBox(
              ...
              child: PartViewport(
                ...
                onBackgroundTap: () {},
                selectionMode: true,
                onSelectionToggle: (_) => toggleCount++,
                onClearSelection: () {},
              ),
            ),
          ),
      ...
      // Simulate double-tap-then-drag: first tap registers the first half of
      // the double-tap; second tap-down triggers onDoubleTapDown which arms the
      // box; dragging across the full viewport guarantees every entity whose
      // projected position is on screen falls inside the box.
      const center = Offset(200, 200);
      await tester.tapAt(center);
      await tester.pump(const Duration(milliseconds: 100));

      // Second pointer-down: GestureDetector fires onDoubleTapDown.
      final gesture = await tester.startGesture(center);
      await tester.pump();

      // Drag well past the 4dp box-activation threshold to cover the viewport.
      await gesture.moveBy(const Offset(150, 150));
      await tester.pump();
      await gesture.up();
      await tester.pump();

      // At least one entity should have been toggled via the box hit-test
      // (the mesh has a topology vertex at (0,0,0) that projects near the
      // viewport centre from the default camera orientation).
  ...
  testWidgets(
    'A2: double-tap with no drag (< 4 dp) commits no box selection',
    (tester) async {
      var toggled = false;
        ...
        MaterialApp(
          home: Scaffold(
            body: SizedBox(
              ...
              height: 400,
              child: PartViewport(
                // mesh: null so the first tap fires onClearSelection (not
                // onSelectionToggle), making the toggled flag a reliable
                // signal that the box-selection path fired unexpectedly.
                mesh: null,
                selectedPlane: null,
                onPlaneTap: (_) {},
                onBackgroundTap: () {},
      ...
      // Double-tap at the same spot without dragging: onDoubleTapDown arms the
      // box, but the pointer-up with < 4dp travel calls _cancelBoxSelection,
      // which clears state and fires nothing.
      const center = Offset(200, 200);
      await tester.tapAt(center);
      await tester.pump(const Duration(milliseconds: 100));

      final gesture = await tester.startGesture(center);
      await tester.pump(); // onDoubleTapDown fires
      // Tiny nudge (1 dp) stays under the 4dp threshold.
      await gesture.moveBy(const Offset(1, 0));
      await tester.pump();
      await gesture.up();
      await tester.pump();

      // The box-selection path must not have called onSelectionToggle.
      expect(toggled, isFalse);
      expect(tester.takeException(), isNull);
    },
  );
